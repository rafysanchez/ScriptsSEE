
function wordUpper(text) {
    debugger;
    var words = text.toLowerCase().split(" ");
    for (var a = 0; a < words.length; a++) {
        var w = words[a];
        words[a] = w[0].toUpperCase() + w.slice(1);
    }
    return words.join(" ");
}


function gerarinformacaoResponsabilidadeFiscal(data) {

    
    var config = {
        pageSize: "A4",
        pageOrientation: "portrait",
        // pageOrientation: 'landscape',
        title: "InformacaoResponsabilidadeFiscal",
    };

    sedPdfExporter.normalizeConfig(config);

    config.codigoUnidadeGestora = data[0].unidadeGestora.codigoUnidadeGestora != undefined ? data[0].unidadeGestora.codigoUnidadeGestora : "";
    config.nomeUnidadeGestora = data[0].unidadeGestora.nomeUnidadeGestora != undefined ? data[0].unidadeGestora.nomeUnidadeGestora : "";
    config.NmUnidGest = data[0].unidadeGestora.codigoUnidadeGestora != undefined ? data[0].unidadeGestora.codigoUnidadeGestora : "";
    config.descricaoEnderecoNumero = data[0].unidadeGestora.descricaoEnderecoNumero != undefined ? data[0].unidadeGestora.descricaoEnderecoNumero : "";
    config.descricaoMunicipio = data[0].unidadeGestora.descricaoMunicipio != undefined && data[0].unidadeGestora.descricaoMunicipio != "" ? titleize(data[0].unidadeGestora.descricaoMunicipio.toLowerCase()) : "São Paulo";
    config.numeroProcesso = data[0].processo.numeroProcesso != undefined ? data[0].processo.numeroProcesso : "";
    config.NrContrato = data[0].numeroContrato != undefined ? data[0].numeroContrato : "";
    config.NmCpfCgcCredor = data[0].nomeCredor != undefined ? data[0].nomeCredor : "";
    config.NrCpfCgcCredor = data[0].numeroCpfCnpj != undefined ? data[0].numeroCpfCnpj.replace(/^(\d{2})(\d{3})(\d{3})(\d{4})(\d{2})/, "$1.$2.$3/$4-$5") : "";
    config.dsAssunto = data[0].assuntoPlanejado.descricaoAssuntoPlanejado != undefined ? data[0].assuntoPlanejado.descricaoAssuntoPlanejado : "";
    config.DsClausulaContrato = data[0].clausulaContrato != undefined ? data[0].clausulaContrato : "";
    config.DtReajustemes = data[0].dataBaseReajuste != undefined ? dataAtualFormatadames(data[0].dataBaseReajuste) : "";
    config.DtReajusteano = data[0].dataBaseReajuste != undefined ? dataAtualFormatadaano(data[0].dataBaseReajuste) : "";
    config.DtReajuste = data[0].dataBaseReajuste != undefined ? dataAtualFormatada(data[0].dataBaseReajuste) : "";
    config.NrPercentual = data[0].percentualReajuste != undefined ? data[0].percentualReajuste + '%' : "";
    config.DsFolhaCadTerc = data[0].folhaCadTerc != undefined ? data[0].folhaCadTerc : "";
    config.IdPeriodoContrato = data[0].idPeriodoContrato != undefined ? data[0].idPeriodoContrato : "";
    config.DsUnidadeContratada = data[0].descricaoUnidadeContratada != undefined ? data[0].descricaoUnidadeContratada : "";
    config.VLReajuste = data[0].valorReajuste != undefined ? data[0].valorReajuste.toFixed(2).replace(".", ",").replace(/(\d)(?=(\d{3})+(?!\d))/g, "$1.") : "";
    config.VLReajustado = data[0].valorReajustado != undefined ? data[0].valorReajustado.toFixed(2).replace(".", ",").replace(/(\d)(?=(\d{3})+(?!\d))/g, "$1.") : "";
    config.descricaoObjeto = data[0].descricaoObjeto != undefined ? data[0].descricaoObjeto : "";
    config.dataVigenciaIni = data[0].dataVigenciaIni != undefined ? dataAtualFormatada(data[0].dataVigenciaIni) : "";
    config.dataVigenciaFim = data[0].dataVigenciaFim != undefined ? dataAtualFormatada(data[0].dataVigenciaFim) : "";
    config.nomeGestor = data[0].nomeGestor != undefined ? titleize(data[0].nomeGestor) : "";
    config.dataImpressao = data[0].dataImpressao != undefined ? dataAtualFormatada(data[0].dataImpressao) : "";
    config.DtImpressaodia = data[0].dataImpressao != undefined ? dataAtualFormatadadia(data[0].dataImpressao) : "";
    config.DtImpressaomes = data[0].dataImpressao != undefined ? dataAtualFormatadames(data[0].dataImpressao) : "";
    config.DtImpressaonomemes = data[0].dataImpressao != undefined ? dataAtualFormatadanomemes(data[0].dataImpressao) : "";
    config.DtImpressaoano = data[0].dataImpressao != undefined ? dataAtualFormatadaano(data[0].dataImpressao) : "";
    config.nrFolha = data[0].nrFolha != undefined ? data[0].nrFolha : "";
    config.codProgramaDeTrabalho = data[0].codProgramaTrabalho != undefined ? data[0].codProgramaTrabalho : "";
    config.Elemento = data[0].Elemento != undefined ? data[0].Elemento : "";
    config.Fonte = data[0].Fonte != undefined ? data[0].Fonte : "";
    config.valorAssuntoPlanejado = data[0].valorAssuntoPlanejado != undefined ? data[0].valorAssuntoPlanejado.toFixed(2).replace(".", ",").replace(/(\d)(?=(\d{3})+(?!\d))/g, "$1.") : "";
    config.nomeResponsavel = data[0].usuarioResponsavel.nmResponsavel != undefined ? titleize(data[0].usuarioResponsavel.nmResponsavel) : "";
    config.ordenadorDespesa = data[0].ordenadorDespesa.nmResponsavel != undefined ? titleize(data[0].ordenadorDespesa.nmResponsavel) : "";
    config.valorTotalContrato = data[0].valorTotalContrato != undefined ? data[0].valorTotalContrato.toFixed(2).replace(".", ",").replace(/(\d)(?=(\d{3})+(?!\d))/g, "$1.") : "";
    config.valorTotalProrrogacao = data[0].valorTotalProrrogacao != undefined ? data[0].valorTotalProrrogacao.toFixed(2).replace(".", ",").replace(/(\d)(?=(\d{3})+(?!\d))/g, "$1.") : "";
    
 
    for (var i = 0; i < data[0].listaLeis.length; i++) {

        if(data[0].listaLeis[i].flagTipo == 1)
        {

            config.descricaoLeiPPA = data[0].listaLeis[i].descricaoLei != undefined ? data[0].listaLeis[i].descricaoLei : "";
            config.dataLeiPPA = data[0].listaLeis[i].dataLei != undefined ? dataAtualFormatada(data[0].listaLeis[i].dataLei) : "";
            config.dataLeiPPAdia = data[0].listaLeis[i].dataLei != undefined ? dataAtualFormatadadia(data[0].listaLeis[i].dataLei) : "";
            config.dataLeiPPAmes = data[0].listaLeis[i].dataLei != undefined ? dataAtualFormatadames(data[0].listaLeis[i].dataLei) : "";
            config.dataLeiPPAano = data[0].listaLeis[i].dataLei != undefined ? dataAtualFormatadaano(data[0].listaLeis[i].dataLei) : "";
            config.dataLeiPPAnomemes = data[0].listaLeis[i].dataLei != undefined ? dataAtualFormatadanomemes(data[0].listaLeis[i].dataLei) : "";
         
            config.iniVigenciaPPA = data[0].listaLeis[i].numeroAnoVigencia != undefined ? data[0].listaLeis[i].numeroAnoVigencia : "";
            config.fimVigenciaPPA = data[0].listaLeis[i].numeroFimVigencia != undefined ? data[0].listaLeis[i].numeroFimVigencia : "";

        }
        else if (data[0].listaLeis[i].flagTipo == 2) {


            config.descricaoLeiLDO = data[0].listaLeis[i].descricaoLei != undefined ? data[0].listaLeis[i].descricaoLei : "";
            config.dataLeiLDO = data[0].listaLeis[i].dataLei != undefined ? dataAtualFormatada(data[0].listaLeis[i].dataLei) : "";
            config.dataLeiLDOdia = data[0].listaLeis[i].dataLei != undefined ? dataAtualFormatadadia(data[0].listaLeis[i].dataLei) : "";
            config.dataLeiLDOmes = data[0].listaLeis[i].dataLei != undefined ? dataAtualFormatadames(data[0].listaLeis[i].dataLei) : "";
            config.dataLeiLDOnomemes = data[0].listaLeis[i].dataLei != undefined ? dataAtualFormatadanomemes(data[0].listaLeis[i].dataLei) : "";
            config.dataLeiLDOano = data[0].listaLeis[i].numeroAnoVigencia != undefined ? data[0].listaLeis[i].numeroAnoVigencia : "";

        }
        else if (data[0].listaLeis[i].flagTipo == 3)
        {
            config.descricaoLeiLOA = data[0].listaLeis[i].descricaoLei != undefined ? data[0].listaLeis[i].descricaoLei : "";
            config.dataLeiLOA = data[0].listaLeis[i].dataLei != undefined ? dataAtualFormatada(data[0].listaLeis[i].dataLei) : "";
            config.dataLeiLOAdia = data[0].listaLeis[i].dataLei != undefined ? dataAtualFormatadadia(data[0].listaLeis[i].dataLei) : "";
            config.dataLeiLOAmes = data[0].listaLeis[i].dataLei != undefined ? dataAtualFormatadames(data[0].listaLeis[i].dataLei) : "";
            config.dataLeiLOAnomemes = data[0].listaLeis[i].dataLei != undefined ? dataAtualFormatadanomemes(data[0].listaLeis[i].dataLei) : "";
            config.dataLeiLOAano = data[0].listaLeis[i].numeroAnoVigencia != undefined ? data[0].listaLeis[i].numeroAnoVigencia : "";
        }
       
    }
  


 
    if (config.codigoUnidadeGestora == "080101" || config.codigoUnidadeGestora == "080102" ||
    config.codigoUnidadeGestora == "080011" || config.codigoUnidadeGestora == "080104" ||
    config.codigoUnidadeGestora == "080259" || config.codigoUnidadeGestora == "080353" ||
    config.codigoUnidadeGestora == "080356" || config.codigoUnidadeGestora == "080357" ||
    config.codigoUnidadeGestora == "080358" || config.codigoUnidadeGestora == "080359" ||
    config.codigoUnidadeGestora == "080040" || config.codigoUnidadeGestora == "080360"
    ) {

        config.descricaoEnderecoNumero = "Endereço SEE : Praça da República, 53 - República, São Paulo/SP";
    }
   

    config.descricaoProgramaTrabalho = data[0].descricaoProgramaTrabalho != undefined ? data[0].descricaoProgramaTrabalho : "";
    config.descricaoNaturezaDespesa = data[0].descricaoNaturezaDespesa != undefined ? data[0].descricaoNaturezaDespesa : "";

    debugger;
    config.docGenerator = function (config) {
        var doc = {

            pageMargins: [30, 150, 50, 30],
            header: [
           {
               margin: [50, 20, 50, 0],
               table: {
                   widths: ["5%", "95%"],
                   body: [
                       [
                           {
                               stack: [
                                   {
                                       image: 'data:image/png;base64,',
                                       width: 65,
                                       height: 75
                                   }
                               ]
                           },
                           {
                               stack: [
                                   {
                                       text: 'GOVERNO DO ESTADO DE SÃO PAULO',
                                       fontSize: 15,
                                       bold: true
                                   },
                                   {
                                       text: 'SECRETARIA DE ESTADO DA EDUCAÇÃO',
                                       fontSize: 14,
                                       bold: true
                                   },
                                   {
                                       text: config.nomeUnidadeGestora,
                                       fontSize: 11,
                                       bold: false
                                   },

                               ],
                               alignment: "center",
                           }
                       ]
                   ]
               },
               layout: "noBorders"
           },
           {
               alignment: "center",
               stack: [

                   {
                       text: "INFORMAÇÃO SOBRE A LEI DE RESPONSABILIDADE FISCAL"
                   }
               ],
           }
            ],
            content: [
                     {
                         margin: [20, 10, 50, 0],
                         stack: [
                                 {
                                     fontSize: 10,
                                     text:
                                      [                     
                                         { text: "PROCESSO Nº : ", bold: true },
                                         { text: config.numeroProcesso, bold: false },                     
                                      ],
                                 },
                     
                                 {
                                     fontSize: 10,
                                     text:
                                      [                     
                                         { text: "CONTRATO Nº : ", bold: true },
                                         { text: config.NrContrato, bold: false },                     
                                      ],
                                 },
                     
                                 {
                                     fontSize: 10,
                                     text:
                                         [                     
                                     { text: "ASSUNTO PLANEJADO : ", bold: true },
                                     { text: config.descricaoObjeto, bold: false },                     
                                         ],
                                }
                         ]
                     
                     },
                     {
                         margin: [40, 20, 50, 0],
                         fontSize: 10,
                         alignment: 'justify',
                         stack: [
                                   {
                                       text: [
                                       { text: "Declaro, nos termos no inciso ||, artigos 16 da Lei Complementar nº 101, de 04 de maio de 2000," },
                                       ],
                                   }
                         ]
                     },
                     {
                         margin: [20, 0, 50, 0],
                         fontSize: 10,
                         alignment: 'justify',
                         stack: [
                                   {
                                       text: [
                                       { text: " que a despesa ora solicitada está compatível com o disposto na Lei nº " },
                                       { text: config.descricaoLeiPPA },
                                       { text: ", de " },
                                       { text: config.dataLeiPPAdia },
                                       { text: " de " }, 
                                       { text: config.dataLeiPPAnomemes },
                                       { text: " de " },
                                       { text: config.dataLeiPPAano },
                                       { text: ", que instituiu o Plano Plurianual para o quadriênio - " },
                                       { text: config.iniVigenciaPPA },
                                       { text: "; assim como com a Lei Orçamentária Anual nº " },
                                       { text: config.descricaoLeiLOA },
                                       { text: ", de " },
                                       { text: config.dataLeiLOAdia },
                                       { text: " de " },
                                       { text: config.dataLeiLOAnomemes },
                                       { text: " de " },
                                       { text: config.dataLeiLOAano },
                                       { text: ", que dispôs sobre as DIRETRIZES ORÇAMENTÁRIAS para o exercício de " },
                                       { text: config.dataLeiLOAano },
                                       { text: "." },
                                       

                                       ],
                                   }
                         ]
                     },
                      {
                          margin: [40, 0, 50, 0],
                          fontSize: 10,
                          alignment: 'justify',
                          stack: [
                                    {
                                        text: [
                                        { text: "Em tempo, destaco que as despesas ora solicitada deverá onerar a dotação orçamentaria de :" },
                                        ],
                                    }
                          ]
                      },
                       {
                           margin: [20, 0, 50, 0],
                           fontSize: 10,
                           alignment: 'justify',
                           stack: [
                                     {
                                         text: [                                         
                                         { text: config.dataLeiLDOano },
                                         ],
                                     }
                           ]
                       },
                       {
                           margin: [20, 10, 50, 0],
                           fontSize: 10,
                           alignment: 'justify',
                           stack: [
                                     {
                                         text: [
                                         { text: "Programa de Trabalho : ", bold: true },
                                         { text: config.codProgramaDeTrabalho },
                                         ],
                                     },
                                      {
                                          text: [
                                          { text: "Elemento : ", bold: true },
                                          { text: config.Elemento },
                                          ],
                                      },
                                      {
                                          text: [
                                          { text: "Valor R$ : ", bold: true },
                                          { text: config.valorTotalProrrogacao },
                                          ],
                                      },
                           ]
                       },
                       {
                           margin: [40, 10, 50, 0],
                           fontSize: 10,
                           alignment: 'justify',
                           stack: [
                                     {
                                         text: [
                                         { text: "Nos termos da legislação supracitada, confirmo que a despesa em tela está adequada aos" },
                                         ],
                                     }
                           ]
                       },
                        {
                            margin: [20, 0, 50, 0],
                            fontSize: 10,
                            alignment: 'justify',
                            stack: [
                                      {
                                          text: [
                                          { text: " objetivos, prioridades e metas prevista para o período, não caracterizando nenhum infringência a qualquer dispositivo legal." },
                                          ],
                                      }
                            ]
                        },
                          {
                              margin: [20, 50, 40, 0],
                              fontSize: 10,
                              alignment: "right",
                              stack: [
                                  {
                                      text:
                                          [
                                                { text: config.descricaoMunicipio + ", " },
                                                { text: config.DtImpressaodia + " de " + config.DtImpressaonomemes + " de " + config.DtImpressaoano },
                                          ],
                                  }

                              ],
                          },
                          {
                              margin: [20, 40, 50, 0],
                              fontSize: 10,
                              alignment: "center",
                              stack: [
                                  { text: "_______________________________________________" },
                                  { text: config.ordenadorDespesa },
                                  { text: "Ordenador Despesa" },

                              ],

                          }






            ],
            footer: function (currentPage, pageCount) {
                return {
                    margin: [50, 0, 40, 0],
                    columns: [
                        {
                            table: {
                                widths: [100, 100, 240],
                                headerRows: 1,
                                fontSize: 9,
                                body: [
                                         [{ text: "Endereço UGE : " + config.descricaoEnderecoNumero + "                            pagina(s) " + currentPage.toString() + " de " + pageCount, colSpan: 3, fontSize: 9, border: [false, true, false, false], bold: false, color: 'gray' }],

                                ]
                            },
                        },
                    ]
                };

            },
            styles: {
                destaque: {
                    margin: [20, 0, 40, 0],
                    fontSize: 13,
                    bold: true,
                },
                simples: {
                    fontSize: 13,
                    bold: false,
                    },
                invisivel: {
                    margin: [20, 0, 20, 0],
                    fontSize: 20,
                    color: 'white',
                },
                tabela: {
                    margin: [20, 0, 40, 0], color: 'gray'
                },
                header: {
                    fontSize: 10,
                    bold: false,
                    margin: [50, 0, 30, 0]
                },
                visivel: {
                    margin: [20, 0, 20, 0],
                    fontSize: 12,
                    color: 'black',
                },
            },
        }
        return doc;
    };
    sedPdfExporter.exportPdf(config);
}