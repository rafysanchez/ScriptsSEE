function obterDemonstrativoReajuste(IdContrato) {
   
    if (IdContrato != 0) {
        
        $.ajax({
            cache: false,
            dataType: 'JSON',
            contentType: 'application/x-www-form-urlencoded; charset=utf-8',
            //url: "@Url.Action('GerarDemonstrativoReajuste', 'Reajuste')",            
            url: '/Contratos/Reajuste/GerarDemonstrativoReajuste',
            data: JSON.stringify({ "IdContrato": IdContrato }),
            type: 'POST',
            success: function (data) {
                gerarDemonstrativoReajuste(data);
            },
            contentType: 'application/json; charset=utf-8',
            error: function (jqXHR, txtStatus, errorThrown) {
                mensagemAlerta.ErroAjax('Ocorreu um erro durante o processo' + errorThrown);
            }
        });
    }
    else {
        mensagemAlerta.ErroAjax('Ocorreu um erro durante o processo');
    }
}

function dataAtualFormatada(result) {
    var d = result;
    var unix = +d.replace(/\D/g, '');
    var date = new Date(unix);
    dia = date.getDate();
    mes = date.getMonth() + 1;
    if (mes.toString().length == 1)
        mes = "0" + mes;
    ano = date.getFullYear();
    return [dia, mes, ano].join('/');

}

function dataAtualFormatadadia(resultdia) {
    
    var d = resultdia;
    var unix = +d.replace(/\D/g, '');
    var date = new Date(unix);
    var dia = (date.getDate());
    if (dia.toString().length == 1)
        dia = "0" + dia;
    return [dia];

}

function dataAtualFormatadames(resultmes) {
    debugger;
    var d = resultmes;
    var unix = +d.replace(/\D/g, '');
    var date = new Date(unix);
    var mes = (date.getMonth() + 1);
    if (mes.toString().length == 1)
        mes = "0" + mes;
    return [mes];

}

function dataAtualFormatadanomemes(resultmes) {
    var d = resultmes;
    var unix = +d.replace(/\D/g, '');
    var date = new Date(unix);
    var mes = (date.getMonth() + 1);

    var mesreferencia = new Array();
    mesreferencia[1] = "Janeiro";
    mesreferencia[2] = "Fevereiro";
    mesreferencia[3] = "Março";
    mesreferencia[4] = "Abril";
    mesreferencia[5] = "Maio";
    mesreferencia[6] = "Junho";
    mesreferencia[7] = "Julho";
    mesreferencia[8] = "Agosto";
    mesreferencia[9] = "Setembro";
    mesreferencia[10] = "Outubro";
    mesreferencia[11] = "Novembro";
    mesreferencia[12] = "Dezembro";

    var n = mesreferencia[mes];

    return [n];
}

function dataAtualFormatadaano(resultano) {
    var d = resultano;
    var unix = +d.replace(/\D/g, '');
    var date = new Date(unix);
    var ano = date.getFullYear();
    return [ano];

}

function gerarDemonstrativoReajuste(data) {
    
    var config = {
        pageOrientation: "landscape",
        pageSize: "A4",
        title: "Demonstrativo de Reajuste",
    };

    sedPdfExporter.normalizeConfig(config);
   
    config.nomeUnidadeGestora = data[0].unidadeGestora.nomeUnidadeGestora != undefined ? data[0].unidadeGestora.nomeUnidadeGestora : "";
    config.NmUnidGest = data[0].unidadeGestora.codigoUnidadeGestora != undefined ? data[0].unidadeGestora.codigoUnidadeGestora : "";
    config.descricaoMunicipio = data[0].unidadeGestora.descricaoMunicipio != undefined && data[0].unidadeGestora.descricaoMunicipio != "" ? titleize(data[0].unidadeGestora.descricaoMunicipio.toLowerCase()) : "São Paulo";
    config.numeroProcesso = data[0].processo.numeroProcesso != undefined ? data[0].processo.numeroProcesso : "";
    config.NrContrato = data[0].numeroContrato != undefined ? data[0].numeroContrato : "";
    config.NmCpfCgcCredor = data[0].nomeCredor != undefined ? data[0].nomeCredor : "";
    config.NrCpfCgcCredor = data[0].numeroCpfCnpj != undefined ? data[0].numeroCpfCnpj.replace(/^(\d{2})(\d{3})(\d{3})(\d{4})(\d{2})/, "$1.$2.$3/$4-$5") : "";
    config.dsAssunto = data[0].assuntoPlanejado.descricaoAssuntoPlanejado != undefined ? data[0].assuntoPlanejado.descricaoAssuntoPlanejado : "";
    config.DsClausulaContrato = data[0].clausulaContrato != undefined ? data[0].clausulaContrato : "";    
    var dataReferenciaReajuste = data[0].dataReferenciaReajuste != undefined ? dataAtualFormatada(data[0].dataReferenciaReajuste) : "";
    config.DtReajustemes = dataReferenciaReajuste != undefined ? dataAtualFormatadames(dataReferenciaReajuste) : "";
    config.DtReajusteano = data[0].dataBaseReajuste != undefined ? dataAtualFormatadaano(data[0].dataBaseReajuste) : "";
    config.DtReajuste = data[0].dataBaseReajuste != undefined ? dataAtualFormatada(data[0].dataBaseReajuste) : "";
    config.NrPercentual = data[0].percentualReajuste != undefined ? data[0].percentualReajuste + '%' : "";
    config.DsFolhaCadTerc = data[0].folhaCadTerc != undefined ? data[0].folhaCadTerc : "";
    config.IdPeriodoContrato = data[0].idPeriodoContrato != undefined ? data[0].idPeriodoContrato : "";
    config.DsUnidadeContratada = data[0].descricaoUnidadeContratada != undefined ? data[0].descricaoUnidadeContratada : "";
    config.VLReajuste = data[0].valorReajuste != undefined ? data[0].valorReajuste.toFixed(2).replace(".", ",").replace(/(\d)(?=(\d{3})+(?!\d))/g, "$1.") : "";
    config.VLReajustado = data[0].valorReajustado != undefined ? data[0].valorReajustado.toFixed(2).replace(".", ",").replace(/(\d)(?=(\d{3})+(?!\d))/g, "$1.") : "";
    config.descricaoObjeto = data[0].descricaoObjeto != undefined ? data[0].descricaoObjeto : "";
    config.dataVigenciaIni = data[0].dataVigenciaIni != undefined ? dataAtualFormatada(data[0].dataVigenciaIni) : "";
    config.dataVigenciaFim = data[0].dataVigenciaFim != undefined ? dataAtualFormatada(data[0].dataVigenciaFim) : "";
    config.nomeGestor = data[0].nomeGestor != undefined ? data[0].nomeGestor : "";
    config.existeValorReferencial = data[0].existeValorReferencial != undefined ? data[0].existeValorReferencial : "";
    config.VLTotalContrato = data[0].valorTotalContrato != undefined ? data[0].valorTotalContrato.toFixed(2).replace(".", ",").replace(/(\d)(?=(\d{3})+(?!\d))/g, "$1.") : "";
    config.VLTotalContratoReajustado = data[0].valorTotalContratoReajustado != undefined ? data[0].valorTotalContratoReajustado.toFixed(2).replace(".", ",").replace(/(\d)(?=(\d{3})+(?!\d))/g, "$1.") : "";
    config.dataReferenciaReajustemes = data[0].dataReferenciaReajuste != undefined ? dataAtualFormatadames(data[0].dataReferenciaReajuste) : "";
    config.dataReferenciaReajusteano = data[0].dataReferenciaReajuste != undefined ? dataAtualFormatadaano(data[0].dataReferenciaReajuste) : "";
    config.dataReferenciaReajuste = data[0].dataReferenciaReajuste != undefined ? dataAtualFormatada(data[0].dataReferenciaReajuste) : "";

    if (config.DsUnidadeContratada == "2") {
        config.DsUnidadeContratada = "Diário";
    }
    else if (config.DsUnidadeContratada == "1") {
        config.DsUnidadeContratada = "Mensal";
    }

    var body = [];

    //soh mostra o campo valor referencial se houver valor CadTerc
    if (config.existeValorReferencial == true) {
        body.push([
            { text: 'Item', fontsize: 9 },
            { text: 'Descrição', fontsize: 9 },
            { text: 'Quantidade', fontsize: 9 },
            { text: 'Valor Unitario', fontsize: 9 },
            { text: 'Valor Diário/Mensal', fontsize: 9 },
            { text: 'Valor Unitario reajustado', fontsize: 9 },
            { text: 'Valor mensal reajustado', fontsize: 9 },
            { text: 'Valor referencial', fontsize: 9 },
            { text: 'Validação do Valor', fontsize: 9 }
        ]);
    }else if (config.existeValorReferencial == false) {
        body.push([
            { text: 'Item', fontsize: 9 },
            { text: 'Descrição', fontsize: 9 },
            { text: 'Quantidade', fontsize: 9 },
            { text: 'Valor Unitario', fontsize: 9 },
            { text: 'Valor Diário/Mensal', fontsize: 9 },
            { text: 'Valor Unitario reajustado', fontsize: 9 },
            { text: 'Valor mensal reajustado', fontsize: 9 },
            { text: 'Validação do Valor', fontsize: 9 }
        ]);
    }

    var index = 0;
    var dados = data[0].listDemonstrativoReajuste;
    var total = 0;
    //var totalUnitarioReajustado = 0;
    var somaValorTotalMensal = 0;
    var somaValorUnitario = 0;
    var i = 0;
    var NrQuantidade = 0;
    var NrQuantidadePreReajuste = 0;
    var NrQuantidadeReajustada = 0;
    var NrValorUnitario = 0; //valor unitario reajustado
    var NrValorUnitarioPreReajuste = 0;
    var NrPercentual = data[0].percentualReajuste;
    var NrValorReferencial = 0;
    var NrValorTotalMensalPreReajuste = 0;
    var NrValorUnitarioReajustado = 0;
    var NrValorTotalMensalReajustado = 0;
    var NrPercentualReajustado = 0;
    var valorContrato = data[0].valorTotalContrato;
    var flg = "";
  
    if (dados.length > 0) {
        for (var key in dados)
        {
            //SE o Reajuste ainda não foi aplicado, os dados do Reajuste ainda não existem na tabela Historico. Para que os dados não fiquem vazios no documento, os valores são simulados 
            //Para exibição
            var _valorPreReajuste = dados[key].valorUnitarioPreReajuste;

            if (_valorPreReajuste == 0)
            {
                if (data[0].percentualDoContratoReajustado != "undefined")
                {
                    NrPercentualReajustado = data[0].percentualDoContratoReajustado;
                }
                NrPercentual = data[0].percentualReajuste;

                NrQuantidade = dados[key].valorQuantidade;
                NrQuantidadeReajustada = parseFloat(NrQuantidade) * parseFloat(NrPercentualReajustado / 100);//Simula a quantidade que sofrera reajuste
 
                NrValorUnitarioPreReajuste = dados[key].valorUnitario;
                NrValorUnitario = NrValorUnitarioPreReajuste + (NrValorUnitarioPreReajuste * NrPercentual / 100); //Simula p valor unitario reajustado

                NrValorTotalMensalPreReajuste = NrQuantidade * NrValorUnitarioPreReajuste;
                NrValorTotalMensalReajustado = NrQuantidade * NrValorUnitario;
               
                total += parseFloat(NrValorTotalMensalReajustado.toString().replace(",", "."));
                somaValorTotalMensal += parseFloat(NrValorTotalMensalPreReajuste.toString().replace(",", "."));
                somaValorUnitario += parseFloat(NrValorUnitario.toString().replace(",", "."));

            }
            else {                 
                if (data[0].percentualDoContratoReajustado != "undefined") {
                    NrPercentualReajustado = data[0].percentualDoContratoReajustado;
                }
 
                NrQuantidade = dados[key].valorQuantidade;
                NrQuantidadeReajustada = NrQuantidade * (NrPercentualReajustado/100);
                NrValorUnitarioPreReajuste = dados[key].valorUnitarioPreReajuste;
                NrValorUnitario = NrValorUnitarioPreReajuste + (NrValorUnitarioPreReajuste * NrPercentual / 100); //valor reajustado
                                
                NrValorReferencial = dados[key].valorReferencial;
                NrValorTotalMensalPreReajuste = 0;
                //NrValorUnitarioReajustado = 0;
                NrValorTotalMensalReajustado = 0;
                flg = "";
                 
                //valor mensal PREREAJUSTE
                NrValorTotalMensalPreReajuste = parseFloat(parseFloat(NrQuantidade) * parseFloat(NrValorUnitarioPreReajuste));
                NrValorTotalMensalReajustado = parseFloat(dados[key].VALOR_TOTAL_ITEM_REAJUSTADO);

                //NrValorUnitarioReajustado = parseFloat(NrValorUnitario) * parseFloat(NrQuantidade);
                //totalUnitarioReajustado += NrValorUnitarioReajustado;

                total += parseFloat(NrValorTotalMensalReajustado.toString().replace(",", "."));
                somaValorTotalMensal += parseFloat(NrValorTotalMensalPreReajuste.toString().replace(",", "."));
                somaValorUnitario += parseFloat(NrValorUnitario.toString().replace(",", "."));
            }
            
            if (config.existeValorReferencial == false)
            {
                flg = "Não Se Aplica";
            }
            else {
                debugger;
                if (parseFloat(NrValorUnitario) <= parseFloat(NrValorReferencial)) {

                    flg = "ACEITO";
                }
                else {
                    flg = "RENEGOCIAR";
                }
            }

            i++;                

            debugger;
            if (dados[key].idReajusteDemonstrativo > 0)
            {
                //soh mostra o campo valor referencial se houver valor CadTerc
                if (config.existeValorReferencial == true) {
                    body.push([
                       i.toString(),
                       dados[key].descricaoResumida.toString(),
                       NrQuantidade.toFixed(2).formatMoney("4", ",", ".").toString(),
                       NrValorUnitarioPreReajuste.toFixed(2).replace(".", ",").replace(/(\d)(?=(\d{3})+(?!\d))/g, "$1.").toString(),
                       NrValorTotalMensalPreReajuste.toFixed(2).replace(".", ",").replace(/(\d)(?=(\d{3})+(?!\d))/g, "$1.").toString(),
                       NrValorUnitario.toFixed(2).replace(".", ",").replace(/(\d)(?=(\d{3})+(?!\d))/g, "$1.").toString(),
                       NrValorTotalMensalReajustado.toFixed(2).replace(".", ",").replace(/(\d)(?=(\d{3})+(?!\d))/g, "$1.").toString(),
                       NrValorReferencial.toFixed(2).replace(".", ",").replace(/(\d)(?=(\d{3})+(?!\d))/g, "$1.").toString(),                       
                       flg.toString()
                    ]);
                } else if (config.existeValorReferencial == false)
                {
                    body.push([
                       i.toString(),
                       dados[key].descricaoResumida.toString(),
                       NrQuantidade.toFixed(2).formatMoney("4", ",", ".").toString(),
                       NrValorUnitarioPreReajuste.toFixed(2).replace(".", ",").replace(/(\d)(?=(\d{3})+(?!\d))/g, "$1.").toString(),
                       NrValorTotalMensalPreReajuste.toFixed(2).replace(".", ",").replace(/(\d)(?=(\d{3})+(?!\d))/g, "$1.").toString(),
                       NrValorUnitario.toFixed(2).replace(".", ",").replace(/(\d)(?=(\d{3})+(?!\d))/g, "$1.").toString(),                       
                       NrValorTotalMensalReajustado.toFixed(2).replace(".", ",").replace(/(\d)(?=(\d{3})+(?!\d))/g, "$1.").toString(),                      
                       flg.toString()
                    ]);
                 }
            }
        }
    }

    if (config.existeValorReferencial == true) {
        body.push([
                { text: 'TOTAL\n\n', colSpan: 4 },
                { text: "" },
                { text: data[0].valorTotalItens.toFixed(2).replace(".", ",").replace(/(\d)(?=(\d{3})+(?!\d))/g, "$1.").toString(), style: 'tableHeader' },
                { text: "" },
                { text: somaValorTotalMensal.toFixed(2).replace(".", ",").replace(/(\d)(?=(\d{3})+(?!\d))/g, "$1.").toString() },
                { text: "" },
                { text: total.toFixed(2).replace(".", ",").replace(/(\d)(?=(\d{3})+(?!\d))/g, "$1.").toString() },
                { text: "", colSpan: 2 },
                { text: "" },
              
        ]);
    } else if (config.existeValorReferencial == false) {
        body.push([
                { text: 'TOTAL\n\n', colSpan: 4,  },
                { text: "" },
                { text: data[0].valorTotalItens.toFixed(2).replace(".", ",").replace(/(\d)(?=(\d{3})+(?!\d))/g, "$1.").toString(), style: 'tableHeader' },
                { text: "" },
                { text: somaValorTotalMensal.toFixed(2).replace(".", ",").replace(/(\d)(?=(\d{3})+(?!\d))/g, "$1.").toString() },
                { text: "" },
                { text: total.toFixed(2).replace(".", ",").replace(/(\d)(?=(\d{3})+(?!\d))/g, "$1.").toString() },
                { text: "", colSpan: 1 },
              
        ]);
    }

    config.body = body;
    config.docGenerator = function (config) {
        var doc = {

            pageMargins: [30, 170, 50, 30],
            header: [
           {
               margin: [50, 20, 50, 0],
               table: {
                   widths: ["5%", "95%"],
                   body: [
                       [
                           {
                               stack: [
                                   {
                                       image: 'data:image/png;base64,',
                                       width: 65,
                                       height: 75
                                   }
                               ]
                           },
                           {
                               stack: [
                                   {
                                       text: 'GOVERNO DO ESTADO DE SÃO PAULO',
                                       fontSize: 15,
                                       bold: true
                                   },
                                   {
                                       text: 'SECRETARIA DE ESTADO DA EDUCAÇÃO',
                                       fontSize: 14,
                                       bold: true
                                   },
                                   {
                                       text: config.nomeUnidadeGestora,
                                       fontSize: 11,
                                       bold: false
                                   },

                               ],
                               alignment: "center",
                           }
                       ]
                   ]
               },
               layout: "noBorders"
           },
           {
               alignment: "center",
               stack: [

                   {
                       text: "PLANILHA DO DEMONSTRATIVO DE REAJUSTE"
                   }
               ],
           },
            {
                margin: [50, 0, 50, 0],
                stack: [
                   
                            {
                               
                                fontSize: 10,
                                text:
                                 [
                                    { text: "PROCESSO Nº : ", bold: true },
                                    { text: config.numeroProcesso, bold: false },
                                 ],
                            },
                            {
                                fontSize: 10,
                                text:
                                 [
                                    { text: "CONTRATO Nº : ", bold: true },
                                    { text: config.NrContrato, bold: false },
                                 ],
                            },
                             {
                                 fontSize: 10,
                                 text: [
                                          { text: "Contratada : ", bold: true },
                                          { text: config.NmCpfCgcCredor, bold: false },
                                 ],
                             },

                             {
                                 fontSize: 10,
                                 text: [
                                          { text: "Cnpj/mf : ", bold: true },
                                          { text: config.NrCpfCgcCredor, bold: false },
                                 ],
                             },
                            {
                                fontSize: 10,
                                text:
                                    [
                                { text: "ASSUNTO PLANEJADO : ", bold: true },
                                { text: config.descricaoObjeto, bold: false },
                                    ],
                            }
                ]

            },
            ],

            content: [
                  {
                      margin: [40, 15, 50, 0],
                      fontSize: 10,
                      alignment: 'justify',
                      stack: [
                                {
                                    text: [
                                    { text: "Nos termos de instrução dos autos e considerando o disposto na cláusula " },
                                    { text: config.DsClausulaContrato },
                                    { text: " observando-se a legislação vigente, em especial o Decreto Estadual" },
                                    ],                             
                                }
                      ]
                  },
                   {
                       margin: [20, 0, 50, 0],
                       fontSize: 10,
                       alignment: 'justify',
                       stack: [
                                 {
                                     text: [
                                     { text: " nº 48.326/2003 e das disposições da " },
                                     { text: "Resolução CC 79/2003 , providenciamos os cálculos referentes ao reajuste previsto , adotando como indexador o índice oficial" }, 
                                     { text: " ( IPC-FIPE – mês de referência de preços : " }, 
                                     { text: config.dataReferenciaReajustemes },
                                     { text: "/" }, 
                                     { text: config.dataReferenciaReajusteano },
                                     { text: " ) ,cuja variação no período foi de " }, 
                                     { text: config.NrPercentual }, 
                                     { text: " conforme simulador no CADTERC documentos de fls " }, 
                                     { text: config.DsFolhaCadTerc }, 
                                     { text: ", sendo que o valor passará de R$ " }, 
                                     { text: config.VLTotalContrato },
                                     { text: " para R$ " }, 
                                     { text: config.VLTotalContratoReajustado },
                                     { text: " a partir de " }, 
                                     { text: config.DtReajuste },
                                     { text: ", conforme demonstrativo abaixo :" }, 
                                     ],
                                 }
                       ]
                   },
                   {
                       margin: [20, 10, 20, 0],
                       fontSize: 10,
                       table: {
                          
                           body: config.body
                       },
                   },
                    
                     {
                         margin: [20, 50, 30, 0],
                         fontSize: 10,
                         alignment: "right",
                         stack: [
                             {
                                 text:
                                     [
                                           { text: config.descricaoMunicipio + ", " },
                                           { text: "________________________" },
                                           { text: " de " },
                                           { text: "________________________" },
                                           { text: " de" },
                                     ],
                             }

                         ],
                     },
                     {
                         margin: [20, 60, 50, 0],
                         fontSize: 10,
                         alignment: "center",
                         stack: [
                             { text: "_______________________________________________" },
                             { text: "Gestor do Contrato" },
                         ],
            
                     }
                
            ],
            footer: function (currentPage, pageCount) {
                return {
                    margin: [50, 0, 40, 0],
                    columns: [
                        {
                            table: {
                                widths: [200, 200, 300],
                                headerRows: 1,
                                fontSize: 9,
                                body: [
                                         [{ text: "pagina(s) " + currentPage.toString() + " de " + pageCount, colSpan: 3, fontSize: 9, border: [false, true, false, false], bold: false, color: 'gray', alignment: "right" }],

                                ]
                            },
                        },
                    ]
                };

            },
            styles: {

                tabela: {
                    margin: [0, 5, 0, 1]
                },
                tableHeader: {
                    bold: true,
                    fontSize: 11,
                    color: 'black'
                },
                header: {
                    fontSize: 12,
                    bold: false,
                    margin: [10, 0, 30, 0]
                }
            },
        }
        return doc;
    };
     
    sedPdfExporter.exportPdf(config);
}