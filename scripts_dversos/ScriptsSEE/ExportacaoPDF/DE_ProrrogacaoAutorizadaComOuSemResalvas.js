function DocProrrogacaoAutorizadaComOuSemResalvas(idContrato, nrDocumento, anoProrrogacao, idProrrogacao, action) {
    $.ajax({
        type: "POST",
        url: action,
        data: { "idContrato": idContrato, "nrDocumento": nrDocumento, "anoProrrogacao": anoProrrogacao, "idProrrogacao": idProrrogacao }
    }).done(function (data) {
        DocumentoProrrogacaoAutorizadaComOuSemResalvas(data);
        //gerarReferencialCADTERC(data);
    }).fail(function (e) {
        mensagemAlerta.ErroAjax('Ocorreu um erro durante o processo');
    });
}
function DocumentoProrrogacaoAutorizadaComOuSemResalvas(data) {

    debugger;
    var config = {
        pageSize: "A4",
        pageOrientation: "portrait",
        // pageOrientation: 'landscape',
        title: "ProrrogacaoAutorizadaComOuSemResalvas",
    };

    sedPdfExporter.normalizeConfig(config);

    config.nomeUnidadeGestora = data[0].unidadeGestora.nomeUnidadeGestora != undefined ? data[0].unidadeGestora.nomeUnidadeGestora : "";
    config.NmUnidGest = data[0].unidadeGestora.codigoUnidadeGestora != undefined ? data[0].unidadeGestora.codigoUnidadeGestora : "";
    config.descricaoMunicipio = data[0].unidadeGestora.descricaoMunicipio != undefined && data[0].unidadeGestora.descricaoMunicipio != "" ? titleize(data[0].unidadeGestora.descricaoMunicipio.toLowerCase()) : "São Paulo";
    config.descricaoMunicipio = "São Paulo";
    config.numeroProcesso = data[0].processo.numeroProcesso != undefined ? data[0].processo.numeroProcesso : "";
    config.NrContrato = data[0].numeroContrato != undefined ? data[0].numeroContrato : "";
    config.NmCpfCgcCredor = data[0].nomeCredor != undefined ? data[0].nomeCredor : "";
    config.NrCpfCgcCredor = data[0].numeroCpfCnpj != undefined ? data[0].numeroCpfCnpj.replace(/^(\d{2})(\d{3})(\d{3})(\d{4})(\d{2})/, "$1.$2.$3/$4-$5") : "";
    config.dsAssunto = data[0].assuntoPlanejado.descricaoAssuntoPlanejado != undefined ? data[0].assuntoPlanejado.descricaoAssuntoPlanejado : "";
    config.DsClausulaContrato = data[0].clausulaContrato != undefined ? data[0].clausulaContrato : "";
    config.DtReajustemes = data[0].dataBaseReajuste != undefined ? dataAtualFormatadames(data[0].dataBaseReajuste) : "";
    config.DtReajusteano = data[0].dataBaseReajuste != undefined ? dataAtualFormatadaano(data[0].dataBaseReajuste) : "";
    config.DtReajuste = data[0].dataBaseReajuste != undefined ? dataAtualFormatada(data[0].dataBaseReajuste) : "";
    config.NrPercentual = data[0].percentualReajuste != undefined ? data[0].percentualReajuste + '%' : "";
    config.DsFolhaCadTerc = data[0].folhaCadTerc != undefined ? data[0].folhaCadTerc : "";
    config.IdPeriodoContrato = data[0].idPeriodoContrato != undefined ? data[0].idPeriodoContrato : "";
    config.DsUnidadeContratada = data[0].descricaoUnidadeContratada != undefined ? data[0].descricaoUnidadeContratada : "";
    config.VLReajuste = data[0].valorReajuste != undefined ? data[0].valorReajuste.toFixed(2).replace(".", ",").replace(/(\d)(?=(\d{3})+(?!\d))/g, "$1.") : "";
    config.VLReajustado = data[0].valorReajustado != undefined ? data[0].valorReajustado.toFixed(2).replace(".", ",").replace(/(\d)(?=(\d{3})+(?!\d))/g, "$1.") : "";
    config.descricaoObjeto = data[0].descricaoObjeto != undefined ? data[0].descricaoObjeto : "";
    config.dataVigenciaIni = data[0].dataVigenciaIni != undefined ? dataAtualFormatada(data[0].dataVigenciaIni) : "";
    config.dataVigenciaFim = data[0].dataVigenciaFim != undefined ? dataAtualFormatada(data[0].dataVigenciaFim) : "";
    config.nomeGestor = data[0].nomeGestor != undefined ? titleize(data[0].nomeGestor) : "";
    config.dataImpressao = data[0].dataImpressao != undefined ? dataAtualFormatada(data[0].dataImpressao) : "";
    config.DtImpressaodia = data[0].dataImpressao != undefined ? dataAtualFormatadadia(data[0].dataImpressao) : "";
    config.DtImpressaomes = data[0].dataImpressao != undefined ? dataAtualFormatadames(data[0].dataImpressao) : "";
    config.DtImpressaonomemes = data[0].dataImpressao != undefined ? dataAtualFormatadanomemes(data[0].dataImpressao) : "";
    config.DtImpressaoano = data[0].dataImpressao != undefined ? dataAtualFormatadaano(data[0].dataImpressao) : "";
    config.nrFolha = data[0].nrFolha != undefined ? data[0].nrFolha : "";
    config.nomeResponsavel = data[0].usuarioResponsavel.nmResponsavel != undefined ? titleize(data[0].usuarioResponsavel.nmResponsavel) : "";
    config.UsuarioLogadoNome = data[0].usuarioLogadoNome != undefined ? titleize(data[0].usuarioLogadoNome) : "";
    config.UsuarioLogadoCargo = data[0].usuarioLogadoCargo != undefined ? data[0].usuarioLogadoCargo : " ";
    config.descricaoEnderecoNumero = data[0].descricaoEnderecoNumero != undefined ? data[0].descricaoEnderecoNumero : "";
    config.codigoUnidadeGestora = data[0].unidadeGestora.codigoUnidadeGestora != undefined ? data[0].unidadeGestora.codigoUnidadeGestora : "";
    config.descricaoTermoAditamento = data[0].credor.representanteEmpresa.descricaoTermoAditamento != undefined ? titleize(data[0].credor.representanteEmpresa.descricaoTermoAditamento) : "";
    
    if (config.descricaoTermoAditamento == "") {
        config.Termo = "Termo";
    }
    else {
        config.Termo = config.descricaoTermoAditamento + " termo";
    }


    //alteração feita para atender Fluxo CJ
    config.nomeResponsavel = "FLAVIA CRISTINA DA SILVA ROMANO";
    config.nomeCargo = "DIRETOR TECNICO III";


    var idTipoContratada = 0;
    for (var i = 0; i <= data[0].listProrrogacao.length - 1; i++) {
        if (data[0].listProrrogacao[i].prorrogacaoDocumento.IdDocumento == DOCS.Quadro_Demonstrativo_Valores_Prorrogacao_Cronograma_Desembolso.value) {
            config.NovaDataVigenciaIni = data[0].listProrrogacao[i].dataVigenciaIni != undefined ? dataAtualFormatada(data[0].listProrrogacao[i].dataVigenciaIni) : "";
            config.NovaDataVigenciaFim = data[0].listProrrogacao[i].dataVigenciaFim != undefined ? dataAtualFormatada(data[0].listProrrogacao[i].dataVigenciaFim) : "";
            config.NovaIdPeriodoContrato = data[0].listProrrogacao[i].contrato.idPeriodoContrato != undefined ? data[0].listProrrogacao[i].contrato.idPeriodoContrato : "";
            idTipoContratada = data[0].listProrrogacao[i].contrato.unidadeContratada;
        }
    }


    config.periodoPorExtenso = data[0].periodoPorExtenso != undefined ? data[0].periodoPorExtenso : "";
    config.NomeContradaSiafemSiafisico = data[0].NomeContradaSiafemSiafisico != undefined ? data[0].NomeContradaSiafemSiafisico : "";
    //config.nrFolhasMinuta = data[0].nrFolhasMinuta != undefined ? data[0].nrFolhasMinuta : "";
    config.MesesVigenciaContrato = data[0].MesesVigenciaContrato != undefined ? data[0].MesesVigenciaContrato : "";

    //informação do meio do corpo do documento - padrão
    config.nrFolhasMinuta = GetDocumentoNrFolha(data[0].listProrrogacao, DOCS.Minuta_Termo_Prorrogacao_Contratos_Servicos_Continuos.value);

    var motivo1 = GetDocumentoMotivoResalva(data[0].listProrrogacao, DOCS.Quadro_Comparativo_Precos.value);
    var motivo2 = GetDocumentoMotivoResalva(data[0].listProrrogacao, DOCS.Quadro_Comparativo_de_Precos_Referencial_CADTERC.value);

    config.motivoResalvaQuadroComparativoCadtercOuPesquisaDepreco = motivo1 != "" && motivo1 != undefined ? motivo1 : motivo2;

    if (config.motivoResalvaQuadroComparativoCadtercOuPesquisaDepreco == "" || config.motivoResalvaQuadroComparativoCadtercOuPesquisaDepreco == undefined) {
        config.motivoResalvaQuadroComparativoCadtercOuPesquisaDepreco = "Não houve ressalvas para o documento ( Pesquisa de preço ou quadro comparativo de preço) pelo CCONT.";
    }

    config.motivoResalvaContratada_Documento_Habilitacao = GetDocumentoMotivoResalva(data[0].listProrrogacao, DOCS.Solicita_Manifestacao_Contratada_Documento_Habilitacao.value);

    config.nrFolhaAutoridadeCompetente = GetDocumentoNrFolha(data[0].listProrrogacao, DOCS.Declaracao_Autoridade_Competente.value);
    config.motivoResalvaDeclaracao_Autoridade_Competente = GetDocumentoMotivoResalva(data[0].listProrrogacao, DOCS.Declaracao_Autoridade_Competente.value);

    config.nrFolhaMinuta = GetDocumentoNrFolha(data[0].listProrrogacao, DOCS.Minuta_Termo_Prorrogacao_Contratos_Servicos_Continuos.value);
    config.motivoResalvaMinuta_Termo_Prorrogacao_Contratos_Servicos_Continuos = GetDocumentoMotivoResalva(data[0].listProrrogacao, DOCS.Minuta_Termo_Prorrogacao_Contratos_Servicos_Continuos.value);

    config.numeroDocumentoAno = data[0].fluxoDocumento.ProrrogacaoFluxo.numeroDocumento + "/" + data[0].fluxoDocumento.ProrrogacaoFluxo.anoVigenteProrrogacao;
    



    function GetDocumentoMotivoResalva(listProrrogacao, documento) {
        for (var i = 0; i <= listProrrogacao.length - 1; i++) {
            if (data[0].listProrrogacao[i].prorrogacaoDocumento.IdDocumento == documento) {
                //Gravar motivo de resalva
                var motivo = "";
                motivo = data[0].listProrrogacao[i].motivoReprovacao != undefined ? data[0].listProrrogacao[i].motivoReprovacao : "";
                return motivo;
            }
        }
    }

    function GetDocumentoNrFolha(listProrrogacao, documento) {
        for (var i = 0; i <= listProrrogacao.length - 1; i++) {
            if (data[0].listProrrogacao[i].prorrogacaoDocumento.IdDocumento == documento) {
                var nrFolha = "";
                nrFolha = data[0].listProrrogacao[i].nrFolha != undefined ? data[0].listProrrogacao[i].nrFolha : "";
                return data[0].listProrrogacao[i].nrFolha;
            }
        }
    }




    if (idTipoContratada == 1) {
        config.TipoContrada = "meses";
    }
    else if (idTipoContratada == 2) {
        config.TipoContrada = "diária";
    }

    var body = [];
    body.push([
                { text: 'DOCUMENTO', alignment: "center", bold: true, fontsize: 11, color: "black" },
                { text: 'FOLHAS', alignment: "center", bold: true, fontsize: 11, color: "black" }
    ]);


    var index = 0;
    var dados = data;

    var nomeDocumentoAnt = "";

    var nrFolhaAnt = "";

    if (dados[0].listProrrogacao.length > 0) {
        for (var key in dados[0].listProrrogacao) {
            var nomeDocumento = dados[0].listProrrogacao[key].prorrogacaoDocumento.descricaoDocumento != undefined ? dados[0].listProrrogacao[key].prorrogacaoDocumento.descricaoDocumento.toString() : "";
            var nrFolha = dados[0].listProrrogacao[key].nrFolha != undefined ? dados[0].listProrrogacao[key].nrFolha.toString() : "";

            if (nomeDocumento != nomeDocumentoAnt) {
                if (nomeDocumento != "") {

                    if (nomeDocumento != "Gerar Check List") {

                        body.push([

                            { text: nomeDocumento.toString(), alignment: "left", bold: true, fontsize: 13, color: "black" },
                            { text: nrFolha.toString(), alignment: "left", bold: true, fontsize: 13, color: "black" },

                        ]);

                    }

                }

            }

            var nomeDocumentoAnt = dados[0].listProrrogacao[key].prorrogacaoDocumento.nomeDocumento != undefined ? dados[0].listProrrogacao[key].prorrogacaoDocumento.nomeDocumento.toString() : "";

            var nrFolhaAnt = dados[0].listProrrogacao[key].nrFolha != undefined ? dados[0].listProrrogacao[key].nrFolha.toString() : "";

        }
    }

    if (config.codigoUnidadeGestora == "080101" || config.codigoUnidadeGestora == "080102" ||
   config.codigoUnidadeGestora == "080011" || config.codigoUnidadeGestora == "080104" ||
   config.codigoUnidadeGestora == "080259" || config.codigoUnidadeGestora == "080353" ||
   config.codigoUnidadeGestora == "080356" || config.codigoUnidadeGestora == "080357" ||
   config.codigoUnidadeGestora == "080358" || config.codigoUnidadeGestora == "080359" ||
   config.codigoUnidadeGestora == "080040" || config.codigoUnidadeGestora == "080360"
   ) {

        config.descricaoEnderecoNumero = "Endereço SEE : Praça da República, 53 - República, São Paulo/SP";
    }
    
    config.body = body;
    config.docGenerator = function (config) {
        var doc = {
            pageSize: "A4",
            title: "ProrrogacaoAutorizadaComOuSemResalvas",
            pageMargins: [30, 150, 50, 45],

            header: {
                fontSize: 10,
                margin: [0, 20, 0, 0],
                columns: [
                    {
                        image: "data:image/png;base64,",
                        width: 65,
                        height: 75,
                        margin: [40, 10, 20, 0],
                    },
                    [
                        { text: "GOVERNO DO ESTADO DE SÃO PAULO", alignment: "center", fontSize: 15, bold: true, margin: [0, 10, 40, 0], },
                        { text: "SECRETARIA DA EDUCAÇÃO", alignment: "center", fontSize: 14, bold: true, margin: [0, 0, 40, 0], },
                        { text: "COORDENADORIA DE ORÇAMENTO E FINANÇAS", alignment: "center", fontSize: 11, bold: false, margin: [0, 0, 40, 0], },
                        { text: "DEPARTAMENTO DE CONTROLE DE CONTRATOS E CONVÊNIOS", alignment: "center", fontSize: 11, bold: false, margin: [0, 0, 40, 0], },
                        { text: "CENTRO DE ACOMPANHAMENTO E CONTROLE DE CONTRATOS", alignment: "center", fontSize: 11, bold: false, margin: [0, 0, 40, 0], },
                    ],
                ],
            },

            content: {
                margin: [0, 0, 0, 0],
                columns: [
                    [
                        {
                            bold: true, fontSize: 10, margin: [20, 10, 40, 0], text: ["PROCESSO Nº : ",
                            { text: config.numeroProcesso, bold: false }, ]
                        },
                        {
                            bold: true, fontSize: 10, margin: [20, 0, 40, 0], text: ["INTERESSADO : ",
                            { text: config.nomeUnidadeGestora, bold: false }, ]
                        },
                        {
                            bold: true, fontSize: 10, margin: [20, 0, 40, 0], text: ["ASSUNTO PLANEJADO : ",
                            { text: config.descricaoObjeto, bold: false }, ]
                        },
                        {
                            bold: true, fontSize: 10, margin: [20, 0, 40, 0], text: ["INFORMAÇÃO Nº : ",
                            { text: "" + config.numeroDocumentoAno + " \n\n", bold: false }, ]
                        },


                        { text: "Senhor (a) Diretor (a) do CCONT, \n\n\n", alignment: "justify", fontSize: 10, bold: false, margin: [20, 0, 20, 0], },


                        
                        {
                            alignment: "justify",
                            color: "black",
                            style: "invisivel",
                            fontSize: 10,
                            text:
                                [
                                    { text: "\n.................", style: "invisivel" },
                                    { text: "Trata o presente de prestação de serviços de " + config.descricaoObjeto, alignment: "justify"},
                                    { text: ", " + config.nomeUnidadeGestora + " - Contrato nº " + config.NrContrato + ", firmado com a empresa " + config.NomeContradaSiafemSiafisico + ", com proposta advinda da citada Diretoria de Ensino quanto à análise da minuta do " },
                                    { text: config.Termo + " de Aditamento Contratual ", bold: true },
                                    { text: "(minuta às folhas: (" + config.nrFolhasMinuta + "), no qual se objetiva a " },
                                    { text: "PRORROGAÇÃO do prazo de vigência do Contrato, ", bold: true },
                                    { text: "por " + config.NovaIdPeriodoContrato + " " + config.TipoContrada + ", contados a partir de " + config.NovaDataVigenciaIni + " e término em " + config.NovaDataVigenciaFim + ". \n\n" },

                            ]
                        },


                        { text: "À vista da análise dos documentos constantes dos autos, em especial ", alignment: "justify", fontSize: 10, bold: false, margin: [70, 0, 20, 0], }, 
                        { text: "daqueles apresentados para a pretendida Prorrogação da vigência contratual, listados abaixo: \n\n", alignment: "justify", fontSize: 10, bold: false, margin: [20, 0, 20, 0], },

                                            {
                                                margin: [20, 0, 0, 0],
                                                table: {
                                                    body: config.body
                                                },
                                                fontSize: 10
                                            },

                        
                        {
                            alignment: "justify",
                            color: "black",
                            style: "invisivel",
                            fontSize: 10,
                            text: [
                                    { text: "\n.................", style: "invisivel" },
                                    { text: "E considerando-se a edição da " },
                                    { text: "Resolução PGE nº 23/2015 \n", bold: true },
                                    { text: "(D.O.E. de 13/11/2015, republicada no D.O.E. de 14/11/2015), bem como da " },
                                    { text: "Resolução SE-55/2015 ", bold: true }, {
                                    text: "(D.O.E. de 08/12/2015), que dispõe sobre a tramitação de contratos de prestação de serviços contínuos com termo de aditamento para" +
                                    "prorrogação de prazo de vigência, e em complementação às informações prestadas acima, opinamos: "
                                    },

                            ]
                        },

                        {
                            alignment: "justify",
                            color: "black",
                            fontSize: 10,
                            style: "invisivel",
                            text: [
                                    { text: "\n.................", style: "invisivel" },
                                    { text: "1) O " },
                                    { text: "art. 57, inciso II, da Lei Federal nº 8.666/93, ", bold: true },
                                    { text: "autoriza a prorrogação de " },
                                    {
                                        text: "contratos de prestação de serviços executados de forma contínua até o limite de 60 meses, sendo que o contrato em questão está vigente e a prorrogação proposta obedece "
                                                          + "aos limites estabelecidos na citada legislação; "
                                    },

                            ]
                        },
                        
                        {
                            alignment: "justify",
                            color: "black",
                            style: "invisivel",
                            fontSize: 10,
                            text: [
                                    { text: "\n.................", style: "invisivel" },
                                    { text: "2)	" + config.motivoResalvaQuadroComparativoCadtercOuPesquisaDepreco },
                            ]
                        },

                        {
                            alignment: "justify",
                            color: "black",
                            style: "invisivel",
                            fontSize: 10,
                            text: [
                                    { text: "\n.................", style: "invisivel" },
                                    { text: "3) Os " }, { text: "reajustes de preços ", bold: true }, { text: "devem observados e aplicados conforme consignado " }

                            ]
                        },
                                    {
                                        margin: [20, 0, 20, 0],
                                        alignment: "justify",
                                        color: "black",
                                        fontSize: 10,
                                        text: [
                                                { text: "na cláusula própria do contrato, a qual deverá estar em consonância com o instrumento convocatório da licitação que originou o contrato" +
                                                         "(edital do pregão eletrônico), bem como atentando-se às orientações da legislação aplicável ao caso específico (Decreto n.º 48.326/2003 e" +
                                                         "Resolução CC-79/03 e alterações posteriores);" }
                                        ]
                                    },
                        
                        {
                            alignment: "justify",
                            color: "black",
                            style: "invisivel",
                            fontSize: 10,
                            text: [
                                    { text: "\n.................", style: "invisivel" },
                                    { text: "4)	Consignamos que a contratada deverá manter todas as condições de" },
                                    { text: " habilitação e qualificação exigidas na licitação e/ou previamente à celebração do ajuste e a cada pagamento." }

                            ]
                        },

                        {
                            alignment: "justify",
                            color: "black",
                            style: "invisivel",
                            fontSize: 10,
                            text: [
                                    { text: "\n.................", style: "invisivel" },
                                    { text: "" + config.motivoResalvaContratada_Documento_Habilitacao },
                            ]
                        },

                        //Neste ponto, deverão ser inseridas as informações digitadas pelo CCONT durante a análise do processo, se houver, referente ao item “Documentos de habilitação da contratada” no check list;

                        {
                            alignment: "justify",
                            color: "black",
                            style: "invisivel",
                            fontSize: 10,
                            text: [
                                    { text: "\n.................", style: "invisivel" },
                                    { text: "5)	Deverá o Gestor do Contrato atentar às certidões apresentadas, quando da " },
                                    { text: "assinatura do aditamento em prorrogação, " },
                                    { text: "uma vez que devem estar todas atualizadas;", bold: true }

                            ]
                        },
                        
                        {
                            alignment: "justify",
                            color: "black",
                            style: "invisivel",
                            fontSize: 10,
                            text: [
                                    { text: "\n.................", style: "invisivel" },
                                    { text: "6) Ressaltamos que nos termos da " },
                                    { text: "Portaria Subg-Cons 02,", bold: true },
                                    { text: "de 21-9-2016, e " },
                                    { text: "previamente à assinatura do termo de prorrogação, além da consulta ao “Cadastro Informativo dos Créditos não Quitados de Órgãos e Entidades Estaduais – " },
                                    { text: "CADIN ESTADUAL", bold: true }, { text: "” (art. 2º, §3º da Resolução PGE-23, de 12-11-2015), " },
                                    { text: "a autoridade competente deverá verificar a regularidade da contratada perante ", decoration: 'underline' },
                                    {
                                        text: "o “Sistema Eletrônico de Aplicação e Registro de Sanções Administrativas – e-Sanções”, no endereço www.esancoes.sp.gov.br, e no “Cadastro Nacional de Empresas Inidôneas" +
                                                              "e Suspensas – CEIS”, no endereço http://www.portaltransparencia.gov.br;"
                                    }

                            ]
                        },                         
                         {
                             margin: [70, 20, 20, 0],
                             alignment: "justify",
                             color: "black",
                             fontSize: 10,
                             text: [
                                     { text: "7) As reservas e empenhos deverão " },
                                     { text: "SEMPRE ", bold: true },
                                     { text: "ser feitos no " },
                                     { text: "PROCESSO ", bold: true },

                             ]
                         },

                                            {
                                                margin: [20, 0, 20, 0],
                                                alignment: "justify",
                                                color: "black",
                                                fontSize: 10,
                                                text: [
                                                        { text: "LICITATÓRIO, nunca ", bold: true },
                                                        { text: "no processo de pagamento, sendo este último destinado apenas ao arquivamento dos documentos relativos aos pagamentos mensais; " }
                                                ]
                                            },
                           
                           {
                               margin: [70, 20, 20, 0],
                               alignment: "justify",
                               color: "black",
                               fontSize: 10,
                               text: [
                                       { text: "8) A " },
                                       { text: "reserva orçamentária ", bold: true },
                                       { text: "deverá ser providenciada oportunamente e a " },

                               ]
                           },

                                            {
                                                margin: [20, 0, 20, 0],
                                                alignment: "justify",
                                                color: "black",
                                                fontSize: 10,
                                                text: [
                                                        { text: "inobservância deste procedimento acarreta nulidade do ajuste e responsabilidade do servidor que lhe tenha dado causa, conforme o artigo 7º, §2º, Inciso III e §6º, da Lei Federal nº 8.666/93; "}
                                                ]
                                            },

                            {
                                margin: [70, 20, 20, 0],
                                alignment: "justify",
                                color: "black",
                                fontSize: 10,
                                text: [
                                        { text: "9)	Destaca-se que a viabilidade da prorrogação foi " },
                                        { text: "justificada e autorizada por ", bold: true }
                                ]
                            },

                                            {
                                                margin: [20, 0, 20, 0],
                                                alignment: "justify",
                                                color: "black",
                                                fontSize: 10,
                                                text: [
                                                        { text: "escrito ", bold: true },
                                                        { text: "pela autoridade competente (Dirigente Regional de Ensino), às folhas : (" + config.nrFolhaAutoridadeCompetente + "), utilizando-se corretamente o " },
                                                        { text: "MODELO DE DECLARAÇÃO DA AUTORIDADE COMPETENTE", bold: true },
                                                        { text: ", em consonância com o estabelecido no artigo 2º, § 1º, da Resolução PGE-23/2015." }
                                                ]
                                            },


                                            {
                                                alignment: "justify",
                                                color: "black",
                                                style: "invisivel",
                                                fontSize: 10,
                                                text: [
                                                        { text: "\n.................", style: "invisivel" },
                                                        { text: "" + config.motivoResalvaDeclaracao_Autoridade_Competente + "\n\n" },
                                                ]
                                            },


                            {
                                margin: [70, 20, 20, 0],
                                alignment: "justify",
                                color: "black",
                                fontSize: 10,
                                text: [
                                        { text: "10) No que toca à minuta apresentada às folhas: (" + config.nrFolhaMinuta + ") foi utilizada a " },
                                        
                                ]
                            },

                                            {
                                                margin: [20, 0, 20, 0],
                                                alignment: "justify",
                                                color: "black",
                                                fontSize: 10,
                                                text: [
                                                        { text: "MINUTA PADRÃO ", bold: true },
                                                        { text: ", nos termos do artigo 2º, inciso IV, da Resolução PGE-23/2015, e encontra-se adequada para atingir os fins propostos " },
                                                        { text: "(prorrogação da vigência). ", bold: true }
                                                ]
                                            },


                            {
                                alignment: "justify",
                                color: "black",
                                style: "invisivel",
                                fontSize: 10,
                                text: [
                                        { text: "\n.................", style: "invisivel" },
                                        { text: "" + config.motivoResalvaMinuta_Termo_Prorrogacao_Contratos_Servicos_Continuos },
                                ]
                            },
                            //Neste ponto, deverão ser inseridas as informações digitadas pelo CCONT durante a análise do processo, se houver, referente ao item “Minuta do Termo de Aditamento” no check list;
                        


                            {
                                margin: [70, 20, 20, 0],
                                alignment: "justify",
                                color: "black",
                                fontSize: 10,
                                text: [
                                        { text: "11) A contratada deverá renovar ou complementar a " },
                                        { text: "garantia ", bold: true },
                                        { text: "ofertada " }
                                ]
                            },

                                          {
                                              margin: [20, 0, 20, 0],
                                              alignment: "justify",
                                              color: "black",
                                              fontSize: 10,
                                              text: [
                                                      { text: "originalmente, se prevista no contrato, adequando-se ao novo prazo de vigência;" }
                                              ]
                                          },


                            {
                                margin: [70, 20, 20, 0],
                                alignment: "justify",
                                color: "black",
                                fontSize: 10,
                                text: [
                                        { text: "12) Após assinatura do aditamento, deverá ser providenciada a " },
                                        { text: "publicação do ", bold: true }
                                ]
                            },

                                            {
                                                margin: [20, 0, 20, 0],
                                                alignment: "justify",
                                                color: "black",
                                                fontSize: 10,
                                                text: [
                                                        { text: "respectivo extrato, ", bold: true },
                                                        { text: "nos termos do Decreto nº 61.476, de 03/09/2015;" }
                                                ]
                                            },
                            
                            {
                                margin: [70, 20, 20, 0],
                                alignment: "justify",
                                color: "black",
                                fontSize: 10,
                                text: [
                                        { text: "13) Lembramos que os contratos e/ou aditamentos não podem ter efeitos " }
                                ]
                            },


                                            {
                                                margin: [20, 0, 20, 0],
                                                alignment: "justify",
                                                color: "black",
                                                fontSize: 10,
                                                text: [
                                                        { text: "respectivo extrato, ", bold: true },
                                                        { text: "efeitos pretéritos (art. 56 da Lei Estadual nº 6.544/89), podendo ser assinados até " },
                                                        { text: "10 (dez) dias antes da vigência, nunca depois ", bold: true },
                                                        { text: "ou em " }, { text: "dias não úteis;", bold: true }
                                                ]
                                            },


                              {
                                  margin: [70, 20, 20, 0],
                                  alignment: "justify",
                                  color: "black",
                                  fontSize: 10,
                                  text: [
                                          { text: "14)	Destacamos que o Gestor do Contrato deverá observar que: " }
                                  ]
                              },

                                            {
                                                margin: [20, 0, 20, 0],
                                                alignment: "justify",
                                                color: "black",
                                                fontSize: 10,
                                                text: [
                                                        { text: "“As informações referentes aos contratos de serviços devem ser obrigatoriamente cadastradas e mensalmente atualizadas no sistema da Corregedoria" +
                                                            " Geral da Administração, no endereço eletrônico "
                                                        },
                                                        { text: "www.terceirizados.sp.gov.br”, ", decoration: 'underline' },
                                                        { text: "lembrando que compete a Corregedoria Geral da Administração a gestão do referido cadastro, conforme Comunicado de 21 de outubro de 2004;" }

                                                ]
                                            },

                                
                                {
                                    margin: [70, 20, 20, 0],
                                    alignment: "justify",
                                    color: "black",
                                    fontSize: 10,
                                    text: [
                                            { text: "15) Frisamos, por oportuno, ser necessário também a atualização dos " },
                                            { text: "dados do ", bold : true }
                                    ]
                                },

                                            {
                                                margin: [20, 0, 20, 0],
                                                alignment: "justify",
                                                color: "black",
                                                fontSize: 10,
                                                text: [
                                                        { text: "cadastro deste contrato no Sistema de Contratos (SED).", bold: true }

                                                ]
                                            },

                                
                                {
                                    margin: [70, 20, 20, 0],
                                    alignment: "justify",
                                    color: "black",
                                    fontSize: 10,
                                    text: [
                                            { text: "Diante do exposto, mostra-se " },
                                            { text: "possível a prorrogação da vigência do contrato, ", bold: true }
                                    ]
                                },

                                            
                                            {
                                                margin: [20, 0, 20, 0],
                                                alignment: "justify",
                                                color: "black",
                                                fontSize: 10,
                                                text: [
                                                        { text: "desde que atendida toda a legislação de regência e os requisitos apontados na presente Informação. ", bold: true },
                                                        { text: "Proponho a devolução dos autos à "},
                                                        { text:  config.nomeUnidadeGestora, bold: true },
                                                        { text: ", para ciência e providências pertinentes." }

                                                ]
                                            },

                                { text: "\n\n"+config.descricaoMunicipio +", " + config.DtImpressaodia + " de " + config.DtImpressaonomemes + " de " + config.DtImpressaoano + "\n\n\n\n", fontSize: 10, bold: false, margin: [20, 0, 40, 0], alignment: "right" },

                                { text: "_______________________________________________\n" + config.UsuarioLogadoNome + "\n" + config.UsuarioLogadoCargo + " \n\n\n\n\n\n", fontSize: 10, bold: false, margin: [20, 0, 20, 0], alignment: "center" },

                                { text: "De acordo. ", margin: [20, 30, 20, 0], bold: true, fontSize: 10 },

                                {
                                    margin: [20, 0, 20, 0],
                                    alignment: "justify",
                                    color: "black",
                                    fontSize: 10,
                                    text: [
                                            { text: "Encaminhe-se à " + config.nomeUnidadeGestora + ", para as pertinentes providências.  \n\n" }, { text: "", bold: true }

                                    ]
                                },

                                  { text: "_______________________________________________\n " + config.nomeResponsavel + "\n" + config.nomeCargo + "", fontSize: 10, bold: false, margin: [20, 30, 20, 0], alignment: "center" },
                    ],
                ],
            },
            footer: function (currentPage, pageCount)
            {return {
                margin: [20, 0, 0, 40],
                columns:
                [
                    {
                        style: "tabela",
                        table: {
                            widths: [100, 100, 300],
                            headerRows: 2,
                            fontSize: 9,
                            body: [
                                     [{ text: "Endereço SEE : Praça da República, 53 - República, São Paulo/SP" + "                                                                    pagina(s) " + currentPage.toString() + " de " + pageCount, colSpan: 3, fontSize: 9, border: [false, true, false, false], bold: false, color: 'gray' }],
                                     [{ text: "                                                                                                                                                                           www.educacao.sp.gov.br", colSpan: 3, fontSize: 9, border: [false, false, false, false], bold: false, color: 'gray' }]
                            ]
                        },
                    },
                ]
            };
            },
            styles: {
                destaque: {
                    margin: [20, 0, 20, 0],
                    fontSize: 10,
                    bold: true,
                },
                tabela: {
                    margin: [20, 0, 0, 0], color: 'gray'
                },
                simples: {
                    margin: [20, 0, 20, 0],
                    fontSize: 10,
                    bold: false,
                },
                invisivel: {
                    margin: [20, 0, 20, 0],
                    fontSize: 10,
                    color: 'white',
                },
                
            },
        }
        return doc;
    };
    sedPdfExporter.exportPdf(config);
}