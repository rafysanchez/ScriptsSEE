function DocEncaminhaParaAnaliseCJ(idContrato, nrDocumento, anoProrrogacao, idProrrogacao, action) {

    $.ajax({
        type: "POST",
        url: action,
        data: { "idContrato": idContrato, "nrDocumento": nrDocumento, "anoProrrogacao": anoProrrogacao, "idProrrogacao": idProrrogacao }
    }).done(function (data) {
        debugger
        GerarDocumentoEncaminhaParaAnaliseCJ(data);
        //gerarReferencialCADTERC(data);
    }).fail(function (e) {
        mensagemAlerta.ErroAjax('Ocorreu um erro durante o processo');
    });
}


function GerarDocumentoEncaminhaParaAnaliseCJ(data) {

    debugger
    var config = {
        pageSize: "A4",
        pageOrientation: "portrait",
        title: "EncaminhaParaAnaliseCJ",
    };

    sedPdfExporter.normalizeConfig(config);

    debugger

    config.nomeUnidadeGestora = data[0].unidadeGestora.nomeUnidadeGestora != undefined ? data[0].unidadeGestora.nomeUnidadeGestora : "";
    config.NmUnidGest = data[0].unidadeGestora.codigoUnidadeGestora != undefined ? data[0].unidadeGestora.codigoUnidadeGestora : "";
    config.descricaoMunicipio = data[0].unidadeGestora.descricaoMunicipio != undefined && data[0].unidadeGestora.descricaoMunicipio != "" ? titleize(data[0].unidadeGestora.descricaoMunicipio.toLowerCase()) : "São Paulo";
    config.descricaoMunicipio = "São Paulo";
    config.numeroProcesso = data[0].processo.numeroProcesso != undefined ? data[0].processo.numeroProcesso : "";
    config.NrContrato = data[0].numeroContrato != undefined ? data[0].numeroContrato : "";
    config.NmCpfCgcCredor = data[0].nomeCredor != undefined ? data[0].nomeCredor : "";
    config.NrCpfCgcCredor = data[0].numeroCpfCnpj != undefined ? data[0].numeroCpfCnpj.replace(/^(\d{2})(\d{3})(\d{3})(\d{4})(\d{2})/, "$1.$2.$3/$4-$5") : "";
    config.dsAssunto = data[0].assuntoPlanejado.descricaoAssuntoPlanejado != undefined ? data[0].assuntoPlanejado.descricaoAssuntoPlanejado : "";
    config.DsClausulaContrato = data[0].clausulaContrato != undefined ? data[0].clausulaContrato : "";
    config.DtReajustemes = data[0].dataBaseReajuste != undefined ? dataAtualFormatadames(data[0].dataBaseReajuste) : "";
    config.DtReajusteano = data[0].dataBaseReajuste != undefined ? dataAtualFormatadaano(data[0].dataBaseReajuste) : "";
    config.DtReajuste = data[0].dataBaseReajuste != undefined ? dataAtualFormatada(data[0].dataBaseReajuste) : "";
    config.NrPercentual = data[0].percentualReajuste != undefined ? data[0].percentualReajuste + '%' : "";
    config.DsFolhaCadTerc = data[0].folhaCadTerc != undefined ? data[0].folhaCadTerc : "";
    config.IdPeriodoContrato = data[0].idPeriodoContrato != undefined ? data[0].idPeriodoContrato : "";
    config.DsUnidadeContratada = data[0].descricaoUnidadeContratada != undefined ? data[0].descricaoUnidadeContratada : "";
    config.VLReajuste = data[0].valorReajuste != undefined ? data[0].valorReajuste.toFixed(2).replace(".", ",").replace(/(\d)(?=(\d{3})+(?!\d))/g, "$1.") : "";
    config.VLReajustado = data[0].valorReajustado != undefined ? data[0].valorReajustado.toFixed(2).replace(".", ",").replace(/(\d)(?=(\d{3})+(?!\d))/g, "$1.") : "";
    config.descricaoObjeto = data[0].descricaoObjeto != undefined ? data[0].descricaoObjeto : "";
    config.dataVigenciaIni = data[0].dataVigenciaIni != undefined ? dataAtualFormatada(data[0].dataVigenciaIni) : "";
    config.dataVigenciaFim = data[0].dataVigenciaFim != undefined ? dataAtualFormatada(data[0].dataVigenciaFim) : "";
    config.nomeGestor = data[0].nomeGestor != undefined ? titleize(data[0].nomeGestor) : "";
    config.dataImpressao = data[0].dataImpressao != undefined ? dataAtualFormatada(data[0].dataImpressao) : "";
    config.DtImpressaodia = data[0].dataImpressao != undefined ? dataAtualFormatadadia(data[0].dataImpressao) : "";
    config.DtImpressaomes = data[0].dataImpressao != undefined ? dataAtualFormatadames(data[0].dataImpressao) : "";
    config.DtImpressaonomemes = data[0].dataImpressao != undefined ? dataAtualFormatadanomemes(data[0].dataImpressao) : "";
    config.DtImpressaoano = data[0].dataImpressao != undefined ? dataAtualFormatadaano(data[0].dataImpressao) : "";
    config.nrFolha = data[0].nrFolha != undefined ? data[0].nrFolha : "";
    config.nomeResponsavel = data[0].usuarioResponsavel.nmResponsavel != undefined ? titleize(data[0].usuarioResponsavel.nmResponsavel) : "";
    config.UsuarioLogadoNome = data[0].usuarioLogadoNome != undefined ? titleize(data[0].usuarioLogadoNome) : "";
    debugger
    config.UsuarioLogadoCargos = data[0].usuarioLogadoCargo != undefined ? titleize(data[0].usuarioLogadoCargo) : " ";
    config.descricaoEnderecoNumero = data[0].descricaoEnderecoNumero != undefined ? data[0].descricaoEnderecoNumero : "";
    config.numeroDocumentoAno = data[0].fluxoDocumento.ProrrogacaoFluxo.numeroDocumento + "/" + data[0].fluxoDocumento.ProrrogacaoFluxo.anoVigenteProrrogacao;

    //alteração feita para atender Fluxo CJ
    config.nomeResponsavel = "FLAVIA CRISTINA DA SILVA ROMANO";
    config.nomeCargo = "DIRETOR TECNICO III";
    //Encaminhar Fluxo CJ


    config.numeroFolhaPropostaVencedorDe = data[0].fluxoDocumento.ProrrogacaoFluxoCJ.numeroFolhaPropostaVencedorDe != undefined ? data[0].fluxoDocumento.ProrrogacaoFluxoCJ.numeroFolhaPropostaVencedorDe : "";
    config.numeroFolhaPropostaVencedorAte = data[0].fluxoDocumento.ProrrogacaoFluxoCJ.numeroFolhaPropostaVencedorAte != undefined ? data[0].fluxoDocumento.ProrrogacaoFluxoCJ.numeroFolhaPropostaVencedorAte : "";
    config.folhaUltimoReajusteAte = data[0].fluxoDocumento.ProrrogacaoFluxoCJ.folhaUltimoReajusteAte != undefined ? data[0].fluxoDocumento.ProrrogacaoFluxoCJ.folhaUltimoReajusteAte : "";
    config.numeroFolhaPropostaVencedorVolume = data[0].fluxoDocumento.ProrrogacaoFluxoCJ.numeroFolhaPropostaVencedorVolume != undefined ? data[0].fluxoDocumento.ProrrogacaoFluxoCJ.numeroFolhaPropostaVencedorVolume : "";
    config.valorTotalContratoOriginario = data[0].fluxoDocumento.ProrrogacaoFluxoCJ.valorTotalContratoOriginario != undefined ? data[0].fluxoDocumento.ProrrogacaoFluxoCJ.valorTotalContratoOriginario.toFixed(2).replace(".", ",").replace(/(\d)(?=(\d{3})+(?!\d))/g, "$1.") : "";
    config.periodoContrato = data[0].fluxoDocumento.ProrrogacaoFluxoCJ.periodoContrato != undefined ? data[0].fluxoDocumento.ProrrogacaoFluxoCJ.periodoContrato : "";
    config.valorContrato = data[0].fluxoDocumento.ProrrogacaoFluxoCJ.valorContrato != undefined ? data[0].fluxoDocumento.ProrrogacaoFluxoCJ.valorContrato.toFixed(2).replace(".", ",").replace(/(\d)(?=(\d{3})+(?!\d))/g, "$1.") : "";
    config.dataVigenciaDe = data[0].fluxoDocumento.ProrrogacaoFluxoCJ.dataVigenciaDe != undefined ? dataAtualFormatada(data[0].fluxoDocumento.ProrrogacaoFluxoCJ.dataVigenciaDe) : "";
    config.dataVigenciaAte = data[0].fluxoDocumento.ProrrogacaoFluxoCJ.dataVigenciaAte != undefined ? dataAtualFormatada(data[0].fluxoDocumento.ProrrogacaoFluxoCJ.dataVigenciaAte) : "";
    config.numeroFolhasContratoDe = data[0].fluxoDocumento.ProrrogacaoFluxoCJ.numeroFolhasContratoDe != undefined ? data[0].fluxoDocumento.ProrrogacaoFluxoCJ.numeroFolhasContratoDe : "";
    config.numeroFolhasContratoAte = data[0].fluxoDocumento.ProrrogacaoFluxoCJ.numeroFolhasContratoAte != undefined ? data[0].fluxoDocumento.ProrrogacaoFluxoCJ.numeroFolhasContratoAte : "";
    config.clausulaPrecoReajuste = data[0].fluxoDocumento.ProrrogacaoFluxoCJ.clausulaPrecoReajuste != undefined ? data[0].fluxoDocumento.ProrrogacaoFluxoCJ.clausulaPrecoReajuste : "";
    config.contratoSofreuReajuste = data[0].fluxoDocumento.ProrrogacaoFluxoCJ.contratoSofreuReajuste != undefined ? data[0].fluxoDocumento.ProrrogacaoFluxoCJ.contratoSofreuReajuste : "";
    config.folhaUltimoReajusteDe = data[0].fluxoDocumento.ProrrogacaoFluxoCJ.folhaUltimoReajusteDe != undefined ? data[0].fluxoDocumento.ProrrogacaoFluxoCJ.folhaUltimoReajusteDe : "";
    config.mesReferencia = data[0].fluxoDocumento.ProrrogacaoFluxoCJ.mesReferencia != undefined ? data[0].fluxoDocumento.ProrrogacaoFluxoCJ.mesReferencia : "";
    config.anoReferencia = data[0].fluxoDocumento.ProrrogacaoFluxoCJ.anoReferencia != undefined ? data[0].fluxoDocumento.ProrrogacaoFluxoCJ.anoReferencia : "";
    config.indiceAplicado = data[0].fluxoDocumento.ProrrogacaoFluxoCJ.indiceAplicado != undefined ? data[0].fluxoDocumento.ProrrogacaoFluxoCJ.indiceAplicado : "";
    config.valorAposReajuste = data[0].fluxoDocumento.ProrrogacaoFluxoCJ.valorAposReajuste != undefined ? data[0].fluxoDocumento.ProrrogacaoFluxoCJ.valorAposReajuste.toFixed(2).replace(".", ",").replace(/(\d)(?=(\d{3})+(?!\d))/g, "$1.") : "";
    config.numeroTermoAditamento = data[0].fluxoDocumento.ProrrogacaoFluxoCJ.numeroTermoAditamento != undefined ? data[0].fluxoDocumento.ProrrogacaoFluxoCJ.numeroTermoAditamento : "";
    config.numeroFolhasAditamentoDe = data[0].fluxoDocumento.ProrrogacaoFluxoCJ.numeroFolhasAditamentoDe != undefined ? data[0].fluxoDocumento.ProrrogacaoFluxoCJ.numeroFolhasAditamentoDe : "";
    config.numeroFolhasAditamentoAte = data[0].fluxoDocumento.ProrrogacaoFluxoCJ.numeroFolhasAditamentoAte != undefined ? data[0].fluxoDocumento.ProrrogacaoFluxoCJ.numeroFolhasAditamentoAte : "";
    config.volumeAditamento = data[0].fluxoDocumento.ProrrogacaoFluxoCJ.volumeAditamento != undefined ? data[0].fluxoDocumento.ProrrogacaoFluxoCJ.volumeAditamento : "";
    config.aplicacaoFoiCorreta = data[0].fluxoDocumento.ProrrogacaoFluxoCJ.aplicacaoFoiCorreta != undefined ? CheckLista(data[0].fluxoDocumento.ProrrogacaoFluxoCJ.aplicacaoFoiCorreta) : "";
    config.detalharIncorreto = data[0].fluxoDocumento.ProrrogacaoFluxoCJ.detalharIncorreto != undefined ? data[0].fluxoDocumento.ProrrogacaoFluxoCJ.detalharIncorreto : "";
    config.detalhamentoMotivo = data[0].fluxoDocumento.ProrrogacaoFluxoCJ.detalhamentoMotivo != undefined ? data[0].fluxoDocumento.ProrrogacaoFluxoCJ.detalhamentoMotivo : "";
    config.idDocProrrogacaoFluxo = data[0].fluxoDocumento.ProrrogacaoFluxoCJ.idDocProrrogacaoFluxo != undefined ? data[0].fluxoDocumento.ProrrogacaoFluxoCJ.idDocProrrogacaoFluxo : "";
    config.motivoAplicaoIncorreto = data[0].fluxoDocumento.ProrrogacaoFluxoCJ.motivoAplicaoIncorreto != undefined ? data[0].fluxoDocumento.ProrrogacaoFluxoCJ.motivoAplicaoIncorreto : "";

    config.termoAssinadoCorretoTrue = data[0].fluxoDocumento.ProrrogacaoFluxoCJ.termoAssinadoCorreto != undefined ? CheckLista(data[0].fluxoDocumento.ProrrogacaoFluxoCJ.termoAssinadoCorreto) : "";

    config.termoAssinadoCorretoFalse = data[0].fluxoDocumento.ProrrogacaoFluxoCJ.termoAssinadoCorreto != true ? "X" : "";

    config.aplicacaoFoiCorretaTrue = data[0].fluxoDocumento.ProrrogacaoFluxoCJ.aplicacaoFoiCorreta != undefined ? CheckLista(data[0].fluxoDocumento.ProrrogacaoFluxoCJ.aplicacaoFoiCorreta) : "";

    config.aplicacaoFoiCorretaFalse = data[0].fluxoDocumento.ProrrogacaoFluxoCJ.aplicacaoFoiCorreta != true ? "X" : "";

    if (config.periodoContrato == "D") {
        config.periodoContrato = "Meses";
    }
    else if (config.periodoContrato == "M") {
        config.periodoContrato = "Diário";
    }

    for (var i = 0; i <= data[0].listProrrogacao.length - 1; i++) {
        if (data[0].listProrrogacao[i].prorrogacaoDocumento.IdDocumento == DOCS.Minuta_Termo_Prorrogacao_Contratos_Servicos_Continuos.value) {
            config.nrFolhasMinuta = data[0].listProrrogacao[i].nrFolha != undefined ? data[0].listProrrogacao[i].nrFolha : "";
        }
    }

    for (var i = 0; i <= data[0].listProrrogacao.length - 1; i++) {
        if (data[0].listProrrogacao[i].prorrogacaoDocumento.IdDocumento == DOCS.Quadro_Comparativo_de_Precos_Referencial_CADTERC.value) {
            config.nrFolhasQuadroCadterc = data[0].listProrrogacao[i].nrFolha != undefined ? data[0].listProrrogacao[i].nrFolha : "";
        }
    }

    debugger

    //Aonde fica as informações da nova Vigência do Contrato --- Quadro_Demonstrativo_Valores_Prorrogacao_Cronograma_Desembolso
    var idTipoContratada = 0;
    for (var i = 0; i <= data[0].listProrrogacao.length - 1; i++) {
        if (data[0].listProrrogacao[i].prorrogacaoDocumento.IdDocumento == DOCS.Quadro_Demonstrativo_Valores_Prorrogacao_Cronograma_Desembolso.value) {
            config.NovaDataVigenciaIni = data[0].listProrrogacao[i].dataVigenciaIni != undefined ? dataAtualFormatada(data[0].listProrrogacao[i].dataVigenciaIni) : "";
            config.NovaDataVigenciaFim = data[0].listProrrogacao[i].dataVigenciaFim != undefined ? dataAtualFormatada(data[0].listProrrogacao[i].dataVigenciaFim) : "";
            config.NovaIdPeriodoContrato = data[0].listProrrogacao[i].contrato.idPeriodoContrato != undefined ? data[0].listProrrogacao[i].contrato.idPeriodoContrato : "";
            idTipoContratada = data[0].listProrrogacao[i].contrato.unidadeContratada;
            config.DATA_ASSINATURA_CONTRATO_ORIGINARIO = data[0].listProrrogacao[i].contrato.DATA_ASSINATURA_CONTRATO_ORIGINARIO != undefined ? dataAtualFormatada(data[0].listProrrogacao[i].contrato.DATA_ASSINATURA_CONTRATO_ORIGINARIO) : "";
            config.CLAUSULA_VIGENCIA_PRORROGACAO_CONTRATO_ORIGINARIO = data[0].listProrrogacao[i].contrato.CLAUSULA_VIGENCIA_PRORROGACAO_CONTRATO_ORIGINARIO != undefined ? data[0].listProrrogacao[i].contrato.CLAUSULA_VIGENCIA_PRORROGACAO_CONTRATO_ORIGINARIO : "";
            config.DataVigenciaAtualContratoIni = data[0].listProrrogacao[i].contrato.dataVigenciaIni != undefined ? dataAtualFormatada(data[0].listProrrogacao[i].contrato.dataVigenciaIni) : "";
            config.DataVigenciaAtualContratoFim = data[0].listProrrogacao[i].contrato.dataVigenciaFim != undefined ? dataAtualFormatada(data[0].listProrrogacao[i].contrato.dataVigenciaFim) : "";
        }
    }

    function CheckLista(check) {
        if (check == true) {
            return "X";
        }
        else {
            return " ";
        }

    }

    config.textoDoParagrafo = "";
    var sofreuReajuste = [];
    var textSofreuReajuste = "";
    if (config.contratoSofreuReajuste == true) {
        config.textoDoParagrafo = "Último Reajuste (s) aplicado (s) ";
        sofreuReajuste.push([

                    { text: "(folhas: " + config.folhaUltimoReajusteDe + "/" + config.folhaUltimoReajusteAte + " – vol. .......) \n", fontSize: 11, margin: [20, 5, 20, 0] },
                    { text: "Mês/ano de referência: " + config.mesReferencia + "/" + config.anoReferencia + " \n", alignment: "justify", fontSize: 11, bold: false, margin: [20, 5, 20, 0] },
                    { text: "Índice aplicado: " + config.indiceAplicado + "% \n", alignment: "justify", fontSize: 11, bold: false, margin: [20, 5, 20, 0], color: "black" },
                    { text: "Valor " + config.periodoContrato + " já reajustado: R$ " + config.valorAposReajuste + " \n", alignment: "justify", fontSize: 11, bold: false, margin: [20, 5, 20, 0] },
                    { text: "Aplicação correta: (  " + " " + config.aplicacaoFoiCorretaTrue.toString().trim() + " " + " ) Sim		(  " + " " + config.aplicacaoFoiCorretaFalse.toString().trim() + " " + " ) Não.", alignment: "justify", fontSize: 11, bold: false, margin: [20, 5, 20, 0] }

        ]);


        for (var i = 0; i <= sofreuReajuste[0].length - 1; i++) {
            textSofreuReajuste += sofreuReajuste[0][i].text;
        }
    }
    else {
        config.textoDoParagrafo = "";
        textSofreuReajuste = "O Contrato não sofreu reajuste de preço até a presente data.";
    }



    var body = [];
    body.push([
                { text: 'DOCUMENTO', alignment: "center", bold: true, fontsize: 11, color: "black" },
                { text: 'FOLHAS', alignment: "center", bold: true, fontsize: 11, color: "black" }
    ]);




    if (idTipoContratada == 1) {
        config.TipoContrada = "Meses";
    }
    else if (idTipoContratada == 2) {
        config.TipoContrada = "Diária";
    }


    var index = 0;
    var dados = data;

    var nomeDocumentoAnt = "";

    var nrFolhaAnt = "";
    debugger
    if (dados[0].listProrrogacao.length > 0) {
        for (var key in dados[0].listProrrogacao) {
            var nomeDocumento = dados[0].listProrrogacao[key].prorrogacaoDocumento.descricaoDocumento != undefined ? dados[0].listProrrogacao[key].prorrogacaoDocumento.descricaoDocumento.toString() : "";

            var nrFolha = dados[0].listProrrogacao[key].nrFolha != undefined ? dados[0].listProrrogacao[key].nrFolha.toString() : "";

            if (nomeDocumento != nomeDocumentoAnt) {
                if (nomeDocumento != "") {

                    if (nomeDocumento != "Gerar Check List") {

                        body.push([

                            { text: nomeDocumento.toString(), alignment: "left", bold: true, fontsize: 13, color: "black" },
                            { text: nrFolha.toString(), alignment: "left", bold: true, fontsize: 13, color: "black" },

                        ]);

                    }

                }

            }

            var nomeDocumentoAnt = dados[0].listProrrogacao[key].prorrogacaoDocumento.nomeDocumento != undefined ? dados[0].listProrrogacao[key].prorrogacaoDocumento.nomeDocumento.toString() : "";

            var nrFolhaAnt = dados[0].listProrrogacao[key].nrFolha != undefined ? dados[0].listProrrogacao[key].nrFolha.toString() : "";

        }
    }

    config.body = body;



    config.sofreuReajuste = textSofreuReajuste;
    config.docGenerator = function (config) {
        var doc =
        {
            pageSize: "A4",
            title: "EncaminhaParaAnaliseCJ",
            pageMargins: [30, 150, 50, 40],
            header:
            {
                margin: [0, 20, 0, 0],
                columns:
                [
                    {
                        image: "data:image/png;base64,",
                        width: 65,
                        height: 75,
                        margin: [40, 10, 20, 0],
                    },
                    [
                        { text: "GOVERNO DO ESTADO DE SÃO PAULO", alignment: "center", fontSize: 15, bold: true, margin: [0, 10, 40, 0], },
                        { text: "SECRETARIA DA EDUCAÇÃO", alignment: "center", fontSize: 14, bold: true, margin: [0, 0, 40, 0], },
                        { text: "COORDENADORIA DE ORÇAMENTOS E FINANÇAS", alignment: "center", fontSize: 11, bold: false, margin: [0, 0, 40, 0], },
                        { text: "DEPARTAMENTO DE CONTROLE DE CONTRATOS E CONVÊNIOS", alignment: "center", fontSize: 11, bold: false, margin: [0, 0, 40, 0], },
                        { text: "CENTRO DE ACOMPANHAMENTO E CONTROLE DE CONTRATOS", alignment: "center", fontSize: 11, bold: false, margin: [0, 0, 40, 0], },
                    ],
                ],
            },

            content:
            {
                margin: [0, 0, 0, 0],
                columns:
                [
                    [
                        {
                            bold: true, fontSize: 10, margin: [20, 10, 40, 0], text: ["PROCESSO Nº : ",
                            { text: config.numeroProcesso, bold: false }, ]
                        },

                        {
                            style: "destaque",
                            text: [
                                "INTERESSADO: ",
                                { text: "" + config.nomeUnidadeGestora, style: "simples" }
                            ]
                        },

                        {
                            bold: true, fontSize: 10, margin: [20, 0, 40, 0], text: ["ASSUNTO PLANEJADO: ",
                            { text: config.descricaoObjeto, bold: false }, ]
                        },

                        {
                            bold: true, fontSize: 10, margin: [20, 0, 40, 0], text: ["INFORMAÇÃO Nº ",
                            { text: "" + config.numeroDocumentoAno + " \n\n", bold: false }, ]
                        },

                        { text: "\n\nSenhor(a) Diretor(a) do CCONT,\n\n\n", alignment: "left", fontSize: 10, bold: false, margin: [20, 5, 10, 0], },

                        {
                            alignment: "justify",
                            style: "invisivel",
                            text:
                            [
                                { text: "\n\n................", color: "white" },
                                { text: "Trata o presente de prestação de serviços de " + config.descricaoObjeto + ", " + config.nomeUnidadeGestora + " - Contrato Nº ", alignment: "justify", bold: false, fontSize: 10, color: "black" },
                                { text: config.NrContrato, bold: true, color: "black" },
                                { text: " firmado com a empresa " + config.NmCpfCgcCredor + ", com proposta advinda da citada Diretoria de Ensino quanto à análise da minuta do ", color: "black" },
                                { text: "Termo de Aditamento Contratual ", bold: true, color: "black" },
                                { text: "(minuta às folhas: (" + config.nrFolhasMinuta + "), no qual se objetiva a ", bold: true, color: "black" },
                                { text: "PRORROGAÇÃO do prazo de vigência do Contrato", bold: true, color: "black" },
                                { text: ", por " + config.NovaIdPeriodoContrato + " " + config.TipoContrada + ", contados a partir de " + config.NovaDataVigenciaIni + " e término em " + config.NovaDataVigenciaFim + ". \n\n", bold: true, color: "black" },
                            ]
                        },

                        {
                            style: "invisivel",
                            text: ["\n\n......................................",
                                { text: "Pelos documentos constantes dos autos, destacamos:\n\n\n", style: "simples", fontSize: 10 }
                            ]
                        },


                        {
                            margin: [70, 20, 20, 0],
                            alignment: "justify",
                            color: "black",
                            fontSize: 10,
                            text: [
                                    { text: "Proposta vencedora", bold: true, decoration: 'underline' },
                            { text: " (folhas: " + config.numeroFolhaPropostaVencedorDe + "/" + config.numeroFolhaPropostaVencedorAte + " –vol. " + config.numeroFolhaPropostaVencedorVolume + "), no valor " + config.periodoContrato + " estimado de R$ " + config.valorContrato + " e valor total estimado para (" + config.NovaIdPeriodoContrato + ") " + config.periodoContrato + " de R$ " + config.valorTotalContratoOriginario + ";\n\n", alignment: "left", fontSize: 10, bold: false, margin: [20, 5, 10, 0], }
                            ],
                        },


                        {
                            margin: [70, 20, 20, 0],
                            alignment: "justify",
                            color: "black",
                            fontSize: 10,
                            text: [
                                    { text: "Contrato nº " + config.NrContrato + " (folhas: " + config.numeroFolhaPropostaVencedorDe + "/" + config.numeroFolhaPropostaVencedorAte + " –vol. " + config.numeroFolhaPropostaVencedorVolume + "), firmado em " + config.DATA_ASSINATURA_CONTRATO_ORIGINARIO + " ", fontSize: 10, decoration: 'underline', bold: true },
                            { text: "Vigência (cláusula " + config.CLAUSULA_VIGENCIA_PRORROGACAO_CONTRATO_ORIGINARIO.toString().trim() + ") – (" + config.NovaIdPeriodoContrato + ") " + config.TipoContrada + ", consecutivos e ininterruptos, com início em " + config.dataVigenciaDe + " e término em " + config.dataVigenciaAte + " ;\n", alignment: "justify", fontSize: 10, bold: false, margin: [20, 5, 20, 0], },
                            { text: "Preço e Reajuste (cláusula " + config.clausulaPrecoReajuste + ") - base " + config.periodoContrato + " estimada de R$ " + config.valorTotalContratoOriginario + " e valor total estimado para (" + config.NovaIdPeriodoContrato + ") " + config.periodoContrato + " de R$ " + config.valorContrato + ", com reajuste anual, sendo " + config.mesReferencia + "/" + config.anoReferencia + " o mês de referência de preços;", alignment: "justify", fontSize: 10, bold: false, margin: [20, 5, 20, 0], },
                            ],
                        },


                        {
                            margin: [70, 20, 20, 0],
                            alignment: "justify",
                            color: "black",
                            fontSize: 10,
                            text: [
                                { text: "", color: "white" },
                                { text: config.textoDoParagrafo, bold: true, decoration: 'underline' },
                                config.sofreuReajuste
                            ]
                        },


                                {
                                    alignment: "justify",
                                    style: "invisivel",
                                    color: "black",
                                    fontSize: 10,
                                    text:
                                    [
                                        { text: "................", color: "white" },
                                        { text: "" + config.motivoAplicaoIncorreto + "" }
                                    ]
                                },


                        {
                            margin: [70, 50, 20, 0],
                            alignment: "justify",
                            color: "black",
                            fontSize: 10,
                            text: [
                                    { text: "Vigência atual do contrato: ", alignment: "justify", bold: true, fontSize: 10, color: "black", decoration: 'underline' },
                                    { text: "" + config.DataVigenciaAtualContratoIni + " a " + config.DataVigenciaAtualContratoFim + " \n", alignment: "justify", bold: false, fontSize: 10, color: "black" },
                                    { text: config.numeroTermoAditamento + " Termo de Aditamento Contratual (folhas: " + config.numeroFolhasAditamentoDe + "/ " + config.numeroFolhasAditamentoAte + " – vol." + config.volumeAditamento + ") \n", fontSize: 10, color: "black" },
                                    { text: "Assinado corretamente: (  " + " " + config.termoAssinadoCorretoTrue.toString().trim() + " " + " ) Sim		(  " + " " + config.termoAssinadoCorretoFalse.toString().trim() + " " + " ) Não. \n", fontSize: 10, color: "black" }
                            ],
                        },

                        {
                            alignment: "justify",
                            style: "invisivel",
                            color: "black",
                            fontSize: 10,
                            text:
                            [
                                { text: "\n\n................", color: "white" },
                                { text: "" + config.detalharIncorreto + "" }
                            ]
                        },

                        {
                            alignment: "justify",
                            style: "invisivel",
                            color: "black",
                            fontSize: 10,
                            text:
                            [
                                { text: "\n\n\n\n\n\n................", color: "white" },
                                { text: "Em relação à " }, { text: "prorrogação", bold: true }, { text: " pretendida, constam dos autos os seguintes documentos: \n\n\n" }
                            ]
                        },


                        {
                            margin: [20, 0, 0, 0],
                            table: {
                                body: config.body
                            },
                            fontSize: 10
                        },
                        //Neste ponto, deverá ser possibilitada a inserção inseridochecklist gerado durante a instrução processual pela diretoria, o qual vai sendo analisado pelo.


                        {
                            alignment: "justify",
                            style: "invisivel",
                            color: "black",
                            fontSize: 10,
                            text:
                            [
                                { text: "\n\n................", color: "white" },
                                { text: "À vista da análise dos documentos constantes dos autos, e considerando que:" }
                            ]
                        },

                        {
                            alignment: "justify",
                            style: "invisivel",
                            color: "black",
                            fontSize: 10,
                            text:
                            [
                                { text: "\n\n................", color: "white" },
                                { text: "" + config.detalhamentoMotivo + "." }
                            ]
                        },

                        //Neste ponto, a Prodesp deverá possibilitar que o analista descreva os motivos pelos quais está encaminhando o processo para análise da CJ.


                        {
                            alignment: "justify",
                            style: "invisivel",
                            text:
                            [
                                "\n\n......................................",
                                { text: "Diante do exposto, propomos o encaminhamento dos autos à", alignment: "justify", bold: false, fontSize: 10, color: "black" },
                                { text: " Douta Consultoria Jurídica da Pasta,", alignment: "justify", bold: true, fontSize: 10, color: "black" },
                                { text: " para fins de análise e parecer nos termos do artigo 38, parágrafo único da Lei Federal nº 8.666/93.", alignment: "justify", bold: false, fontSize: 10, color: "black" }
                            ]
                        },

                        { text: "\n\n" + config.descricaoMunicipio + ", " + config.DtImpressaodia + " de " + config.DtImpressaonomemes + " de " + config.DtImpressaoano + "\n\n\n\n", fontSize: 10, bold: false, margin: [20, 0, 40, 0], alignment: "right" },

                        { text: "_______________________________________________\n" + config.UsuarioLogadoNome + " n/ " + config.UsuarioLogadoCargos, fontSize: 10, bold: false, margin: [20, 0, 20, 0], alignment: "center" },

                        { text: "De acordo.", alignment: "left", fontSize: 10, bold: true, margin: [20, 20, 20, 0], },

                        {
                            alignment: "justify",
                            style: "invisivel",
                            text:
                            [
                                { text: "Encaminhe-se à", bold: false, fontSize: 10, color: "black" },
                                { text: " Consultoria Jurídica.", bold: true, fontSize: 10, color: "black" }
                            ]
                        },

                        { text: "\n\n\n\n", alignment: "left", fontSize: 10, bold: false, margin: [20, 0, 20, 0], },

                        { text: "_______________________________________________\n " + config.nomeResponsavel + "\n" + config.nomeCargo + "", fontSize: 10, bold: false, margin: [20, 30, 20, 0], alignment: "center" },

                    ],
                ],
            },
            footer: function (currentPage, pageCount) {

                return {
                    margin: [20, 0, 0, 40],
                    columns:
                    [
                        {
                            style: "tabela",
                            table: {
                                widths: [100, 100, 300],
                                headerRows: 2,
                                fontSize: 9,
                                body: [
                                         [{ text: "Endereço SEE : Praça da República, 53 - República, São Paulo/SP" + "                                                                     pagina(s) " + currentPage.toString() + " de " + pageCount, colSpan: 3, fontSize: 9, border: [false, true, false, false], bold: false, color: 'gray' }],
                                         [{ text: "                                                                                                                                                                           www.educacao.sp.gov.br", colSpan: 3, fontSize: 9, border: [false, false, false, false], bold: false, color: 'gray' }]
                                ]
                            },
                        },
                    ]
                };
            },
            styles: {
                tabela: {
                    margin: [20, 0, 0, 0], color: 'gray'
                },
                destaque: {
                    margin: [20, 0, 20, 0],
                    fontSize: 10,
                    bold: true
                },
                simples: {
                    margin: [20, 0, 20, 0],
                    fontSize: 10,
                    bold: false,
                    color: 'black',
                },
                invisivel: {
                    margin: [20, 0, 20, 0],
                    fontSize: 10,
                    color: 'white',
                },
            },


        }
        return doc;
    };
    sedPdfExporter.exportPdf(config);
}