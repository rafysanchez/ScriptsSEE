function obterdespachoGestorAutorizaAutoridade_(IdContrato) {
    if (IdContrato != 0) {

        $.ajax({
            cache: false,
            dataType: 'JSON',
            contentType: 'application/x-www-form-urlencoded; charset=utf-8',
            url: '../GerardespachoGestorAutorizaAutoridade_',
            data: JSON.stringify({ "IdContrato": IdContrato }),
            type: 'POST',
            success: function (data) {
                gerardespachoGestorAutorizaAutoridade_(data);
            },
            contentType: 'application/json; charset=utf-8',
            error: function (jqXHR, txtStatus, errorThrown) {
                mensagemAlerta.ErroAjax('Ocorreu um erro durante o processo' + errorThrown);
            }
        });
    }
    else {
        mensagemAlerta.ErroAjax('Ocorreu um erro durante o processo');
    }
}


function gerardespachoGestorAutorizaAutoridade_() {

    
    var config = {
        pageSize: "A4",
        pageOrientation: "portrait",
        // pageOrientation: 'landscape',
        title: "teste",
    };

    sedPdfExporter.normalizeConfig(config);

    config.codigoUnidadeGestora = data[0].unidadeGestora.codigoUnidadeGestora != undefined ? data[0].unidadeGestora.codigoUnidadeGestora : "";
    config.nomeUnidadeGestora = data[0].unidadeGestora.nomeUnidadeGestora != undefined ? data[0].unidadeGestora.nomeUnidadeGestora : "";
    config.NmUnidGest = data[0].unidadeGestora.codigoUnidadeGestora != undefined ? data[0].unidadeGestora.codigoUnidadeGestora : "";
    config.descricaoEnderecoNumero = data[0].unidadeGestora.descricaoEnderecoNumero != undefined ? data[0].unidadeGestora.descricaoEnderecoNumero : "";
    config.descricaoMunicipio = data[0].unidadeGestora.descricaoMunicipio != undefined && data[0].unidadeGestora.descricaoMunicipio != "" ? titleize(data[0].unidadeGestora.descricaoMunicipio.toLowerCase()) : "São Paulo";
    config.numeroProcesso = data[0].processo.numeroProcesso != undefined ? data[0].processo.numeroProcesso : "";
    config.NrContrato = data[0].numeroContrato != undefined ? data[0].numeroContrato : "";
    config.NmCpfCgcCredor = data[0].notaEmpenho.credor.nomeCredor != undefined ? data[0].notaEmpenho.credor.nomeCredor : "";
    config.NrCpfCgcCredor = data[0].notaEmpenho.credor.numeroCpfCnpj != undefined ? data[0].notaEmpenho.credor.numeroCpfCnpj.replace(/^(\d{2})(\d{3})(\d{3})(\d{4})(\d{2})/, "$1.$2.$3/$4-$5") : "";
    config.dsAssunto = data[0].assuntoPlanejado.descricaoAssuntoPlanejado != undefined ? data[0].assuntoPlanejado.descricaoAssuntoPlanejado : "";
    config.DsClausulaContrato = data[0].clausulaVPContratoOriginario != undefined ? data[0].clausulaVPContratoOriginario.trim() : "";
    config.DtReajustemes = data[0].listReajustes[0].dataReajuste != undefined ? dataAtualFormatadames(data[0].listReajustes[0].dataReajuste) : "";
    config.DtReajusteano = data[0].listReajustes[0].dataReajuste != undefined ? dataAtualFormatadaano(data[0].listReajustes[0].dataReajuste) : "";
    config.DtReajuste = data[0].listReajustes[0].dataReajuste != undefined ? dataAtualFormatada(data[0].listReajustes[0].dataReajuste) : "";
    config.NrPercentual = data[0].listReajustes[0].percentualReajuste != undefined ? data[0].listReajustes[0].percentualReajuste + '%' : "";
    config.DsFolhaCadTerc = data[0].listReajustes[0].folhaCadterc != undefined ? data[0].listReajustes[0].folhaCadterc : "";
    config.IdPeriodoContrato = data[0].idPeriodoContrato != undefined ? data[0].idPeriodoContrato : "";
    config.DsUnidadeContratada = data[0].descricaoUnidadeContratada != undefined ? data[0].descricaoUnidadeContratada : "";
    config.VLReajuste = data[0].listReajustes[0].valorReajuste != undefined ? data[0].listReajustes[0].valorReajuste.toFixed(2).replace(".", ",").replace(/(\d)(?=(\d{3})+(?!\d))/g, "$1.") : "";
    config.VLReajustado = data[0].listReajustes[0].valorReajustado != undefined ? data[0].listReajustes[0].valorReajustado.toFixed(2).replace(".", ",").replace(/(\d)(?=(\d{3})+(?!\d))/g, "$1.") : "";
    config.descricaoObjeto = data[0].descricaoObjeto != undefined ? data[0].descricaoObjeto : "";
    config.dataVigenciaIni = data[0].dataVigenciaIni != undefined ? dataAtualFormatada(data[0].dataVigenciaIni) : "";
    config.dataVigenciaFim = data[0].dataVigenciaFim != undefined ? dataAtualFormatada(data[0].dataVigenciaFim) : "";

    if (config.codigoUnidadeGestora == "080101" || config.codigoUnidadeGestora == "080102" ||
   config.codigoUnidadeGestora == "080011" || config.codigoUnidadeGestora == "080104" ||
   config.codigoUnidadeGestora == "080259" || config.codigoUnidadeGestora == "080353" ||
   config.codigoUnidadeGestora == "080356" || config.codigoUnidadeGestora == "080357" ||
   config.codigoUnidadeGestora == "080358" || config.codigoUnidadeGestora == "080359" ||
   config.codigoUnidadeGestora == "080040" || config.codigoUnidadeGestora == "080360"
   ) {

        config.descricaoEnderecoNumero = "Endereço SEE : Praça da República, 53 - República, São Paulo/SP";
    }

    debugger;
    config.docGenerator = function (config) {
        var doc = {
            pageSize: "A4",
            title: "Despacho do Gestor e Autorização da Autoridade",
            pageMargins: [30, 150, 50, 30],

            header: {
                margin: [0, 20, 0, 0],
                columns: [
                    {
                        image: "data:image/png;base64,",
                        width: 80,
                        height: 80,
                        margin: [40, 10, 20, 0],
                    },
                    [
                        { text: "GOVERNO DO ESTADO DE SÃO PAULO", alignment: "center", fontSize: 16, bold: true, margin: [0, 10, 40, 0], },
                        { text: "SECRETARIA DA EDUCAÇÃO", alignment: "center", fontSize: 14, bold: true, margin: [0, 0, 40, 0], },
                        { text: config.nomeUnidadeGestora, alignment: "center", fontSize: 12, bold: false, margin: [0, 0, 40, 0], },
                        { text: "\n\n\n\nDESPACHO DO GESTOR E AUTORIZAÇÃO DA AUTORIDADE", alignment: "center", fontSize: 14, bold: false, margin: [0, 0, 40, 0], },
                    ],
                ],
            },

            content: {
                margin: [0, 0, 0, 0],
                columns: [
                    [
                        {
                            bold: true, fontSize: 13, margin: [20, 10, 40, 0], text: ["PROCESSO Nº : ",
                            { text: config.numeroProcesso + " (1)", bold: false }, ]
                        },
                        {
                            bold: true, fontSize: 13, margin: [20, 0, 40, 0], text: ["CONTRATO Nº : ",
                            { text: config.NrContrato + " (2)", bold: false }, ]
                        },
                        {
                            bold: true, fontSize: 13, margin: [20, 0, 40, 0], text: ["ASSUNTO PLANEJADO : ",
                            { text: config.descricaoObjeto + " (4)\n\n", style: "simples" }]
                        },

                        { text: "Considerando a série histórica da execução do Contrato Nº " + config.NrContrato + " (2), firmado entre a Diretoria de Ensino Região de " + config.unidadeGestora + " (5) e a empresa " + config.NmCpfCgcCredor + " (3), decorrente do acompanhamento e análise da execução do objeto do contrato, e que demonstram que a Contratada detém capacidade técnica para a continuidade na prestação dos serviços, solicito nos termos da cláusula " + config.DsClausulaContrato + " (6) - da vigência e prorrogações, que seja autorizada a prorrogação do contrato de " + config.descricaoObjeto + " (4), destinado ao atendimento das necessidades das Unidades Escolares, com vistas a garantir um ambiente adequado para o desenvolvimento e aprendizagem dos alunos, escopo e missão da Secretaria da Educação.", alignment: "justify", fontSize: 12, bold: false, margin: [20, 0, 40, 0], },
                        { text: "Ressalta-se que os preços praticados no contrato são compatíveis com o mercado, conforme demonstram                        (7). (pesquisas de preço ou valores referenciais do CADTER) às fls. " + config.DsFolhaCadTerc + " (8), e quadro comparativo às fls. " + config.DsFolhaCadTerc + " (3), tornando a prorrogação vantajosa para a Administração.\n\n", alignment: "justify", fontSize: 12, bold: false, margin: [20, 0, 40, 0], },
                     
                        { text: "______________________________________\nGestor do Contrato(10)\n\n\n\n", alignment: "center", fontSize: 12, bold: true, margin: [20, 0, 40, 0], },

                        { text: "Considerando as justificativas acima apresentadas pelo Gestor do Contrato, em cumprimento ao disposto no parágrafo segundo do artigo 57 da Lei 8.666/1993, e no uso de minhas atribuições legais, AUTORIZO a adoção das providências com vistas à prorrogação do contrato nº " + config.NrContrato + " (2), nos termos da cláusula " + config.DsFolhaCadTerc + " (6) - da vigência e prorrogações, firmado entre esta Diretoria de Ensino Região " + config.nomeUnidadeGestora + " (5) e a empresa " + config.NmCpfCgcCredor + " (3), cujo objeto é a prestação de serviços de " + config.descricaoObjeto + " (4).", alignment: "justify", fontSize: 12, bold: false, margin: [20, 0, 40, 0], },
                        { text: "Encaminhe-se preliminarmente ao Núcleo de Finanças para providências quanto à reserva de recursos, e após ao Núcleo de Compras e Serviços para prosseguimento dos trâmites necessários ao aditamento, e posterior envio dos autos ao DECON/CCONT para manifestação, nos termos da Resolução PGE-23/2015.\n\n\n", alignment: "justify", fontSize: 12, bold: false, margin: [20, 0, 40, 0], },
                        { text: config.descricaoMunicipio + ", " + config.DtImpressaodia + " de " + config.DtImpressaonomemes + " de " + config.DtImpressaoano + "\n\n\n", alignment: "right", fontSize: 12, bold: false, margin: [20, 0, 40, 0], },
                        { text: "_____________________________________________________________________________________\n(Autoridade Competente-Ordenador de despesa)(11)\n\n\n\n", alignment: "center", fontSize: 12, bold: true, margin: [20, 0, 40, 0], },
                    ],
                ],
            },
            footer:
                {
                    margin: [20, 0, 0, 40],
                    columns:
                    [
                        {
                            style: "tabela",
                            table: {
                                widths: [100, 100, 300],
                                headerRows: 2,
                                fontSize: 9,
                                body: [
                                         [{ text: "Endereço UGE : " + config.descricaoEnderecoNumero, colSpan: 3, fontSize: 9, border: [false, true, false, false], bold: false, color: 'gray' }],
                                         [{ text: "                                                                                                                                                                           www.educacao.sp.gov.br", colSpan: 3, fontSize: 9, border: [false, false, false, false], bold: false, color: 'gray' }]
                                ]
                            },
                        },
                    ],
                },

            styles: {
                destaque: {
                    margin: [20, 0, 40, 0],
                    fontSize: 13,
                    bold: true
                },
                header: {
                    fontSize: 10,
                    bold: false,
                    margin: [50, 0, 30, 1]
                },
                tabela: {
                    margin: [20, 0, 40, 0], color: 'gray'
                },
                simples: {
                    fontSize: 13,
                    bold: false,
                }
            },


            //    footer: {
            //        margin: [40, 0, 0, 40],
            //        columns: [
            //            {
            //                style: "tabela",
            //                table: {
            //                    widths: [100, 100, 300],
            //                    headerRows: 1,
            //                    body: [
            //                                [{ text: "Endereço da UGE", colSpan: 3, fontSize: 9, border: [false, true, false, false], bold: false }, {}, {}],
            //                    ]
            //                },
            //            },
            //        ]
            //    }
        }
        return doc;
    };
    sedPdfExporter.exportPdf(config);
}