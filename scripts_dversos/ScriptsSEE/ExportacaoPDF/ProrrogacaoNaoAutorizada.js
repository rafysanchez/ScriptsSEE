function DocProrrogacaoNaoAutorizada(idContrato, nrDocumento, anoProrrogacao, idProrrogacao, action) {
    $.ajax({
        type: "POST",
        url: action,
        ...
        debugger
        DocumentoProrrogacaoNaoAutorizada(data);
...
function DocumentoProrrogacaoNaoAutorizada(data) {

    var config = {
        pageSize: "A4",
        pageOrientation: "portrait",
        // pageOrientation: 'landscape',
        title: "ProrrogacaoNaoAutorizada",
    };
    ...
    config.checkFolhaPesquisa = CheckLista(data[0].fluxoDocumento.ProrrogacaoFluxoReprovacao.checkFolhaPesquisa);
    config.numeroFolhaPesquisa = data[0].fluxoDocumento.ProrrogacaoFluxoReprovacao.numeroFolhaPesquisa != undefined ? data[0].fluxoDocumento.ProrrogacaoFluxoReprovacao.numeroFolhaPesquisa : "";


    config.check60Dias = CheckLista(data[0].fluxoDocumento.ProrrogacaoFluxoReprovacao.check60Dias);

    config.checkFolhaQuadro = CheckLista(data[0].fluxoDocumento.ProrrogacaoFluxoReprovacao.checkFolhaQuadro);
    config.numeroFolhaQuadro = data[0].fluxoDocumento.ProrrogacaoFluxoReprovacao.numeroFolhaQuadro != undefined ? data[0].fluxoDocumento.ProrrogacaoFluxoReprovacao.numeroFolhaQuadro : "";

    config.checkCertidoes = CheckLista(data[0].fluxoDocumento.ProrrogacaoFluxoReprovacao.checkCertidoes);
    //trazer documentos reprovados nome e nrFolha ex. QuadroCadterc(100), etc.....

    config.checkOutros = CheckLista(data[0].fluxoDocumento.ProrrogacaoFluxoReprovacao.checkOutros);
    config.especificarOutros = data[0].fluxoDocumento.ProrrogacaoFluxoReprovacao.especificarOutros != undefined ? data[0].fluxoDocumento.ProrrogacaoFluxoReprovacao.especificarOutros : "";

    config.folhasRecusadas = getFolhasRecusadas(data[0].listProrrogacao);

    function getFolhasRecusadas(listProrrogacao) {
        var motivoReprovacao = "";
        var quadroComparativoPrecos = false;
        for (var i = 0; i <= listProrrogacao.length - 1; i++) {
            if (data[0].listProrrogacao[i].motivoReprovacao != "" && (data[0].listProrrogacao[i].statusProrrogacao == StatusConvenioProrrogacao.EmAnalise.value || data[0].listProrrogacao[i].statusProrrogacao == StatusConvenioProrrogacao.Reprovado.value)) {
                for (var propertyName in DOCS) {
                    // O quadro comparativo de preços esta vindo repetido por este motivo da Validação
                    if (data[0].listProrrogacao[i].prorrogacaoDocumento.IdDocumento == DOCS[propertyName].value && data[0].listProrrogacao[i].prorrogacaoDocumento.IdDocumento != DOCS.Quadro_Comparativo_Precos.value) {
                        motivoReprovacao += "(" + DOCS[propertyName].name + ", " + data[0].listProrrogacao[i].nrFolha + "), ";
                    }

                        // O quadro comparativo de preços esta vindo repetido por este motivo da Validação
                    else if (data[0].listProrrogacao[i].prorrogacaoDocumento.IdDocumento == DOCS.Quadro_Comparativo_Precos.value && quadroComparativoPrecos != true) {
                        motivoReprovacao += "(" + DOCS.Quadro_Comparativo_Precos.name + ", " + data[0].listProrrogacao[i].nrFolha + "), ";
                        quadroComparativoPrecos = true;
                    }
                }
            }
        }
        motivoReprovacao = motivoReprovacao.substring(0, (motivoReprovacao.length - 2));
    ...
    config.docGenerator = function (config) {
        var doc = {
            pageSize: "A4",
            title: "Prorrogação não autorizada",
                ...
                columns: [
                    [
...

                        {
                            ...
                            text: [
                                    { text: "\n.................", style: "invisivel" },
                                    { text: "Trata o presente de prestação de serviços de " + config.descricaoObjeto + ", ", alignment: "justify", fontSize: 10, bold: false },
                                    { text: config.nomeUnidadeGestora + " - Contrato nº " + config.numeroProcesso + ", firmado com a empresa " + config.NmCpfCgcCredor + ", com proposta advinda da citada Diretoria de Ensino quanto à análise da minuta do " },
                                    { text: "Termo de Aditamento Contratual ", bold: true },
...

                        {
                            ...
                            text: [
                                    { text: "\n.................", style: "invisivel" },
                                    { text: "À vista da análise dos documentos constantes dos autos, e com base na legislação em vigor, " },
                                    { text: "concluímos que o contrato supracitado " },
                                    { text: "não apresenta condições de seguir com o aditamento objetivando prorrogação da vigência, ", bold: true },
                                    { text: "pelo (s) seguinte (s) motivo (s):" }
                            ]
                        },

                        {
                            ...
                            fontSize: 10,
                            text: [
                                    { text: "(" + config.checkFolhaPesquisa + ") A contratada está penalizada com impedimento de licitar com a Admnistração, conforme pesquisa de folhas: (" + config.numeroFolhaPesquisa + "). \n\n" },
                                    { text: "(" + config.check60Dias + ") A prorrogação pretendida ultrapassa o limite de 60 (sessenta) meses, estabelecido o artigo 57 da lei 8.666/93; \n\n" },
                                    { text: "(" + config.checkFolhaQuadro + ") Há itens contratados com valores acima dos preços referenciais do CADTERC Volume .......... e não renegociados, conforme planilha de folhas: (" + config.numeroFolhaQuadro + "). \n\n" },
                                    { text: "(" + config.checkCertidoes + ") As condições de habilitação da contratada não estão mantidas, conforme documentos e/ou certidões de folhas: " + config.folhasRecusadas + " \n\n" },
                                    { text: "(" + config.checkOutros + ") Outros – Especificar: " + config.especificarOutros + ". \n\n" }
                            ]
                        },
                                {
                                    alignment: "justify",
                                    ...
                                    text: [
                                            { text: "\n\n\n.................", style: "invisivel" },
                                            { text: "Diante do exposto, proponho a devolução dos autos à " },
                                            { text: config.nomeUnidadeGestora + ", ", bold: true },
                                            { text: "para conhecimento e demais providências necessárias à abertura de novo processo licitatório, a fim de garantir a continuidade na prestação dos serviços. \n\n\n\n" }
                                    ]
                                },

                                { text: "\n\n\n\n" + config.descricaoMunicipio + ", " + config.DtImpressaodia + " de " + config.DtImpressaonomemes + " de " + config.DtImpressaoano + "\n\n\n\n", fontSize: 10, bold: false, margin: [20, 40, 40, 0], alignment: "right" },

                                { text: "_______________________________________________\n" + config.UsuarioLogadoNome + " / " + config.UsuarioLogadoCargos, fontSize: 10, bold: false, margin: [20, 0, 20, 0], alignment: "center" },
...

                                {
                                    ...
                                    fontSize: 10,
                                    text: [
                                            { text: "Encaminhe-se à " + config.nomeUnidadeGestora + ", para as pertinentes providências." }, { text: "", bold: true }
                ...
                [
                    {
                        ...
                        table: {
                            ...
                            body: [
                                    [{
                                        text: "Endereço SEE : Praça da República, 53 - República, São Paulo/SP" + "                                                                 pagina(s) " + currentPage.toString() + " de " + pageCount,
                                        colSpan: 3, fontSize: 9, border: [false, true, false, false], bold: false, color: 'gray'
                                    }],
