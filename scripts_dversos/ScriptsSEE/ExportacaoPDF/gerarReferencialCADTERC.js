function gerarReferencialCADTERC(data) {

    var config = {
        pageSize: "A4",
        pageOrientation: "portrait",
        title: "ReferencialCADTERC",
    };
    ...
    body.push([
                ...
                { text: "Quantidade Contratada", fontsize: 9 },

                { text: "Valor Unitário Contratado", fontsize: 9 },

                { text: "Valor Unitário Referencial do CADTERC", fontsize: 9 },

                { text: 'Validação', fontsize: 9 }
    ]);
    ...
    for (var i = 0; i < data[0].listItensNotaContratoDemonstrativo.length; i++) {
        ...
        config.descricao = data[0].listItensNotaContratoDemonstrativo[i].Descricao;

        config.quantidadeContradada = data[0].listItensNotaContratoDemonstrativo[i].QuantidadeItem.toFixed(2).replace(".", ",");

        config.valorUnitarioContratado = parseFloat(data[0].listItensNotaContratoDemonstrativo[i].ValorUnitario).toFixed(2).replace(".", ",").replace(/(\d)(?=(\d{3})+(?!\d))/g, "$1.");

        config.valorReferencialCadterc = parseFloat(data[0].listItensNotaContratoDemonstrativo[i].valorUnitarioReferencialCadterc).toFixed(2).replace(".", ",").replace(/(\d)(?=(\d{3})+(?!\d))/g, "$1.");

        config.validacao = data[0].listItensNotaContratoDemonstrativo[i].descricaoFlgAceito;

        config.somaValorUnitarioContratado = parseFloat(data[0].itemNotaContratoDemonstrativo.somaValorUnitario).toFixed(2).replace(".", ",").replace(/(\d)(?=(\d{3})+(?!\d))/g, "$1.");
        ...
        config.economiaMensal = parseFloat(data[0].itemNotaContratoDemonstrativo.economiaMensal).toFixed(2).replace(".", ",").replace(/(\d)(?=(\d{3})+(?!\d))/g, "$1.");

        if (count == 1) {
            porCentagem = (((data[0].itemNotaContratoDemonstrativo.somaValorUnitarioReferencialCadterc / data[0].itemNotaContratoDemonstrativo.somaValorUnitario) - 1) * 100)
        }

        body.push([

            { text: config.item.toString() },
            { text: config.descricao },
            { text: config.quantidadeContradada },
            { text: config.valorUnitarioContratado },
            { text: config.valorReferencialCadterc },
            { text: config.validacao },


        ]);
        debugger;
    }
    config.porCentagem = porCentagem.toFixed(2).replace(".", ",").replace(/(\d)(?=(\d{3})+(?!\d))/g, "$1.");
            ...
            header: [
               ...
               },
               {
                   ...
                   stack: [
                       ...
                       {
                           text: "QUADRO COMPARATIVO DE PREÇOS - REFERENCIAL DO CADTERC"
                       }
                   ],
               },
                {
            ...
            content: [
                ...
                },
                {
                    ...
                    stack: [
                          ...
                          {
                              text: [
                                  { text: "Valor total do contrato considerando os valores referenciais do CADTERC: R$ " },
                                  { text: config.somaValorReferencialCadterc },
                              ]
                          ...
                          },
                          {
                              text: [
                                   ...
                                   { text: config.NrContrato + " :  R$ " + config.somaValorUnitarioContratado },
                              ]
                          ...
                          },
                           {
                               text: [
                                    ...
                                    { text: "Economia de R$ " },
                                    { text: config.economiaMensal + " (" + " " + config.porCentagem + "% " + ")" },
                               ]
                           },
                    ],
