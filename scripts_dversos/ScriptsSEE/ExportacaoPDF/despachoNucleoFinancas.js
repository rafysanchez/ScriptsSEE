function gerardespachoNucleoFinancas(data) {
    
    var config = {
        pageSize: "A4",
        pageOrientation: "portrait",
        title: "DespachoNucleoFinancas",
    };
    ...
    debugger;
    var valorAno = 0
    var VlParcelaPorExtenso = 0;
    var ano = 0;
    var anoAtual = new Date();
    ano = anoAtual.getFullYear();
    for (var i = 0; i < data[0].listValorTotalParcelasAno.length; i++) {
        config.NrAno = data[0].listValorTotalParcelasAno[i].NrAno;
        if ( ano == config.NrAno)
        {
            valorAno = data[0].listValorTotalParcelasAno[i].VlParcela;
            VlParcelaPorExtenso = data[0].listValorTotalParcelasAno[i].VlParcelaPorExtenso;
        }
    }

    config.valorAno = valorAno != undefined && valorAno != 0 ? valorAno.replace(".", ",").replace(/(\d)(?=(\d{3})+(?!\d))/g, "$1.") : 0;
    config.VlParcelaPorExtenso = VlParcelaPorExtenso;
    debugger;
        ...
        var doc = {
           ...
           },
           {
               ...
               stack: [
                   ...
                   {
                       text: "DESPACHO DO NÚCLEO DE FINANÇAS"
                   }
               ],
           }
            ],
            content:[
                 {
                 ...
                 },
                  {
                      ...
                      stack: [
                                {
                                    text: [
                                    { text: "Tendo em vista a autorização do Ordenador de Despesa às fls. " },
                                    { text: config.NrFolhaDespachoGestor },
                                    { text: ", com vistas à Prorrogação" }
                                    ],
                                }
                      ]
                  },
                       ...
                       stack: [
                                 {
                                     ...
                                     { text: ", firmado com a empresa " },
                                     { text: config.NmCpfCgcCredor },
                                     { text: ", que tem como objeto a prestação de serviços de " },
                                     { text: config.descricaoObjeto },
                                     { text: ", destinado ao atendimento das necessidades das Unidades Escolares jurisdicionadas a esta Diretoria de Ensino" },
                                     { text: ", informo que providenciaremos oportunamente a Reserva de Recursos no valor de R$ " },
                                     { text: config.valorAno },
                                     { text: " (" },
                                     { text: config.VlParcelaPorExtenso },
                                     { text: " ), para o presente exercício orçamentário." },
