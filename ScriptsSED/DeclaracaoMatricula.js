utils = {
    ...
    CarregarCombos: function () {
        ...
        $("#CodigoTipoEnsino").change(function () { $("#TextoTipoEnsino").val($(this).find("option:selected").text()); });
    ...
    CarregarAluno: function (linha) {
        ...
        if (linha) {
            ...
            var parametrosController = {
                //codigoTipoEnsino: 14, //parseInt($("#CodigoTipoEnsino").val()),
                //codigoTurma: 34996343, //parseInt($("#CodigoTurma").val()),
                //codigoDiretoria: 20901, //!$("#CodigoDiretoria").val() ? 0 : $("#CodigoDiretoria").val(),
                //codigoEscola: 31264, // parseInt($("#CodigoEscola").val()),
                //anoLetivo: 2017, //$("#txtAnoLetivo").val(),
        ...
        utils.ajax("/DeclaracaoMatricula/CarregarAluno",
...
var ex = {
    ...
    sucesso: function (msg, fcb) {
        if (fcb) {
            Mensagem.Alert({
                ...
                tipo: "Sucesso",
                botao: "Fechar",
                callback: function () { fcb(); $.unblockUI(); }
            });
        }
        else {
            Mensagem.Alert({
    ...
    dialog: function (cs, data, title, width, fnclose, fnsave, btn) {
        ...
        if (!btn) {
            btn = {
                "Salvar": function () { fnsave(); }
            };
        };

        var opt = {
            modal: true, width: width, title: title,
            close: function (event, ui) { fnclose(); },
            buttons: btn
        };
        $("." + cs).dialog(opt);
    },
...
GerarPDF = {
    ...
    gerar: function (CodigoMatriculaAluno, CodigoTurma) {
        var NomeTurma = $("#TextoTurma").val();
        var NomeEnsino = $("#TextoTipoEnsino").val();
        var CodigoEscola = $("#CodigoEscola").val();

        ex.ajax("POST", "/DeclaracaoMatricula/BuscaDadosAluno", { CodigoAluno: CodigoMatriculaAluno, CodigoTurma: CodigoTurma, NomeTurma: NomeTurma, NomeEnsino: NomeEnsino, CodigoEscola: CodigoEscola },
            function (data, textStatus, jqXHR) {
                GerarPDF.montar(data);
            });
    },
    ...
    montar: function (data) {
        ...
        var config = {
            ...
            pageSize: "A4",
            pageMargins: [8, 8, 8, 8],
            title: "Declaracao_Matricula",
        };

        var nomeMunicipio = $("#CodigoDiretoria option:selected").text();
        ...
        if (config.data.SituacaoAluno) {
            config.Texto = { text: config.data.Nome + ', RA: ' + config.data.RA + ', RG/RNE: ' + config.data.DOC + ', nascido(a) em ' + config.data.DataNasc + ', esteve regularmente matriculado no(a) ' + config.data.NomeTurma + ', do ' + config.data.NomeEnsino + ', horário das ' + config.data.Periodo + ', neste estabelecimento entre ' + config.data.DtInicioMatricula + ' e ' + config.data.DtFimMatricula, alignment: "left", margin: [30, 2, 20, 0] }
        }
        else {
            config.Texto = { text: config.data.Nome + ', RA: ' + config.data.RA + ', RG/RNE: ' + config.data.DOC + ', nascido(a) em ' + config.data.DataNasc + ', está regularmente matriculado no(a) ' + config.data.NomeTurma + ', do ' + config.data.NomeEnsino + ', horário das ' + config.data.Periodo + ', neste estabelecimento. ', alignment: "left", margin: [30, 2, 20, 0] }
        }
        ...
        { text: " " },
        { text: " " },
    	{ text: 'DECLARAÇÃO DE MATRÍCULA', style: 'h6', alignment: "center", bold: true },
        { text: " " },
        { text: " " },
        { text: " " },
        { text: 'Declaramos para os devidos fins que ', alignment: "right", margin: [0, 0, 90, 0] },
        config.Texto
            ];
            content.push({ text: " " });
            content.push({ text: " " });
            ...
            content.push({ text: config.data.DsMunicipio + ", " + config.dia + " de " + config.mes + " de " + config.ano, margin: [30, 0, 20, 0] });
            ...
            //Assinatura
            content.push({ text: "________________________________________________ ", style: "assinatura", margin: [30, 0, 20, 0] });
            content.push({ text: config.data.NomeDiretor, style: "assinaturatexto", margin: [30, 0, 20, 0] });
            content.push({ text: config.data.RgDiretor, margin: [30, 0, 20, 0], fontSize: 8 });
