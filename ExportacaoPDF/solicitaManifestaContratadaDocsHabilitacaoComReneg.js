function gerarsolicitaManifestaContratadaDocsHabilitacaoComReneg(data) {

    debugger;
    var config = {
        pageSize: "A4",
        pageOrientation: "portrait",
        // pageOrientation: 'landscape',
        title: "SolicitaManifestaContratadaDocsHabilitacaoComReneg",
    };

    sedPdfExporter.normalizeConfig(config);

    config.codigoUnidadeGestora = data[0].unidadeGestora.codigoUnidadeGestora != undefined ? data[0].unidadeGestora.codigoUnidadeGestora : "";
    config.nomeUnidadeGestora = data[0].unidadeGestora.nomeUnidadeGestora != undefined ? data[0].unidadeGestora.nomeUnidadeGestora : "";
    config.NmUnidGest = data[0].unidadeGestora.codigoUnidadeGestora != undefined ? data[0].unidadeGestora.codigoUnidadeGestora : "";
    config.descricaoMunicipio = data[0].unidadeGestora.descricaoMunicipio != undefined && data[0].unidadeGestora.descricaoMunicipio != "" ? titleize(data[0].unidadeGestora.descricaoMunicipio.toLowerCase()) : "São Paulo";
    config.descricaoEnderecoNumero = data[0].unidadeGestora.descricaoEnderecoNumero != undefined ? data[0].unidadeGestora.descricaoEnderecoNumero : "";
    config.numeroProcesso = data[0].processo.numeroProcesso != undefined ? data[0].processo.numeroProcesso : "";
    config.NrContrato = data[0].numeroContrato != undefined ? data[0].numeroContrato : "";
    config.NmCpfCgcCredor = data[0].nomeCredor != undefined ? data[0].nomeCredor : "";
    config.NrCpfCgcCredor = data[0].numeroCpfCnpj != undefined ? data[0].numeroCpfCnpj.replace(/^(\d{2})(\d{3})(\d{3})(\d{4})(\d{2})/, "$1.$2.$3/$4-$5") : "";
    config.dsAssunto = data[0].assuntoPlanejado.descricaoAssuntoPlanejado != undefined ? data[0].assuntoPlanejado.descricaoAssuntoPlanejado : "";
    config.DsClausulaContrato = data[0].clausulaContrato != undefined ? data[0].clausulaContrato : "";
    config.DtReajustemes = data[0].dataBaseReajuste != undefined ? dataAtualFormatadames(data[0].dataBaseReajuste) : "";
    config.DtReajusteano = data[0].dataBaseReajuste != undefined ? dataAtualFormatadaano(data[0].dataBaseReajuste) : "";
    config.DtReajuste = data[0].dataBaseReajuste != undefined ? dataAtualFormatada(data[0].dataBaseReajuste) : "";
    config.NrPercentual = data[0].percentualReajuste != undefined ? data[0].percentualReajuste + '%' : "";
    config.DsFolhaCadTerc = data[0].folhaCadTerc != undefined ? data[0].folhaCadTerc : "";
    config.IdPeriodoContrato = data[0].idPeriodoContrato != undefined ? data[0].idPeriodoContrato : "";
    config.DsUnidadeContratada = data[0].descricaoUnidadeContratada != undefined ? data[0].descricaoUnidadeContratada : "";
    config.VLReajuste = data[0].valorReajuste != undefined ? data[0].valorReajuste.toFixed(2).replace(".", ",").replace(/(\d)(?=(\d{3})+(?!\d))/g, "$1.") : "";
    config.VLReajustado = data[0].valorReajustado != undefined ? data[0].valorReajustado.toFixed(2).replace(".", ",").replace(/(\d)(?=(\d{3})+(?!\d))/g, "$1.") : "";
    config.descricaoObjeto = data[0].descricaoObjeto != undefined ? data[0].descricaoObjeto : "";
    config.dataVigenciaIni = data[0].dataVigenciaIni != undefined ? dataAtualFormatada(data[0].dataVigenciaIni) : "";
    config.dataVigenciaFim = data[0].dataVigenciaFim != undefined ? dataAtualFormatada(data[0].dataVigenciaFim) : "";
    config.nomeGestor = data[0].nomeGestor != undefined ? titleize(data[0].nomeGestor) : "";
    config.dataImpressao = data[0].dataImpressao != undefined ? dataAtualFormatada(data[0].dataImpressao) : "";
    config.DtImpressaodia = data[0].dataImpressao != undefined ? dataAtualFormatadadia(data[0].dataImpressao) : "";
    config.DtImpressaomes = data[0].dataImpressao != undefined ? dataAtualFormatadames(data[0].dataImpressao) : "";
    config.DtImpressaonomemes = data[0].dataImpressao != undefined ? dataAtualFormatadanomemes(data[0].dataImpressao) : "";
    config.DtImpressaoano = data[0].dataImpressao != undefined ? dataAtualFormatadaano(data[0].dataImpressao) : "";
    config.nrFolha = data[0].nrFolha != undefined ? data[0].nrFolha : "";
    config.nomeResponsavel = data[0].usuarioResponsavel.nmResponsavel != undefined ? titleize(data[0].usuarioResponsavel.nmResponsavel) : "";
    config.ordenadorDespesa = data[0].ordenadorDespesa.nmResponsavel != undefined ? titleize(data[0].ordenadorDespesa.nmResponsavel) : "";
    config.periodoPorExtenso = data[0].periodoPorExtenso != undefined ? data[0].periodoPorExtenso : "";
    config.tipoPeriodo = data[0].tipoPeriodo != undefined ? data[0].tipoPeriodo : "";
    config.valorReferencial = data[0].valorReferencial != undefined ? data[0].valorReferencial : "";


    config.valorMediaTotalEmpresa = data[0].valorMediaTotalEmpresa != undefined ? data[0].valorMediaTotalEmpresa.toFixed(2).replace(".", ",").replace(/(\d)(?=(\d{3})+(?!\d))/g, "$1.") : "";
    config.descricaoMediaTotalEmpresa = data[0].descricaoMediaTotalEmpresa != undefined ? data[0].descricaoMediaTotalEmpresa : "";

    for (var i = 0; i < data[0].listItensNotaContratoDemonstrativo.length; i++) {

        config.somaValorReferencialCadterc = parseFloat(data[0].itemNotaContratoDemonstrativo.somaValorUnitarioReferencialCadterc).toFixed(2).replace(".", ",").replace(/(\d)(?=(\d{3})+(?!\d))/g, "$1.");
        config.descricaoValorReferencialCadterc = data[0].itemNotaContratoDemonstrativo.descricaoValorUnitarioReferencialCadterc;
        break;
    }


    if (config.valorReferencial != 0 && config.valorReferencial != "0,00" && config.valorReferencial != undefined) {
        config.textReferencial = "os valores referenciais do CADTERC";
        config.textValorReferencial = "R$ " + config.somaValorReferencialCadterc + " ( " + config.descricaoValorReferencialCadterc + " )";
    }
    else {
        config.textReferencial = "as pesquisas de preço com empresas do ramo";
        config.textValorReferencial = "R$ " + config.valorMediaTotalEmpresa + " ( " + config.descricaoMediaTotalEmpresa + " )";
       
    }
    debugger;

    if (config.DsUnidadeContratada == "2") {
        config.DsUnidadeContratada = "dia(s)";
        config.tipoPeriodo = "diária";
    }
    else if (config.DsUnidadeContratada == "1") {
        config.DsUnidadeContratada = "mes(es)";
        config.tipoPeriodo = "mensal";
    }

    if (config.codigoUnidadeGestora == "080101" || config.codigoUnidadeGestora == "080102" ||
    config.codigoUnidadeGestora == "080011" || config.codigoUnidadeGestora == "080104" ||
    config.codigoUnidadeGestora == "080259" || config.codigoUnidadeGestora == "080353" ||
    config.codigoUnidadeGestora == "080356" || config.codigoUnidadeGestora == "080357" ||
    config.codigoUnidadeGestora == "080358" || config.codigoUnidadeGestora == "080359" ||
    config.codigoUnidadeGestora == "080040" || config.codigoUnidadeGestora == "080360"
    ) {

        config.descricaoEnderecoNumero = "Endereço SEE : Praça da República, 53 - República, São Paulo/SP";
    }

    config.docGenerator = function (config) {
        var doc = {
          
            pageMargins: [30, 150, 50, 30],
            header: [
           {
               margin: [50, 20, 50, 0],
               table: {
                   widths: ["5%", "95%"],
                   body: [
                       [
                           {
                               stack: [
                                   {
                                       image: 'data:image/png;base64,',
                                       width: 65,
                                       height: 75
                                   }
                               ]
                           },
                           {
                               stack: [
                                   {
                                       text: 'GOVERNO DO ESTADO DE SÃO PAULO',
                                       fontSize: 15,
                                       bold: true
                                   },
                                   {
                                       text: 'SECRETARIA DE ESTADO DA EDUCAÇÃO',
                                       fontSize: 14,
                                       bold: true
                                   },
                                   {
                                       text: config.nomeUnidadeGestora,
                                       fontSize: 11,
                                       bold: false
                                   },

                               ],
                               alignment: "center",
                           }
                       ]
                   ]
               },
               layout: "noBorders"
           },
           {
               alignment: "center",
               stack: [

                   {
                       text: "SOLICITA MANIFESTAÇÃO DA CONTRATADA E DOCUMENTOS DE HABILITAÇÃO"
                   }
               ],
           }
            ],




            content: [
                    {
                
                        margin: [20, 10, 50, 0],
                        stack: [
                             {
                                 fontSize: 10,
                                 text:
                                  [
                
                                     { text: "PROCESSO Nº : ", bold: true },
                                     { text: config.numeroProcesso, bold: false },
                
                                  ],
                             },
                
                             {
                                 fontSize: 10,
                                 text:
                                  [
                
                                     { text: "CONTRATO Nº : ", bold: true },
                                     { text: config.NrContrato, bold: false },
                
                                  ],
                             },
                
                             {
                                 fontSize: 10,
                                 text:
                                     [
                
                                 { text: "ASSUNTO PLANEJADO : ", bold: true },
                                 { text: config.descricaoObjeto, bold: false },
                
                                     ],
                             },
                             {
                                 fontSize: 10,
                                 margin: [0, 10, 0, 0],
                                 text:
                                     [
                
                                 { text: "ASSUNTO : ", bold: true },
                                 { text: "Renegociação de Preços e Prorrogação de vigência contratual", bold: false },
                
                                     ],
                             }
                
                        ]
                
                    },
                    {
                        margin: [20, 30, 50, 0],
                        fontSize: 10,
                        stack: [
                                { text: "Prezado Senhor," },
                        ]
                
                    },
                     {
                         margin: [40, 10, 50, 0],
                         fontSize: 10,
                         alignment: 'justify',
                         stack: [
                                   {
                                       text: [
                                       { text: "A Diretoria de Ensino Região de " },
                                       { text: config.nomeUnidadeGestora },
                                       { text: ", representada pelo(a) DD.Dirigente" }
                                       ],
                                   }
                         ]
                     },
                      {
                          margin: [20, 0, 50, 0],
                          fontSize: 10,
                          alignment: 'justify',
                          stack: [
                                    {
                                        text: [
                                        { text: " Regional de Ensino, vem por intermédio deste, informar o término da  vigência do Contrato nº " },
                                        { text: config.NrContrato },
                                        { text: ", referente à Prestação de serviços de " },
                                        { text: config.descricaoObjeto },
                                        { text: ", em " },
                                        { text: config.dataVigenciaFim },
                                        { text: ", podendo ser prorrogado por iguais e sucessivos períodos até o limite de 60 (sessenta) meses, conforme disposto no artigo 57," },
                                        { text: " inciso II da Lei Federal 8.666/93, desde que as partes se manifestem com antecedência de 90 (noventa) dias do término do prazo" },
                                        { text: " e desde que os preços praticados estejam compatíveis com o mercado." },
                                        ],
                                    }
                          ]
                      },
                      {
                          margin: [40, 0, 50, 0],
                          fontSize: 10,
                          alignment: 'justify',
                          stack: [
                                    {
                                        text: [
                                        { text: "Ocorre que " },
                                        { text: config.textReferencial },
                                        { text: " demonstraram valor médio inferior" },
                                       
                                        ],
                                    }
                          ]
                      },
                      {
                          margin: [20, 0, 50, 0],
                          fontSize: 10,
                          alignment: 'justify',
                          stack: [
                                    {
                                        text: [
                                        { text: " ao praticado atualmente no contrato, ficando a média " },
                                        { text: config.tipoPeriodo },
                                        { text: " estimada no montante de " },
                                        { text: config.textValorReferencial },
                                        { text: ", tornando a prorrogação da vigência não vantajosa para a Administração, a menos que haja a renegociação de preço." },
                                        ],
                                    }
                          ]
                      },
                       {
                           margin: [40, 0, 50, 0],
                           fontSize: 10,
                           alignment: 'justify',
                           stack: [
                                     {
                                         text: [
                                         { text: "Assim sendo, solicitamos a Vossa Senhoria que se manifeste no prazo de 05 (cinco) dias a" },
                                         ],
                                     }
                           ]
                       },
                      {
                          margin: [20, 0, 50, 0],
                          fontSize: 10,
                          alignment: 'justify',
                          stack: [
                                    {
                                        text: [
                                        { text: " contar do recebimento deste, se há interesse na renegociação do preço atualmente praticado para adequação aos valores de mercado," },
                                        { text: " condição para a prorrogação da vigência do contrato supracitado por mais um período de " },
                                        { text: config.IdPeriodoContrato },
                                        { text: " ( " },
                                        { text: config.periodoPorExtenso },
                                        { text: " ) " },
                                        { text: config.DsUnidadeContratada },
                                        { text: ", bem como encaminhe toda a documentação comprobatória das condições de habilitação, conforme item IV-Da Habilitação, do Edital do Pregão Eletrônico." },
                                        ],
                                    }
                          ]
                      },
                       {
                           margin: [40, 0, 50, 0],
                           fontSize: 10,
                           alignment: 'justify',
                           stack: [
                                     {
                                         text: [
                                         { text: "Sem mais para o momento, e no aguardo de vossa manifestação, antecipadamente " },
                                         ],
                                     }
                           ]
                       },
                       {
                           margin: [20, 0, 50, 0],
                           fontSize: 10,
                           alignment: 'justify',
                           stack: [
                                     {
                                         text: [
                                         { text: "agradecemos." },
                                         ],
                                     }
                           ]
                       },
                           {
                               margin: [20, 40, 50, 0],
                               fontSize: 10,
                               alignment: "right",
                               stack: [
                                   {
                                       margin: [20, 0, 0, 0],
                                       text:
                                           [
                                                 { text: config.descricaoMunicipio + ", " },
                                                 { text: config.DtImpressaodia + " de " + config.DtImpressaonomemes + " de " + config.DtImpressaoano },
                                           ],
                                   }

                               ],
                           },
                      {
                          margin: [20, 30, 50, 0],
                          fontSize: 10,
                          alignment: "center",
                          stack: [
                              { text: "_______________________________________________" },
                              { text: config.nomeGestor },
                              { text: "Gestor do Contarto" },
                          ],

                      },
                  {
                    margin: [20, 40, 50, 0],
                    fontSize: 10,
                    alignment: "center",
                    stack: [
                        { text: "_______________________________________________" },
                        { text: config.ordenadorDespesa },
                        { text: "Ordenador de Despesa" },
                    ],

                },
                 {
                     margin: [20, 40, 50, 0],
                     fontSize: 10,
                     alignment: 'justify',
                     stack: [
                               {
                                   text: [
                                   { text: "A empresa " },
                                   { text: config.NmCpfCgcCredor },
                                   ],
                               }
                     ]
                 },
                     



            ],
             
            
            footer: function (currentPage, pageCount) {
                return {
                    margin: [50, 0, 50, 0],
                    columns: [
                        {
                            table: {
                                widths: [100, 100, 230],
                                headerRows: 1,
                                fontSize: 9,
                                body: [
                                         [{ text: "Endereço UGE : " + config.descricaoEnderecoNumero + "                             pagina(s) " + currentPage.toString() + " de " + pageCount, colSpan: 3, fontSize: 9, border: [false, true, false, false], bold: false, color: 'gray' }],

                                ]
                            },
                        },
                    ]
                };

            },
            styles: {
                destaque: {
                    margin: [20, 0, 40, 0],
                    fontSize: 13,
                    bold: true
                },
                simples: {
                    fontSize: 13,
                    bold: false,
                },
                tabela: {
                    margin: [20, 0, 40, 0], color: 'gray'
                },
                destaque_texto: {
                    fontSize: 12,
                    bold: true,
                    alignment: 'justify',
                },
                simples_texto: {
                    margin: [20, 0, 40, 0],
                    fontSize: 12,
                    bold: false,
                    alignment: 'justify',
                },
                header: {
                    fontSize: 10,
                    bold: false,
                    margin: [10, 0, 30, 1]
                }
            },
        }
        return doc;
    };
    sedPdfExporter.exportPdf(config);
}