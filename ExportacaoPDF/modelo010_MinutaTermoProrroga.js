function gerarmodelo010_MinutaTermoProrroga_2(data) {
    
    var config = {
        pageSize: "A4",
        pageOrientation: "portrait",
        // pageOrientation: 'landscape',
        title: "MinutaTermoProrroga",
    };



    sedPdfExporter.normalizeConfig(config);

    config.codigoUnidadeGestora = data[0].unidadeGestora.codigoUnidadeGestora != undefined ? data[0].unidadeGestora.codigoUnidadeGestora : "";
      config.nomeUnidadeGestora = data[0].unidadeGestora.nomeUnidadeGestora != undefined ? data[0].unidadeGestora.nomeUnidadeGestora : "";
      config.NmUnidGest = data[0].unidadeGestora.codigoUnidadeGestora != undefined ? data[0].unidadeGestora.codigoUnidadeGestora : "";     
      config.cnpjUGE = data[0].unidadeGestora.cnpjUGE != undefined ? data[0].unidadeGestora.cnpjUGE.replace(/^(\d{2})(\d{3})(\d{3})(\d{4})(\d{2})/, "$1.$2.$3/$4-$5") : "";
    config.numeroProcesso = data[0].processo.numeroProcesso != undefined ? data[0].processo.numeroProcesso : "";
    config.NrContrato = data[0].numeroContrato != undefined ? data[0].numeroContrato : "";
    config.NmCpfCgcCredor = data[0].nomeCredor != undefined ? data[0].nomeCredor : "";
    config.NrCpfCgcCredor = data[0].numeroCpfCnpj != undefined ? data[0].numeroCpfCnpj.replace(/^(\d{2})(\d{3})(\d{3})(\d{4})(\d{2})/, "$1.$2.$3/$4-$5") : "";
    config.dsAssunto = data[0].assuntoPlanejado.descricaoAssuntoPlanejado != undefined ? data[0].assuntoPlanejado.descricaoAssuntoPlanejado : "";
    config.DsClausulaContrato = data[0].clausulaContrato != undefined ? data[0].clausulaContrato : "";
    config.DtReajustemes = data[0].dataBaseReajuste != undefined ? dataAtualFormatadames(data[0].dataBaseReajuste) : "";
    config.DtReajusteano = data[0].dataBaseReajuste != undefined ? dataAtualFormatadaano(data[0].dataBaseReajuste) : "";
    config.DtReajuste = data[0].dataBaseReajuste != undefined ? dataAtualFormatada(data[0].dataBaseReajuste) : "";
    config.NrPercentual = data[0].percentualReajuste != undefined ? data[0].percentualReajuste + '%' : "";
    config.DsFolhaCadTerc = data[0].folhaCadTerc != undefined ? data[0].folhaCadTerc : "";
    config.IdPeriodoContrato = data[0].idPeriodoContrato != undefined ? data[0].idPeriodoContrato : "";
    config.periodoPorExtenso = data[0].periodoPorExtenso != undefined ? data[0].periodoPorExtenso : "";
    config.DsUnidadeContratada = data[0].descricaoUnidadeContratada != undefined ? data[0].descricaoUnidadeContratada : "";
    config.VLReajuste = data[0].valorReajuste != undefined ? data[0].valorReajuste.toFixed(2).replace(".", ",").replace(/(\d)(?=(\d{3})+(?!\d))/g, "$1.") : "";
    config.VLReajustado = data[0].valorReajustado != undefined ? data[0].valorReajustado.toFixed(2).replace(".", ",").replace(/(\d)(?=(\d{3})+(?!\d))/g, "$1.") : "";
    config.descricaoObjeto = data[0].descricaoObjeto != undefined ? data[0].descricaoObjeto : "";
    config.dataVigenciaIni = data[0].dataVigenciaIni != undefined ? dataAtualFormatada(data[0].dataVigenciaIni) : "";
    config.dataVigenciaFim = data[0].dataVigenciaFim != undefined ? dataAtualFormatada(data[0].dataVigenciaFim) : "";
    config.nomeGestor = data[0].nomeGestor != undefined ? titleize(data[0].nomeGestor) : "";   
    config.dataInclusao = data[0].dataInclusao != undefined ? dataAtualFormatada(data[0].dataInclusao) : "";
    config.DtInclusaodia = data[0].dataInclusao != undefined ? dataAtualFormatadadia(data[0].dataInclusao) : "";
    config.DtInclusaomes = data[0].dataInclusao != undefined ? dataAtualFormatadames(data[0].dataInclusao) : "";
    config.DtInclusaonomemes = data[0].dataInclusao != undefined ? dataAtualFormatadanomemes(data[0].dataInclusao) : "";
    config.DtInclusaoano = data[0].dataInclusao != undefined ? dataAtualFormatadaano(data[0].dataInclusao) : "";
    config.TipoLogradouro = data[0].TipoLogradouro != undefined ? data[0].TipoLogradouro : "";
    config.DsEndereco = data[0].DsEndereco != undefined ? data[0].DsEndereco : "";
    config.NumeroEnder = data[0].NumeroEnder != undefined ? data[0].NumeroEnder : "";
    config.Bairro = data[0].Bairro != undefined ? data[0].Bairro : "";
    config.Complemento = data[0].Complemento != undefined ? data[0].Complemento : "";
    config.EnderecoCompleto = data[0].EnderecoUGE != undefined ? data[0].EnderecoUGE : "";
    config.valorTotalContrato = data[0].valorTotalContrato != undefined ? data[0].valorTotalContrato.toFixed(2).replace(".", ",").replace(/(\d)(?=(\d{3})+(?!\d))/g, "$1.") : "";
    config.valorPorExtenso = data[0].valorPorExtenso != undefined ? data[0].valorPorExtenso : "";


    config.dataVigenciaInidia = data[0].dataVigenciaIni != undefined ? dataAtualFormatadadia(data[0].dataVigenciaIni) : "";
    config.dataVigenciaInimes = data[0].dataVigenciaIni != undefined ? dataAtualFormatadames(data[0].dataVigenciaIni) : "";
    config.dataVigenciaIniano = data[0].dataVigenciaIni != undefined ? dataAtualFormatadaano(data[0].dataVigenciaIni) : "";


    config.dataImpressao = data[0].dataImpressao != undefined ? dataAtualFormatada(data[0].dataImpressao) : "";
    config.DtImpressaodia = data[0].dataImpressao != undefined ? dataAtualFormatadadia(data[0].dataImpressao) : "";
    config.DtImpressaomes = data[0].dataImpressao != undefined ? dataAtualFormatadames(data[0].dataImpressao) : "";
    config.DtImpressaonomemes = data[0].dataImpressao != undefined ? dataAtualFormatadanomemes(data[0].dataImpressao) : "";
    config.DtImpressaoano = data[0].dataImpressao != undefined ? dataAtualFormatadaano(data[0].dataImpressao) : "";
    config.nrFolha = data[0].nrFolha != undefined ? data[0].nrFolha : "";
    config.NrFolhaDespachoGestor = data[0].NrFolhaDespachoGestor != undefined ? data[0].NrFolhaDespachoGestor : "";
    config.NrFolhaDecAutoridadeCompetente = data[0].NrFolhaDecAutoridadeCompetente != undefined ? data[0].NrFolhaDecAutoridadeCompetente : "";
    
    config.nmCargo = data[0].usuarioResponsavel.nmCargo != undefined ? titleize(data[0].usuarioResponsavel.nmCargo) : "";
    config.descricaoMunicipio = data[0].unidadeGestora.descricaoMunicipio != undefined && data[0].unidadeGestora.descricaoMunicipio != "" ? titleize(data[0].unidadeGestora.descricaoMunicipio.toLowerCase()) : "São Paulo";
    config.descricaoEnderecoNumero = data[0].unidadeGestora.descricaoEnderecoNumero != undefined ? data[0].unidadeGestora.descricaoEnderecoNumero : "";
    config.dataAssinaturaContratoOriginario = data[0].dataAssinaturaContratoOriginario != undefined ? dataAtualFormatada(data[0].dataAssinaturaContratoOriginario) : "";
    config.clausulaVPContratoOriginario = data[0].clausulaVPContratoOriginario != undefined ? data[0].clausulaVPContratoOriginario.trim() : "";
    config.nomeResponsavel = data[0].usuarioResponsavel.nmResponsavel != undefined ? titleize(data[0].usuarioResponsavel.nmResponsavel) : "";
    config.ordenadorDespesa = data[0].ordenadorDespesa.nmResponsavel != undefined ? titleize(data[0].ordenadorDespesa.nmResponsavel) : "";
    config.nmCargoOrdenadorDespesa = data[0].ordenadorDespesa.nmCargo != undefined ? titleize(data[0].ordenadorDespesa.nmCargo) : "";

    config.decricaoResponsavelEmpresa = data[0].decricaoResponsavelEmpresa != undefined ? data[0].decricaoResponsavelEmpresa : "";
    config.numeroRGResponsavelEmpresa = data[0].numeroRGResponsavelEmpresa != undefined ? data[0].numeroRGResponsavelEmpresa : "";
    config.numeroCPFResponsavelEmpresa = data[0].numeroCPFResponsavelEmpresa != undefined ? data[0].numeroCPFResponsavelEmpresa : "";
    config.decricaoTermoAditamento = data[0].decricaoTermoAditamento != undefined ? titleize(data[0].decricaoTermoAditamento) : "";

    if (config.decricaoTermoAditamento == "")
    {
        config.Termo = "Termo";
    }
    else {
        config.Termo = config.decricaoTermoAditamento +  " termo";
    }
    


   
    var body = [];
    debugger;
    for (var i = 0; i < data[0].listValorTotalParcelasAno.length; i++) {
        
        var ano = data[0].listValorTotalParcelasAno[i].NrAno;
        var valorAno = data[0].listValorTotalParcelasAno[i].VlParcela.replace(".", ",").replace(/(\d)(?=(\d{3})+(?!\d))/g, "$1.");
     
        body.push([
                                          { text: "Valor para o Exercicio " + ano + "\n", fontSize: 10},
                                          { text: valorAno, fontSize: 10 },
                                      

        ]);
    }

    if (config.codigoUnidadeGestora == "080101" || config.codigoUnidadeGestora == "080102" ||
    config.codigoUnidadeGestora == "080011" || config.codigoUnidadeGestora == "080104" ||
    config.codigoUnidadeGestora == "080259" || config.codigoUnidadeGestora == "080353" ||
    config.codigoUnidadeGestora == "080356" || config.codigoUnidadeGestora == "080357" ||
    config.codigoUnidadeGestora == "080358" || config.codigoUnidadeGestora == "080359" ||
    config.codigoUnidadeGestora == "080040" || config.codigoUnidadeGestora == "080360"
    ) {

        config.descricaoEnderecoNumero = "Endereço SEE : Praça da República, 53 - República, São Paulo/SP";
    }

    config.body = body;

    config.docGenerator = function (config) {
        var doc2 = {

            pageMargins: [30, 190, 50, 30],
            header: [
           {
               margin: [50, 20, 50, 0],
               table: {
                   widths: ["5%", "95%"],
                   body: [
                       [
                           {
                               stack: [
                                   {
                                       image: 'data:image/png;base64,',
                                       width: 65,
                                       height: 75
                                   }
                               ]
                           },
                           {
                               stack: [
                                   {
                                       text: 'GOVERNO DO ESTADO DE SÃO PAULO',
                                       fontSize: 15,
                                       bold: true
                                   },
                                   {
                                       text: 'SECRETARIA DE ESTADO DA EDUCAÇÃO',
                                       fontSize: 14,
                                       bold: true
                                   },
                                   {
                                       text: config.nomeUnidadeGestora,
                                       fontSize: 11,
                                       bold: false
                                   },

                               ],
                               alignment: "center",
                           }
                       ]
                   ]
               },
               layout: "noBorders"
           },
           {
               alignment: "center",
               margin: [0, 15, 50, 0],
               stack: [

                   {
                       text: "MINUTA DE TERMO DE PRORROGAÇÃO DE CONTRATOS DE SERVIÇOS CONTÍNUOS"
                   }
               ],
           },
          
            ],
            content: [
                 {
                     margin: [20, 0, 40, 30],
                     stack: [
                                 {
                                     fontSize: 10,
                                     text:
                                      [
                                         { text: "PROCESSO Nº : ", bold: true },
                                         { text: config.numeroProcesso, bold: false },
                                      ],
                                 },
                                 {
                                     fontSize: 10,
                                     text:
                                      [
                                         { text: "CONTRATO Nº : ", bold: true },
                                         { text: config.NrContrato, bold: false },
                                      ],
                                 },
                     ]
                 },
                 {                   
                     margin: [160, 0, 50, 0],
                     fontSize: 10,
                     alignment: 'justify',
                     stack: [
                               {
                                   text: [
                                   { text: config.Termo },
                                   { text: " de aditamento ao contrato n° " },
                                   { text: config.NrContrato },
                                   { text: ", firmado entre" }
                                   ],
                               }
                     ]
                 },
                 {
                     margin: [150, 0, 50, 0],
                     fontSize: 10,
                     alignment: 'justify',
                     stack: [
                               {
                                   text: [
                                   { text: " o Estado de São Paulo, por intermédio da Secretaria do Estado da Educação, pela Diretoria de Ensino Região" },
                                   { text: config.nomeUnidadeGestora },
                                   { text: " e a empresa " },
                                   { text: config.NmCpfCgcCredor },
                                   { text: ", objetivando a prestação de serviços de " },
                                   { text: config.descricaoObjeto },
                                   ],
                               }
                     ]
                 },
                  {
                      margin: [40, 10, 50, 0],
                      fontSize: 10,
                      alignment: 'justify',
                      stack: [
                                {
                                    text: [
                                    { text: "Aos " },
                                    { text: config.dataVigenciaInidia },
                                    { text: " dias do mês de " },
                                    { text: config.dataVigenciaInimes },
                                    { text: " do ano de " },
                                    { text: config.dataVigenciaIniano },
                                    { text: ", na cidade de " },
                                    { text: config.descricaoMunicipio },
                                    { text: ", compareceram, de " },
                                    ],
                                }
                      ]
                  },
                  {
                      margin: [20, 0, 50, 0],
                      fontSize: 10,
                      alignment: 'justify',
                      stack: [
                                {
                                    text: [
                                    { text: "um lado, como Contratante o Estado de São Paulo por intermédio da, Secretaria do Estado da Educação pela Diretoria de Ensino Região de " },
                                    { text: config.nomeUnidadeGestora },
                                    { text: " inscrita no CNPJ/MF sob nº " },
                                    { text: config.cnpjUGE },
                                    { text: ", com sede na " },
                                    { text: config.descricaoEnderecoNumero },
                                    { text: ", neste ato representada pelo(a) Sr.(a) " },
                                    { text: config.ordenadorDespesa },
                                    { text: ", " },
                                    { text: config.nmCargoOrdenadorDespesa },
                                    { text: ", e, de outro lado, como Contratada a empresa" },
                                    { text: config.NmCpfCgcCredor },
                                    { text: ", inscrita no CNPJ/MF sob nº " },
                                    { text: config.NrCpfCgcCredor },
                                    { text: ", neste ato representada pelo(a) Sr.(a) " },
                                    { text: config.decricaoResponsavelEmpresa },
                                    { text: ", RG nº " },
                                    { text: config.numeroRGResponsavelEmpresa },
                                    { text: " e CPF nº " },
                                    { text: config.numeroCPFResponsavelEmpresa },
                                    ],
                                }
                      ]
                  },
                 {
                     margin: [40, 10, 50, 0],
                     fontSize: 10,
                     alignment: 'justify',
                     stack: [
                               {
                                   text: [
                                   { text: "As referidas partes, Considerando :" },                                   
                                   ],
                               }
                     ]
                 },
                 {
                     margin: [20, 10, 50, 0],
                     fontSize: 10,
                     alignment: 'justify',
                     stack: [
                               {
                                   text: [
                                   { text: "a) que em " },
                                   { text: config.dataAssinaturaContratoOriginario },
                                   { text: " foi celebrado o Contrato nº " },
                                   { text: config.NrContrato },
                                   { text: " tendo por objeto a prestação de serviços contínuos de " },
                                   { text: config.descricaoObjeto },
                                   { text: " ;" },
                                   ],
                               }
                     ]
                 },
                  {
                      margin: [20, 5, 50, 0],
                      fontSize: 10,
                      alignment: 'justify',
                      stack: [
                                {
                                    text: [
                                    { text: "b) que na Cláusula " },
                                    { text: config.clausulaVPContratoOriginario },
                                    { text: " do referido instrumento ficou estabelecida a vigência do ajuste por " },
                                    { text: config.IdPeriodoContrato },
                                    { text: " ( " },
                                    { text: config.periodoPorExtenso },
                                    { text: " ) meses, prorrogável até o limite de 60 (sessenta) meses ;" },
                                    ],
                                }
                      ]
                  },                
                  {
                      margin: [20, 5, 50, 0],
                      fontSize: 10,
                      alignment: 'justify',
                      stack: [
                                {
                                    text: [
                                    { text: "c) que a Contratada comprovou, perante o Contratante, que mantém as condições de habilitação e qualificação " },
                                    { text: "exigidas à época do certame, nos termos do artigo 55, inciso XIII, da Lei federal 8.666/1993 ;" },

                                    ],
                                }
                      ]
                  },
                   {
                       margin: [20, 5, 50, 0],
                       fontSize: 10,
                       alignment: 'justify',
                       stack: [
                                 {
                                     text: [
                                     { text: "d) que a prorrogação do contrato foi expressamente autorizada e justificada por escrito pela autoridade competente, conforme despacho exarado às fls. " },
                                     { text: config.NrFolhaDecAutoridadeCompetente },
                                     { text: " do Processo nº " },
                                     { text: config.numeroProcesso },
                                     { text: " ;" },

                                     ],
                                 }
                       ]
                   },
                    {
                        margin: [40, 10, 50, 0],
                        fontSize: 10,
                        alignment: 'justify',
                        stack: [
                                  {
                                      text: [
                                      { text: "Resolvem, de comum acordo, aditar o Contrato nº " },
                                      { text: config.NrContrato },
                                      { text: ", nos termos do artigo 57, inciso II, da" },                                   
                                      ],
                                  }
                        ]
                    },
                    {
                        margin: [20, 0, 50, 0],
                        fontSize: 10,
                        alignment: 'justify',
                        stack: [
                                  {
                                      text: [
                                      { text: " Lei federal 8.666/1993, o que ora fazem nos termos a seguir expostos :" },
                                      ],
                                  }
                        ]
                    },
                     {
                         margin: [20, 10, 50, 0],
                         fontSize: 10,
                         alignment: 'justify',
                         bold: true ,
                         stack: [
                                   {
                                       text: [
                                       { text: "Cláusula Primeira da Prorrogação :" },
                                       ],
                                   }
                         ]
                     },
                     {
                         margin: [40, 10, 50, 0],
                         fontSize: 10,
                         alignment: 'justify',                        
                         stack: [
                                   {
                                       text: [
                                       { text: "O prazo de vigência do contrato fica prorrogado por mais " },
                                       { text: config.IdPeriodoContrato },
                                       { text: " ( "},
                                       { text: config.periodoPorExtenso },
                                       { text: " ) meses, de " },
                                       { text: config.dataVigenciaIni },
                                       ],
                                   }
                         ]
                     },
                      {
                          margin: [20, 0, 50, 0],
                          fontSize: 10,
                          alignment: 'justify',
                          stack: [
                                    {
                                        text: [
                                        { text: " a " },
                                        { text: config.dataVigenciaFim },                                       
                                        ],
                                    }
                          ]
                      },
                        {
                            margin: [20, 10, 50, 0],
                            fontSize: 10,
                            alignment: 'justify',
                            bold: true,
                            stack: [
                                      {
                                          text: [
                                          { text: "Cláusula Segunda do Valor e Recursos Orçamentários :" },
                                          ],
                                      }
                            ]
                        },
                         {
                             margin: [40, 10, 50, 0],
                             fontSize: 10,
                             alignment: 'justify',
                             stack: [
                                       {
                                           text: [
                                           { text: "O valor total estimado do presente contrato passa a ser de R$ " },
                                           { text: config.valorTotalContrato },                                          
                                           ],
                                       }
                             ]
                         },
                         {
                             margin: [20, 0, 50, 0],
                             fontSize: 10,
                             alignment: 'justify',
                             stack: [
                                       {
                                           text: [
                                            { text: " ( " },
                                           { text: config.valorPorExtenso },
                                           { text: " ) para o período de " },
                                           { text: config.IdPeriodoContrato },
                                           { text: " ( " },
                                           { text: config.periodoPorExtenso },
                                           { text: " ) meses, sendo : " },
                                           ],
                                       }
                             ]
                         },
                          {
                              margin: [20, 10, 50, 0],
                              table: {                                  
                                  widths: [220, 215],
                                  body: config.body
                              },
                          },
                           {
                               margin: [20, 5, 50, 0],
                               fontSize: 10,
                               alignment: 'justify',
                               stack: [
                                         {
                                             text: [
                                             { text: ", onerando o orçamento em sua classificação orçamentária." },
                                             ],
                                         }
                               ]
                           },
                            {
                                margin: [20, 20, 50, 0],
                                fontSize: 10,
                                alignment: 'justify',
                                bold: true,
                                stack: [
                                          {
                                              text: [
                                              { text: "Cláusula Terceira da Ratificação :" },
                                              ],
                                          }
                                ]
                            },
                            {
                                margin: [40, 0, 50, 0],
                                fontSize: 10,
                                alignment: 'justify',
                                stack: [
                                          {
                                              text: [
                                              { text: "Permanecem em vigor as demais cláusulas e condições contratuais não alteradas pelo" },
                                              ],
                                          }
                                ]
                            },
                             {
                                 margin: [20, 0, 50, 0],
                                 fontSize: 10,
                                 alignment: 'justify',
                                 stack: [
                                           {
                                               text: [
                                               { text: " presente instrumento." },
                                               ],
                                           }
                                 ]
                             },
                            {
                                margin: [40, 0, 50, 0],
                                fontSize: 10,
                                alignment: 'justify',
                                stack: [
                                          {
                                              text: [
                                              { text: "E por estarem assim, justas e acertadas, firmam as partes o presente instrumento, na" },                                              
                                              ],
                                          }
                                ]
                            },
                             {
                                 margin: [20, 0, 50, 0],
                                 fontSize: 10,
                                 alignment: 'justify',
                                 stack: [
                                           {
                                               text: [
                                               { text: " presença de duas testemunhas, que também o assinam para todos os fins e efeitos de direito." },                                               
                                               ],
                                           }
                                 ]
                             },
                                {
                                    margin: [20, 50, 120, 0],
                                    fontSize: 10,
                                    alignment: "right",
                                    stack: [
                                        {
                                            text:
                                                [
                                                      { text: config.descricaoMunicipio + ", " },
                                                      { text: config.DtImpressaodia + " de " + config.DtImpressaonomemes + " de " + config.DtImpressaoano },
                                                ],
                                        }

                                    ],
                                },
                                 {
                                     margin: [20, 40, 50, 0],
                                     fontSize: 10,
                                     stack: [
                                         { text: "_______________________________________________" },
                                         { text: "Ordenador Despesa" },

                                     ],

                                 },
                                 {
                                     margin: [20, 40, 50, 0],
                                     fontSize: 10,
                                     stack: [
                                         { text: "_______________________________________________" },
                                         { text: "Credor" },

                                     ],

                                 },
                                  {
                                      margin: [20, 50, 50, 0],
                                      fontSize: 10,
                                      bold: true,
                                      stack: [
                                          { text: "Testemunhas" },
                                        
                                      ],

                                  },
                               {
                                   margin: [20, 40, 50, 0],
                                   fontSize: 10,
                                   bold: true,
                                   stack: [
                                       { text: "___________________________________" + "                                " + "_______________________________________", },
                                       { text: "Nome : "                             + "                                                                                 " + "Nome : ", },
                                       { text: "RG : "                               + "                                                                                  " + "     RG : ", },
                                       { text: "CPF : "                              + "                                                                                  " + "   CPF : ", },

                                   ],

                               },
                                 
            ],
            footer: function (currentPage, pageCount) {
                return {
                    margin: [50, 0, 50, 0],
                    columns: [
                        {
                            table: {
                                widths: [100, 100, 230],
                                headerRows: 1,
                                fontSize: 9,
                                body: [
                                         [{ text: "Endereço UGE : " + config.descricaoEnderecoNumero + "                     pagina(s) " + currentPage.toString() + " de " + pageCount, colSpan: 3, fontSize: 9, border: [false, true, false, false], bold: false, color: 'gray' }],

                                ]
                            },
                        },
                    ]
                };

            },
            styles: {
                destaque: {
                    margin: [20, 0, 40, 0],
                    fontSize: 13,
                    bold: true,
                    alignment: 'justify',
                },
                header: {
                    fontSize: 10,
                    bold: false,
                    margin: [50, 0, 30, 0]
                },
                tabela: {
                    margin: [20, 0, 40, 0], color: 'gray'
                },
                tablecss: {
                    margin: [20, 0, 40, 0],
                   
                },
                simples: {
                    margin: [20, 0, 40, 0],
                    fontSize: 13,
                    bold: false,
                    alignment: 'justify',
                }
            },
       
        }

        return doc2;

    };
    sedPdfExporter.exportPdf(config);
}