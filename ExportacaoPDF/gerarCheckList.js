function gerarCheckList(data) {
    ...
    config.nomeDocumento = data[0].nomeDocumento != undefined ? data[0].nomeDocumento : "";
    ...
    var count = 0;
    if (dados.length > 0) {
        var pesquisaPreco = false;
        for (var key in dados) {
            debugger
            var nomeDocumento = dados[key].nomeDocumento != undefined ? dados[key].nomeDocumento.toString() : "";

            var nrFolha = dados[key].nrFolha != undefined ? dados[key].nrFolha.toString() : "";
            var nrFolhaPesquisa = dados[key].nrFolhaPesquisa != undefined ? dados[key].nrFolhaPesquisa.toString() : "";
            var empresa = ['EMPRESA A', 'EMPRESA B', 'EMPRESA C'];
            if (dados[key].IdProrrogacaoDocumento == DOCS.Quadro_Comparativo_Precos.value && pesquisaPreco == false) {
                pesquisaPreco = true;

                for (var i = 0; i < dados[key].listProrrogacaoPesquisa.length; i++) {
                    body.push([
                          { text: "Pesquisa de Preço - " + empresa[i]},
                          { text: dados[key].listProrrogacaoPesquisa[i].nrFolhaPesquisa.toString() },
                    ]);
                }
            }
            ...
            if (nomeDocumento != nomeDocumentoAnt) {
                if (nomeDocumento != "") {
                    ...
                    if (nomeDocumento != "Gerar Check List") {
                       ...
                       { text: nomeDocumento.toString()},
                       { text: nrFolha.toString()},
            ...
            var nomeDocumentoAnt = dados[key].nomeDocumento != undefined ? dados[key].nomeDocumento.toString() : "";

            var nrFolhaAnt = dados[key].nrFolha != undefined ? dados[key].nrFolha.toString() : "";
        ...
        var doc =
        {
           ...
           },
           {
               ...
               stack: [
                   ...
                   {
                       text: "CHECK LIST - DOCUMENTOS DE PRORROGAÇÃO"
                   }
               ],
           }
            ],
