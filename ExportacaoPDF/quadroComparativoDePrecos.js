function gerarquadroComparativoDePrecos(data) {

    var config = {
        pageSize: "A4",
        pageOrientation: "landscape",
        title: "QuadroComparativoDePrecos",
        margins: [0, 0, 0, 0]
    };

    sedPdfExporter.normalizeConfig(config);

    //config.debug = true;
    config.codigoUnidadeGestora = data[0].unidadeGestora.codigoUnidadeGestora != undefined ? data[0].unidadeGestora.codigoUnidadeGestora : "";
    config.nomeUnidadeGestora = data[0].unidadeGestora.nomeUnidadeGestora != undefined ? data[0].unidadeGestora.nomeUnidadeGestora : "";
    config.codigoUnidadeGestora = data[0].unidadeGestora.codigoUnidadeGestora != undefined ? data[0].unidadeGestora.codigoUnidadeGestora : "";
    config.descricaoEnderecoNumero = data[0].unidadeGestora.descricaoEnderecoNumero != undefined ? data[0].unidadeGestora.descricaoEnderecoNumero : "";
    config.descricaoMunicipio = data[0].unidadeGestora.descricaoMunicipio != undefined && data[0].unidadeGestora.descricaoMunicipio != "" ? titleize(data[0].unidadeGestora.descricaoMunicipio.toLowerCase()) : "São Paulo";
    config.descricaoUnidadeContratada = data[0].descricaoUnidadeContratada != undefined ? data[0].descricaoUnidadeContratada : "";
    if (config.descricaoUnidadeContratada == 1) {
        config.descricaoUnidadeContratada = "Mensal";
    }
    else if (config.descricaoUnidadeContratada == 2) {
        config.descricaoUnidadeContratada = "Diário";
    }
    config.periodoPorExtenso = data[0].periodoPorExtenso != undefined ? data[0].periodoPorExtenso : "";

    config.numeroProcesso = data[0].processo.numeroProcesso != undefined ? data[0].processo.numeroProcesso : "";
    config.NrContrato = data[0].numeroContrato != undefined ? data[0].numeroContrato : "";
    config.NmCpfCgcCredor = data[0].nomeCredor != undefined ? data[0].nomeCredor : "";
    config.NrCpfCgcCredor = data[0].numeroCpfCnpj != undefined ? data[0].numeroCpfCnpj.replace(/^(\d{2})(\d{3})(\d{3})(\d{4})(\d{2})/, "$1.$2.$3/$4-$5") : "";
    config.descricaoObjeto = data[0].descricaoObjeto != undefined ? data[0].descricaoObjeto : "";
    config.nomeGestor = data[0].nomeGestor != undefined ? titleize(data[0].nomeGestor) : "";
    config.valorTotalContrato = data[0].valorTotalContrato != undefined ? data[0].valorTotalContrato.toFixed(2).replace(".", ",").replace(/(\d)(?=(\d{3})+(?!\d))/g, "$1.") : "";
    config.vrTotalContrato = data[0].valorTotalContrato != undefined ? data[0].valorTotalContrato : "";
    config.dataImpressao = data[0].dataImpressao != undefined ? dataAtualFormatada(data[0].dataImpressao) : "";
    config.DtImpressaodia = data[0].dataImpressao != undefined ? dataAtualFormatadadia(data[0].dataImpressao) : "";
    config.DtImpressaomes = data[0].dataImpressao != undefined ? dataAtualFormatadames(data[0].dataImpressao) : "";
    config.DtImpressaonomemes = data[0].dataImpressao != undefined ? dataAtualFormatadanomemes(data[0].dataImpressao) : "";
    config.DtImpressaoano = data[0].dataImpressao != undefined ? dataAtualFormatadaano(data[0].dataImpressao) : "";

    config.VlEmpresa1 = data[0].VlEmpresa1 != undefined ? data[0].VlEmpresa1.toFixed(2).replace(".", ",").replace(/(\d)(?=(\d{3})+(?!\d))/g, "$1.") : "";
    config.VlEmpresa2 = data[0].VlEmpresa2 != undefined ? data[0].VlEmpresa2.toFixed(2).replace(".", ",").replace(/(\d)(?=(\d{3})+(?!\d))/g, "$1.") : "";
    config.VlEmpresa3 = data[0].VlEmpresa3 != undefined ? data[0].VlEmpresa3.toFixed(2).replace(".", ",").replace(/(\d)(?=(\d{3})+(?!\d))/g, "$1.") : "";

    config.DsEmpresa1 = data[0].DsEmpresa1 != undefined ? data[0].DsEmpresa1 : "";
    config.DsEmpresa2 = data[0].DsEmpresa2 != undefined ? data[0].DsEmpresa2 : "";
    config.DsEmpresa3 = data[0].DsEmpresa3 != undefined ? data[0].DsEmpresa3 : "";

    config.CNPJ1 = data[0].CNPJ1 != undefined ? data[0].CNPJ1.replace(/^(\d{2})(\d{3})(\d{3})(\d{4})(\d{2})/, "$1.$2.$3/$4-$5") : "";
    config.CNPJ2 = data[0].CNPJ2 != undefined ? data[0].CNPJ2.replace(/^(\d{2})(\d{3})(\d{3})(\d{4})(\d{2})/, "$1.$2.$3/$4-$5") : "";
    config.CNPJ3 = data[0].CNPJ3 != undefined ? data[0].CNPJ3.replace(/^(\d{2})(\d{3})(\d{3})(\d{4})(\d{2})/, "$1.$2.$3/$4-$5") : "";

    config.MenorValor = data[0].MenorValor;

    var body = [];
    body.push([
        [{ text: "DESCRIÇÃO:", style: "tableHeader" },
         { text: "·", alignment: "center", bold: false, fontsize: 7, color: "white" },
         { text: "PRESTAÇÃO DE SERVIÇOS", alignment: "center", bold: true, fontsize: 11, color: "black" },
         { text: "DE", alignment: "center", bold: true, fontsize: 11, color: "black" },
         { text: config.descricaoObjeto, alignment: "center", bold: true, fontsize: 11, color: "black" }],

        { text: "EMPRESA A", style: "tableHeader" },

        { text: "EMPRESA B", style: "tableHeader" },

        { text: "EMPRESA C", style: "tableHeader" },

        [{ text: "MÉDIA DOS", alignment: "center", bold: true, fontsize: 11, color: "black" },
         { text: "TRÊS", alignment: "center", bold: true, fontsize: 11, color: "black" },
         { text: "ORÇAMENTOS", alignment: "center", bold: true, fontsize: 11, color: "black" }],

        [{ text: "MENOR", alignment: "center", bold: true, fontsize: 11, color: "black" },
         { text: "PREÇO DOS", alignment: "center", bold: true, fontsize: 11, color: "black" },
         { text: "TRÊS", alignment: "center", bold: true, fontsize: 11, color: "black" },
         { text: "ORÇAMENTOS", alignment: "center", bold: true, fontsize: 11, color: "black" }],

        [{ text: "VALOR", alignment: "center", bold: true, fontsize: 11, color: "black" },
         { text: "P/" + config.periodoPorExtenso + " " + config.descricaoUnidadeContratada, alignment: "center", bold: true, fontsize: 11, color: "black" },
         { text: "MESES", alignment: "center", bold: true, fontsize: 11, color: "black" },
         { text: "(Considerando", alignment: "center", bold: true, fontsize: 11, color: "black" },
         { text: "a média dos", alignment: "center", bold: true, fontsize: 11, color: "black" },
         { text: "três", alignment: "center", bold: true, fontsize: 11, color: "black" },
         { text: "orçamentos)", alignment: "center", bold: true, fontsize: 11, color: "black" }],

        [{ text: "VALOR TOTAL", alignment: "center", bold: true, fontsize: 11, color: "black" },
         { text: "P/" + config.periodoPorExtenso + " " + config.descricaoUnidadeContratada, alignment: "center", bold: true, fontsize: 11, color: "black" },
         { text: "(Considerando", alignment: "center", bold: true, fontsize: 11, color: "black" },
         { text: "o menor preço", alignment: "center", bold: true, fontsize: 11, color: "black" },
         { text: "dos três", alignment: "center", bold: true, fontsize: 11, color: "black" },
         { text: "orçamentos)", alignment: "center", bold: true, fontsize: 11, color: "black" }],

    ]);

    var total = 0;
    var mediadosTres = 0;
    var menorPrecoDosTres = 0;
    var totalMesesMedia = 0;
    var totalMesesMenor = 0;
    var economiaMediaMercado = 0;
    var ValorEmpresa1 = '';
    var ValorEmpresa2 = '';
    var ValorEmpresa3 = '';
    var periodo = 0;
    var valorTotalContrato = 0;

    var VlEmpresa1 = 0;
    var VlEmpresa2 = 0;
    var VlEmpresa3 = 0;

    VlEmpresa1 = data[0].VlEmpresa1;
    VlEmpresa2 = data[0].VlEmpresa2;
    VlEmpresa3 = data[0].VlEmpresa3;

    valorTotalContrato = config.valorTotalContrato;

    total = parseFloat(parseFloat(VlEmpresa1) + parseFloat(VlEmpresa2) + parseFloat(VlEmpresa3))

    mediadosTres = (total / 3);

   

   

    periodo = config.periodoPorExtenso;
    
    totalMesesMedia = (mediadosTres / periodo);
    totalMesesMenor = 0;
    var empresaMenorValor = Math.min(VlEmpresa1, VlEmpresa2, VlEmpresa3);
    totalMesesMenor = (empresaMenorValor / periodo);


    economiaMediaMercado = (((mediadosTres / data[0].valorTotalContrato) - 1)*100);
    debugger;
    mediadosTres = mediadosTres.toFixed(2).replace(".", ",").replace(/(\d)(?=(\d{3})+(?!\d))/g, "$1.")
    totalMesesMedia = totalMesesMedia.toFixed(2).replace(".", ",").replace(/(\d)(?=(\d{3})+(?!\d))/g, "$1.")
    totalMesesMenor = totalMesesMenor.toFixed(2).replace(".", ",").replace(/(\d)(?=(\d{3})+(?!\d))/g, "$1.")
    config.MenorValor = config.MenorValor.toFixed(2).replace(".", ",").replace(/(\d)(?=(\d{3})+(?!\d))/g, "$1.")

    config.mediadosTres = mediadosTres;
    config.totalMesesMedia = totalMesesMedia;
    config.totalMesesMenor = totalMesesMenor;
    config.economiaMediaMercado = economiaMediaMercado.toFixed(2).replace(".", ",").replace(/(\d)(?=(\d{3})+(?!\d))/g, "$1.");
 
    body.push([
        [{ text: "RAZÃO SOCIAL"}],

            { text: config.DsEmpresa1 },
            { text: config.DsEmpresa2 },
            { text: config.DsEmpresa3 },
            { text: "" },
            { text: "" },
            { text: "" },
            { text: "" }
    ]);

    body.push([
        [{ text: "CNPJ" }],

            { text: config.CNPJ1},
            { text: config.CNPJ2},
            { text: config.CNPJ3},
            { text: ""  },
            { text: ""  },
            { text: ""  },
            { text: ""  }
    ]);

    body.push([
        [{ text: "VALORES R$" }],

            { text: config.VlEmpresa1 },
            { text: config.VlEmpresa2 },
            { text: config.VlEmpresa3 },


            { text: config.mediadosTres},
            { text: config.MenorValor},
            { text: config.totalMesesMedia},
            { text: config.totalMesesMenor}
    ]);


    if (config.codigoUnidadeGestora == "080101" || config.codigoUnidadeGestora == "080102" ||
      config.codigoUnidadeGestora == "080011" || config.codigoUnidadeGestora == "080104" ||
      config.codigoUnidadeGestora == "080259" || config.codigoUnidadeGestora == "080353" ||
      config.codigoUnidadeGestora == "080356" || config.codigoUnidadeGestora == "080357" ||
      config.codigoUnidadeGestora == "080358" || config.codigoUnidadeGestora == "080359" ||
      config.codigoUnidadeGestora == "080040" || config.codigoUnidadeGestora == "080360"
      ) {

        config.descricaoEnderecoNumero = "Endereço SEE : Praça da República, 53 - República, São Paulo/SP";
    }


    config.body = body;
    config.docGenerator = function (config) {
        var doc =
        {

            pageMargins: [30, 150, 50, 30],
            header: [
           {
               margin: [50, 20, 50, 0],
               table: {
                   widths: ["5%", "95%"],
                   body: [
                       [
                           {
                               stack: [
                                   {
                                       image: 'data:image/png;base64,',
                                       width: 65,
                                       height: 75
                                   }
                               ]
                           },
                           {
                               stack: [
                                   {
                                       text: 'GOVERNO DO ESTADO DE SÃO PAULO',
                                       fontSize: 15,
                                       bold: true
                                   },
                                   {
                                       text: 'SECRETARIA DE ESTADO DA EDUCAÇÃO',
                                       fontSize: 14,
                                       bold: true
                                   },
                                   {
                                       text: config.nomeUnidadeGestora,
                                       fontSize: 11,
                                       bold: false
                                   },

                               ],
                               alignment: "center",
                           }
                       ]
                   ]
               },
               layout: "noBorders"
           },
           {
               alignment: "center",
               margin: [50, 0, 20, 0],
               stack: [

                   {
                       text: "QUADRO COMPARATIVO DE PREÇO"
                   }
               ],
           }
            ],

            content: [
                 {
                     margin: [20, 10, 50, 0],
                     stack: [
                                 {
                                     fontSize: 10,
                                     text:
                                      [
                                         { text: "PROCESSO Nº : ", bold: true },
                                         { text: config.numeroProcesso, bold: false },
                                      ],
                                 },
                                 {
                                     fontSize: 10,
                                     text:
                                      [
                                         { text: "CONTRATO Nº : ", bold: true },
                                         { text: config.NrContrato, bold: false },
                                      ],
                                 },
                                 {
                                     fontSize: 10,
                                     text:
                                         [
                                     { text: "ASSUNTO PLANEJADO : ", bold: true },
                                     { text: config.descricaoObjeto, bold: false },
                                         ],
                                 }
                     ]

                 },
                  {
                      margin: [20, 10, 20, 0],
                      fontSize: 10,
                      table: {
                          margin: [20, 0, 20, 0],
                          widths: ["16%", "14%", "14%", "14%", "11%", "11%", "11%", "11%"],
                          body: config.body

                      },
                     
                  },
                   {
                       margin: [20, 20, 50, 0],
                       fontSize: 10,
                       alignment: 'justify',
                       stack: [
                                 {
                                     text: [
                                     { text: "Valor do contrato no mercado (considerando a média dos três orçamentos): R$ " },
                                     { text: config.mediadosTres },
                                     { text: "." },
                                    
                                     ],
                                 }
                       ]
                   },
                   {
                       margin: [20, 5, 50, 0],
                       fontSize: 10,
                       alignment: 'justify',
                       stack: [
                                 {
                                     text: [
                                     { text: "Valor do contrato no mercado (considerando o menor preço dos três orçamentos): R$ " },
                                     { text: config.MenorValor },
                                     { text: "." },

                                     ],
                                 }
                       ]
                   },
                   {
                       margin: [20, 5, 50, 0],
                       fontSize: 10,
                       alignment: 'justify',
                       stack: [
                                 {
                                     text: [
                                     { text: "Valor total atualizado do Contrato nº " },
                                     { text: config.NrContrato },
                                     { text: ", R$ " },
                                     { text: config.valorTotalContrato },
                                     { text: "." },
                                     ],
                                 }
                       ]
                   },
                   {
                       margin: [20, 10, 50, 0],
                       fontSize: 10,
                       alignment: 'justify',
                       stack: [
                                 {
                                     text: [
                                     { text: "Economia de " },
                                     { text: config.economiaMediaMercado },
                                     { text: " % em relação à média do mercado." },
                                    
                                     ],
                                 }
                       ]
                   },
                    {
                        margin: [20, 30, 50, 0],
                        fontSize: 10,
                        alignment: "right",
                        stack: [
                            {
                                text:
                                    [
                                          { text: config.descricaoMunicipio + ", " },
                                          { text: config.DtImpressaodia + " de " + config.DtImpressaonomemes + " de " + config.DtImpressaoano },
                                    ],
                            }

                        ],
                    },
                     {
                         margin: [20, 30, 50, 0],
                         fontSize: 10,
                         alignment: "center",
                         stack: [
                             { text: "_______________________________________________" },
                             { text: config.nomeGestor },
                             { text: "Gestor do Contarto" },

                         ],

                     }




            ],      
                    footer: function (currentPage, pageCount) {
                        return {
                            margin: [50, 0, 50, 0],
                            columns: [
                                {
                                    table: {
                                        widths: [200, 200, 300],
                                        headerRows: 1,
                                        fontSize: 9,
                                        body: [
                                                 [
                                                     {
                                                         colSpan: 3,
                                                         fontSize: 9,
                                                         border: [false, true, false, false],
                                                         bold: false, color: 'gray',
                                                         text: [
                                                            
                                                            { text: "Endereço UGE : " + config.descricaoEnderecoNumero  },
                                                            { text: "                                                              " },
                                                            { text: "                                                              " },
                                                            { text: "pagina(s) " + currentPage.toString() + " de " + pageCount },
                                                     ],
                                                     }

                                                 ],

                                        ]
                                    },
                                },
                            ]
                        };

                    },
            styles:
               {
              
                tabela: {
                    margin: [20, 0, 40, 0], color: 'gray'
                },
                tableHeader: {
                    bold: true,
                    fontSize: 11,
                    color: "black",
                    alignment: "center"
                },
                tabelafooter: {
                    margin: [20, 0, 40, 0], color: 'gray'
                },
                destaque: {
                    margin: [20, 0, 40, 0],
                    fontSize: 13,
                    bold: true
                },
                simples: {
                    fontSize: 14,
                    bold: false,
                }
            },
        }
        return doc;
    };

    sedPdfExporter.exportPdf(config);

}