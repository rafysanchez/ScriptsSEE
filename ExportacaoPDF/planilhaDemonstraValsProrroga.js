function gerarplanilhaDemonstraValsProrroga(data) {


    var config = {
        pageSize: "A4",
        pageOrientation: "portrait",
        title: "PlanilhaDemonstraValsProrroga",
    };
    ...
    if (config.tipoPeriodo == 1) {
        config.TipoContrada = "meses";
    }
    else if (config.tipoPeriodo == 2) {
    ...
    if (data[0].descricaoUnidadeContratada == "2") {

        body.push([
            [{ text: 'A) MÊS/ANO (De acordo com a', alignment: "center", bold: false, fontsize: 11, color: "black" },
             { text: 'nova vigência)', alignment: "left", bold: false, fontsize: 11, color: "black" },
             { text: '------------------------------------------------', fontsize: 11, color: "white", width: 48 }],

            [{ text: "B) QTDE DE", alignment: "center", bold: false, fontsize: 11, color: "black" },
             { text: "DIAS LETIVOS", alignment: "center", bold: false, fontsize: 11, color: "black" },
             { text: '----------------------------------', fontsize: 11, color: "white", width: 34 }],

            [{ text: "C) VALOR POR", alignment: "center", bold: false, fontsize: 11, color: "black" },
             { text: "DIA LETIVO", alignment: "center", bold: false, fontsize: 11, color: "black" },
             { text: '----------------------------------', fontsize: 11, color: "white", width: 34 }],

            [{ text: "D) VALOR", alignment: "center", bold: false, fontsize: 11, color: "black" },
             { text: "MENSAL", alignment: "center", bold: false, fontsize: 11, color: "black" },
             { text: '----------------------------------', fontsize: 11, color: "white", width: 34 }]
        ...
        var valorTotal = 0;
        var valorAno = 0;
        for (var i = 0; i < data[0].Parcelas.length; i++) {

            config.NrMes = data[0].Parcelas[i].NrMes;
            config.NrAno = data[0].Parcelas[i].NrAno;
            config.NrDiasLetivo = data[0].Parcelas[i].NrDiasLetivo;
            config.VlPorDiaLetivo = data[0].Parcelas[i].VlPordiaLetivo.replace(".", ",").replace(/(\d)(?=(\d{3})+(?!\d))/g, "$1.");

            config.VlParcela = data[0].Parcelas[i].VlParcela.replace(".", ",").replace(/(\d)(?=(\d{3})+(?!\d))/g, "$1.");

            body.push([
                [{ text: config.NrMes + '/' + config.NrAno, alignment: "center", bold: false, fontsize: 11, color: "black" },
                 { text: '------------------------------------------------', fontsize: 11, color: "white", width: 48 }],

                [{ text: "" + config.NrDiasLetivo, alignment: "center", bold: false, fontsize: 11, color: "black" },
                 { text: '----------------------------------', fontsize: 11, color: "white", width: 34 }],

                [{ text: config.VlPorDiaLetivo, alignment: "center", bold: false, fontsize: 11, color: "black" },
                 { text: '----------------------------------', fontsize: 11, color: "white", width: 34 }],

                [{ text: config.VlParcela, alignment: "center", bold: false, fontsize: 11, color: "black" },
                 { text: '----------------------------------', fontsize: 11, color: "white", width: 34 }]

            ]);
        }
        body.push([
                       { text: "Valor total do Contrato para os " + config.IdPeriodoContrato + " meses \n\n", colSpan: 3, style: 'tableHeader' },
                       { text: "", style: 'tableHeader' },
                       { text: "", style: 'tableHeader' },
                       { text: "", style: 'tableHeader' },
        ]);
        var parcelasLength = data[0].Parcelas.length;
        parcelasLength = (parcelasLength - 1);
        for (var i = 0; i < data[0].Parcelas.length; i++) {
            debugger;
            config.NrAno = data[0].Parcelas[i].NrAno;
            ...
            else if (parseInt(count) > 0 && config.NrAno != ano) {
                count = 0;
                body.push([
                                     { text: "Valor para o Exercicio " + ano + " meses \n\n", style: 'tableHeader' },
                                     { text: valorAno.toFixed(2).replace(".", ",").replace(/(\d)(?=(\d{3})+(?!\d))/g, "$1."), colSpan: 3, style: 'tableHeader' },
                                     { text: "", style: 'tableHeader' },
                                     { text: "", style: 'tableHeader' },
            ...
            valorAno += parseFloat(data[0].Parcelas[i].VlParcela.replace(".", " ").replace(",", ".").trim());
            if (parcelasLength == i) {
                body.push([
                                     { text: "Valor para o Exercicio " + ano + " " + config.tipoPeriodo + "  \n\n", style: 'tableHeader' },
                                     { text: valorAno.toFixed(2).replace(".", ",").replace(/(\d)(?=(\d{3})+(?!\d))/g, "$1."), colSpan: 3, style: 'tableHeader' },
                                     { text: "", style: 'tableHeader' },
                                     { text: "", style: 'tableHeader' },
        ...
        body.push([
        [
            { text: 'A) MÊS/ANO (De acordo com a nova vigência', fontsize: 9 },
        ],

        [
            { text: "D) VALOR MENSAL", fontsize: 9 },
        ]
        ...
        for (var i = 0; i < data[0].Parcelas.length; i++) {
            config.NrMes = data[0].Parcelas[i].NrMes;
            config.nrAno = data[0].Parcelas[i].NrAno;
            var valor = data[0].Parcelas[i].VlParcela;
            valorTotal += parseFloat(data[0].Parcelas[i].VlParcela.replace(".", " ").replace(",", ".").trim());
            config.VlParcela = valor.replace(".", ",").replace(/(\d)(?=(\d{3})+(?!\d))/g, "$1.");

            body.push([

                           [
                               { text: config.NrMes + '/' + config.nrAno },
                           ],

                           [
                               { text: config.VlParcela },
                           ]
        ...
        body.push([
                     { text: "Valor total do Contrato para os " + config.IdPeriodoContrato + " meses \n\n", colSpan: 1 },
                        { text: valorTotal.toFixed(2).replace(".", ",").replace(/(\d)(?=(\d{3})+(?!\d))/g, "$1.") },
        ]);
        ...
        for (var i = 0; i < data[0].Parcelas.length; i++) {
            ...
            if (parseInt(count) == 0) {
                ...
                body.push([
                                     { text: "Valor para o Exercicio " + ano + " meses \n\n", colSpan: 1 },
                                     { text: valorAno.toFixed(2).replace(".", ",").replace(/(\d)(?=(\d{3})+(?!\d))/g, "$1.") },
    ...
    config.body = body;
    config.dataVigenciaIni = dataAtualFormatada(data[0].dataVigenciaIni);
    config.dataVigenciaFim = dataAtualFormatada(data[0].dataVigenciaFim);
        ...
        var doc =
        {
            ...
            },
            {
                ...
                stack: [
                    {
                        text: [
                        { text: "PLANILHA DEMONSTRATIVA DE VALORES PARA A PRORROGAÇÃO E CRONOGRAMA DE DESEMBOLSO ( Vigência : " },
